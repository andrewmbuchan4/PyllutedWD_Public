...
class DictPlotter:
    ...
    def yield_output(self, output_dir=None):
        for plot_name, plot in self.plots.items():
            print('Yielding output for ' + plot_name)
            plot.yield_output(output_dir)
        ...
        if self.filenames is not None:
            for filename in self.filenames:
                if output_dir is None:
                    ...
                    self.fig.savefig(output_dir + '/' + filename, dpi=self.dpi)
...
class Subplot:
    ...
    def draw(self, figure, gridspec=None, twin_subplot=None, sharex_subplot=None, sharey_subplot=None):
        ...
        for series_name, series in self.series.items():
            ...
            if series.legend:
                self.legend_labels.append(series_name)
                if series.plot_type in [PlotType.shade, PlotType.hline, PlotType.vline, PlotType.arrow]:
        ...
        if self.x_tick_labels is not None:
            self.ax.set_xticks(list(range(0, len(self.x_tick_labels))))
...
class Series:
    ...
    def __init__(self, input_dict):
        self.mandatory_args = {
            ...
            PlotType.contour_scatter: [
                'x_data',
                'y_data',
                'z_data',
                'fill'
            ],
            PlotType.tricontour_scatter: [
                'x_data',
                'y_data',
                'z_data',
                'fill'
    ...
    def draw(self, ax, figure):
        if self.plot_type == PlotType.function_2d:
        ...
        elif self.plot_type == PlotType.scatter_2d:
        ...
        elif self.plot_type == PlotType.function_3d:
            x = np.linspace(self.x_start, self.x_end, self.x_points)
            ...
            return contours
        elif self.plot_type == PlotType.contour_scatter:
            ...
            return contours
        elif self.plot_type == PlotType.tricontour_scatter:
            if self.fill:
            ...
            return vline
        elif self.plot_type == PlotType.hline:
            ...
            return hline
        elif self.plot_type == PlotType.text:
            text_args = {
            ...
            return text
        elif self.plot_type == PlotType.shade:
            ...
            return shade
        elif self.plot_type == PlotType.scatter_2d_error:
            ...
            return errors
        elif self.plot_type == PlotType.ellipse:
            patch = ax.add_patch(self.generate_confidence_ellipse(ax))
            return patch
        elif self.plot_type == PlotType.arrow:
            ...
            return arrow
        elif self.plot_type == PlotType.hist2d:
            ...
            if self.step:
                ...
                if self.bar_align is None:
                    bar = ax.bar(self.x_data, self.y_data, width=self.bar_width, bottom=self.bar_bottom, color=self.colours, edgecolor=self.edge_colours, alpha=self.face_alpha)
                else:
                    bar = ax.bar(self.x_data, self.y_data, width=self.bar_width, bottom=self.bar_bottom, align=self.bar_align, color=self.colours, edgecolor=self.edge_colours)
            return bar
        elif self.plot_type == PlotType.pie:
            pie = ax.pie(self.x_data, labels=self.labels, colors=self.colours, startangle=90)
            return pie
        else:
