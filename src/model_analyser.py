...
class ModelAnalyser:
    ...
    def confidence_intervals(self, input_array):
        # Cumulative probability of +1 sigma level (0.8413) (0.1587)
        sig_1 = 50 + 68.26 / 2
        # Cumulative probability of +2 sigma level (0.977) (0.023)
        sig_2 = 50 + 95.4 / 2
        # Cumulative probability of +3 sigma level (0.9985) (0.0015)
        sig_3 = 50 + 99.7 / 2
        ...
        if length > 1:
            ...
            for i, sample in enumerate(input_array.T):
                ...
                toret[i, :] = [
                    np.percentile(sample_filter, pv) for pv in percentile_vals
                ]
            return (
                toret[:, 0],
                toret[:, 1],
            ...
            sample_filter = [
                a for a in input_array if a is not None and not np.isnan(a)
            ]
    ...
    def detection_significance(
        self, err_data, n_params, n_removed, basename_M1, basename_M2
    ):
        ...
        retrieved_M2 = pn.Analyzer(
            n_params=(n_params - n_removed), outputfiles_basename=basename_M2
        )
        s_M1 = retrieved_M1.get_stats()
        s_M2 = retrieved_M2.get_stats()

        # Best-fitting parameter combination from model 1
        best_fit_M1 = retrieved_M1.get_best_fit()
        # Maximum log-likelihood for model 1
        max_likelihood_M1 = best_fit_M1["log_likelihood"]
        # Best chi-square for model 1
        best_chi_square_M1 = -2.0 * (max_likelihood_M1 - norm_log)

        ln_Z_M1 = s_M1["global evidence"]
        # ^ Natural log of Bayesian evidence from model 1

        # Best-fitting parameter combination from model 2
        best_fit_M2 = retrieved_M2.get_best_fit()
        # Maximum log-likelihood for model 2
        max_likelihood_M2 = best_fit_M2["log_likelihood"]
        # Best chi-square for model 2
        best_chi_square_M2 = -2.0 * (max_likelihood_M2 - norm_log)

        ln_Z_M2 = s_M2["global evidence"]
    ...
    def compare(self, model_dict, observation_number, number_of_data_points):
        # model_dict should be an OrderedDict with the base model in first position.
        # Comparison of base to itself is to get ln_Z
        ...
        for model_name, model in model_dict.items():
            ...
            self.compare_two_models(
                model, base_model, observation_number, number_of_data_points
            )
    ...
    def find_best_heated_model(self, models, system, number_of_data_points, stats_file):
        heating_string = mp.model_parameter_strings[
        ...
        for model_name, model in models.items():
            if (
                model.prior_name == "Default"
                and heating_string in model.get_model_params()
            ):
                if best_heated_model_name is None:
                    best_heated_model_name = model_name
                    if len(model.comparison.items()) == 0:
                        # Compare model to itself - we just need the ln Z so actual
                        # comparison is arbitrary
                        self.compare_two_models(
                            model, model, system, number_of_data_points
                        )
                    ...
                    if test_model_lnZ > best_heated_model_lnZ:
                        ...
                        best_heated_model_name = model_name
        if best_heated_model_name is None:
            # Then there was no model which actually included heating, so no point
            # continuing
            ...
            with open(stats_file, "a", newline="", encoding="utf-8") as f:
                ...
                to_write.writerow([])
                to_write.writerow(["Best heated model name:", best_heated_model_name])

    def find_parameter_sigma(
        self, process_name, parameter, models, system, number_of_data_points, stats_file
    ):
        # Find sigma significance of a parameter that describes a certain process
        ...
        for model_name, model in models.items():
            if (
                model.prior_name == "Default"
                and parameter_string not in model.get_model_params()
            ):
        ...
        if best_model_name != best_non_param_model_name:
            if best_non_param_model_name not in models[best_model_name].comparison:
                self.compare_two_models(
                    ...
                    models[best_non_param_model_name],
                    system,
                ...
                with open(stats_file, "a", newline="", encoding="utf-8") as f:
                    ...
                    to_write.writerow(
                        [
                            ...
                            (
                                "N/A"
                                if models[best_model_name].best_model is None
                                else models[best_model_name].best_model
                            ),
                            models[best_model_name].comparison[
            ...
            param_sigma_str = str(
                models[best_model_name].comparison[best_non_param_model_name][
                    "n_sigma_model_base"
                ]
            )
            bayes_factor_str = str(
                models[best_model_name].comparison[best_non_param_model_name][
                    "Bayes_factor_model_base"
                ]
            )
            chi_sq_nondiff_str = str(
                models[best_model_name].comparison[best_non_param_model_name][
                    "chi_base_per_data_point"
                ]
            )
    ...
    def make_plots_and_dump_fit(
        ...
        bonus_error_lows,
        bonus_error_highs,
        suppress_graphical_output=False,
    ):
        (
            temp_stats,
            mass_stats,
    ...
    def make_all_plots(
        ...
        consider_thermohaline,
        bonus_fits=None,
        bonus_error_lows=None,
        bonus_error_highs=None,
        suppress_graphical_output=False,
    ):
        ...
        temp_stats = self.make_temperature_plot(
            chains_dir,
            model,
            model_name,
            wd_name_tuple,
        ...
        mass_stats = self.make_mass_plot(
            chains_dir,
            model,
            model_name,
            wd_name_tuple,
        ...
        accretion_rate_stats = self.make_accretion_rate_plot(
            chains_dir,
            model,
            model_name,
            wd_name_tuple,
        ...
        semisampled_eo_dict = self.make_semisampled_eo_distibution_plot(
            chains_dir,
            model_name,
            model,
            observation_number,
        ...
        if not suppress_graphical_output:
            ...
            self.make_time_plot(
                chains_dir, model, model_name, wd_name_tuple, observation_number
            )
            self.make_scaled_tevent_plot(
                chains_dir,
                model,
                model_name,
                white_dwarf,
            ...
            self.make_p_v_fO2_plot(
                chains_dir, model, model_name, wd_name_tuple, observation_number
            )
            self.make_pressure_plot(
                chains_dir, model, model_name, wd_name_tuple, observation_number
            )
            self.make_eo_distribution_plot(
                model, model_name, wd_name_tuple, observation_number, eo_samples
            )
        return (
            temp_stats,
            mass_stats,
    ...
    def get_eo_sigma(self, eo_samples):
        ...
        eo_sample_stats_no_nans = [
            eo for eo in eo_samples if eo is not None and not np.isnan(eo)
        ]
        ...
        return (
            median_excess_oxygen,
            upper_error_excess_oxygen - median_excess_oxygen,
            median_excess_oxygen - lower_error_excess_oxygen,
            eo_sigma_2,
            len(eo_sample_stats_no_nans),
            len(eo_samples),
            probability_of_result,
        )

    def get_predicted_abundances(self, model, parameter_values):
        # I don't think we should do this any more, we should use the abundance fits
        # found in make_composition_plot
        raise
        input_values = list()
        for param in mp.get_model_params_in_order():
            if mp.model_parameter_strings[param] in model.get_model_params():
                index = model.get_model_params().index(
                    mp.model_parameter_strings[param]
                )
    ...
    def reconstruct_composition_dict(
        self, bulk_composition, core_composition, mantle_composition
    ):
        toret = dict()
    ...
    def dump_model_fit(
        ...
        stats, weightpost, best_fit_lnz, best_fit_params = (
            self.get_stats_weightpost_and_best_fit(model, N_wd, chains_dir)
        )
        ...
        accretion_timescale_index = parameter_indices[
        ...
        # model_result, diagnostics, input_values = self.get_predicted_abundances(
        #     model, medians
        # )
        try:
            elements_list = list()
            for el in fit_data[white_dwarf.get_atmospheric_type().value][
                "Model median"
            ].keys():
        ...
        descaled_elements_results_dict = sa.scale_abundances_to_solar(
            fit_data[white_dwarf.get_atmospheric_type().value]["Model median"][0],
            white_dwarf.get_atmospheric_type().value,
            None,
            True,
        )
        ...
        for el in elements_list:
            try:
                raw_result = fit_data[white_dwarf.get_atmospheric_type().value][
                    "Model median"
                ][0][el]
                elements_results.append(raw_result)
                # ^ Note that each entry is a tuple, the 0th element is the median,
                # the others are the 16th and 84th percentiles
            ...
            (
                wd_abundance_els_list,
                wd_abundances_list,
                wd_errors_list,
                wd_lower_errors_list,
            ) = white_dwarf.get_measurements_as_lists(
                True, False, ci.usual_elements, True
            )
            (
                wd_excluded_abundance_els_list,
                wd_excluded_abundances_list,
                wd_excluded_errors_list,
                wd_excluded_lower_errors_list,
            ) = white_dwarf.get_measurements_as_lists(
                False, True, ci.usual_elements, True
            )
            wd_upper_bound_els_list, wd_abundance_upper_bounds_list = (
                white_dwarf.get_upper_bounds_as_lists(
                    True, False, ci.usual_elements, True
                )
            )
            (
                wd_excluded_upper_bound_els_list,
                wd_excluded_abundance_upper_bounds_list,
            ) = white_dwarf.get_upper_bounds_as_lists(
                False, True, ci.usual_elements, True
            )
            wd_lower_bound_els_list, wd_abundance_lower_bounds_list = (
                white_dwarf.get_lower_bounds_as_lists(
                    True, False, ci.usual_elements, True
                )
            )
            (
                wd_excluded_lower_bound_els_list,
                wd_excluded_abundance_lower_bounds_list,
            ) = white_dwarf.get_lower_bounds_as_lists(
                False, True, ci.usual_elements, True
            )
        ...
        median_composition_dict = self.reconstruct_composition_dict(
            bulk_composition_medians,
            core_composition_medians,
            mantle_composition_medians,
        )
        if (
            mp.model_parameter_strings[mp.ModelParameter.fragment_core_frac]
            in model.get_model_params()
            and mp.model_parameter_strings[mp.ModelParameter.pressure]
            in model.get_model_params()
        ):
            ...
            # ^ Argument here irrelevent, we will supply the needed abundances
            cmf_median = geo_model.convert_core_number_fraction_to_core_mass_fraction(
                median_fcf,
                median_composition_dict,
                # Caveat: using this^ composition for all fcf values is an approximation
                # - there may be systematic changes in composition with fcf
            )
            ...
            pollutant_composition = geo_model.get_earth_mix(
                median_fcf, median_composition_dict
            )
            for el in elements_list:
                try:
            ...
            radius_mass = geo_model.calculate_radius_and_mass(
                median_pressure, median_composition_dict
            )
            radius = radius_mass[0]
            mass = radius_mass[1]
            radius_mass_lower_limit = geo_model.calculate_radius_and_mass(
                median_pressure, median_composition_dict, True
            )
            radius_lower_limit = radius_mass_lower_limit[0]
            mass_lower_limit = radius_mass_lower_limit[1]
            mass_abundance_dict = geo_model.convert_number_abundances_to_mass(
                median_composition_dict
            )
            mantle_mass_fraction = 1 - geo_model.get_core_fraction_from_abundance_dict(
                mass_abundance_dict
            )
            mantle_mass = mantle_mass_fraction * mass
            # Temporary hack for Marc's thing! But this should be incorporated properly,
            # it's quite useful. And should really take a white dwarf object as an
            # argument rather than all the little bits
            input_dict = {model_prediction_string: pollutant_composition}
            eoc_stats = self.extract_and_plot_excess_oxygen(
                model,
                N_wd,
                input_dict,
                white_dwarf.full_name(),
                white_dwarf.get_abundance_values_dict(),
                white_dwarf.get_error_values_dict(),
                white_dwarf.get_timescale_values_dict(timescale_type),
                1,
                suppress_graphical_output,
            )
        else:
            ...
            try:
                ...
                # Bit of a cheap solution for now - this probably means Mg wasn't in the
                # data so skip this.
                # But really we should just use a different reference element
                eoc_stats = None

        accretion_timescale_extract = 10 ** (
            weightpost[:, accretion_timescale_index] - 6
        )
        ...
        bu_ss_cutoff = (
            5
            * white_dwarf.atmosphere_data.atmosphere_data_dict[
                timescale_type
            ].timescale_dict[ci.Element.Mg]
        ) / 1000000 # ^ Important to convert to Myr!
        # Physically we're saying it takes 5 Mg sinking times to reach steady state
        for t_acc_t_obs_pair in zip(
            accretion_timescale_extract, t_sinceaccretion_extract
        ):
        ...
        ARmedian, ARerrorplus, ARerrorminus = self.extract_median_and_error(
            accretion_rate_stats
        )

        pressure_stats = self.get_untransformed_parameter_stats(
            chains_dir, model, N_wd, mp.ModelParameter.pressure
        )
        ...
        for ox_strat, stats in eo_sample_stats.items():
            ...
            sampled_eo_stats[ox_strat][self.absolute_string] = self.get_eo_sigma(
                absolute_sample
            )
            fractional_sample = stats.get(self.fractional_string)
            sampled_eo_stats[ox_strat][self.fractional_string] = self.get_eo_sigma(
                fractional_sample
            )
        ...
        with open(stats_file, "a", newline="", encoding="utf-8") as f:
            ...
            to_write.writerow(
                ["Excluded Upper Bounds:"] + wd_excluded_abundance_upper_bounds_list
            )
            to_write.writerow(
                ["Excluded Lower Bounds:"] + wd_excluded_abundance_lower_bounds_list
            )
            to_write.writerow(["Output from median run:"])
            to_write.writerow(["Disc Composition:"] + disc_comp)
            # to_write.writerow(['Partition Coefficients (X_c for Oxygen):'] + ds)
            # ^ For old Oxygen logic
            ...
            to_write.writerow(
                ["Final result, expressed as (X/Hx):"] + descaled_elements_results
            )
            # try:
            ...
            to_write.writerow(
                [
                    "Fragment core mass fraction, +error, -error:",
                    cmf_median,
                    cmf_error_plus,
                    cmf_error_minus,
                ]
            ...
            to_write.writerow(
                ["delta time/Myrs, +error, -error:", tmedian, terrorplus, terrorminus]
            )
            ...
            to_write.writerow(
                ["Temperature /K, +error, -error:", Tmedian, Terrorplus, Terrorminus]
            )
            to_write.writerow(
                ["log(Mass /kg), +error, -error:", Mmedian, Merrorplus, Merrorminus]
            )
            to_write.writerow(
                [
                    "log(Accretion rate /gs^-1), +error, -error:",
                    ARmedian,
                    ARerrorplus,
                    ARerrorminus,
                ]
            ...
            to_write.writerow(
                ["Pressure /GPa, +error, -error:", Pmedian, Perrorplus, Perrorminus]
            )
            ...
            if eoc_stats is not None:
                for oa_strat, oa_stats in eoc_stats["Model prediction"].items():
                    ...
                    to_write.writerow(
                        ["Oxygen Assignation Elements:"] + oa_elements[oa_strat]
                    )
                    to_write.writerow(
                        ["Oxygen Assignations:"] + oa_assignations[oa_strat]
                    )
                if eoc_stats.get("Model with O data point") is not None:
                    for oa_strat, oa_stats in eoc_stats[
                        "Model prediction, data O"
                    ].items():
                        to_write.writerow(
                            ["Excess Oxygen using O data point:", oa_stats[0]]
                        )
                if eoc_stats.get("Model prediction, data (SS)") is not None:
                    for oa_strat, oa_stats in eoc_stats[
                        "Model prediction, data (SS)"
                    ].items():
                        list_of_els = list()
            ...
            for ox_strat, stats in sampled_eo_stats.items():
                ...
                to_write.writerow(
                    [
                        ...
                        stats[self.absolute_string][0],
                        stats[self.absolute_string][1],
                        stats[self.absolute_string][2],
                    ]
                )
                to_write.writerow(
                    [
                        "Sampled Excess Oxygen Sigma Significance:",
                        stats[self.absolute_string][3],
                    ]
                )
                to_write.writerow(
                    [
                        "Sampled Oxygen Excess or Deficit Rate (whichever greater):",
                        stats[self.absolute_string][6],
                    ]
                )
                to_write.writerow(
                    [
                ...
                to_write.writerow(
                    [
                        "Sampled Fractional Excess Oxygen, +error, -error:",
                        stats[self.fractional_string][0],
                        stats[self.fractional_string][1],
                        stats[self.fractional_string][2],
                    ]
                )
                to_write.writerow(
                    [
                        "Sampled Fractional Excess Oxygen Sigma Significance:",
                        stats[self.fractional_string][3],
                    ]
                )
                to_write.writerow(
                    [
            ...
            with open(sample_dump, "w", newline="", encoding="utf-8") as f:
                ...
                to_write.writerow(
                    [
                        ...
                        "Time Since Accretion /Myr",
                        "Delta /Myr",
                    ]
                )
                zipped = zip(
                    accretion_timescale_extract, t_sinceaccretion_extract, delta_time
                )
                for z in zipped:
                    to_write.writerow(z)
    ...
    def get_stats_weightpost_and_best_fit(self, model, observation_number, chains_dir):
        a = pn.Analyzer(
            n_params=model.get_n_dims(),
            outputfiles_basename=model.get_full_prefix(chains_dir),
        )
        stats = a.get_stats()
        weightpost = a.get_equal_weighted_posterior()[:, 0 : model.get_n_dims()]
        # ^ This is basically just excluding the final column of the
        # ...post_equal_weights.dat file
        best_fit = a.get_best_fit()
        return stats, weightpost, best_fit["log_likelihood"], best_fit["parameters"]

    def extract_and_plot_excess_oxygen(
        self,
        model,
        observation_number,
        input_dict,
        ...
        eoc_stats = self.excess_oxygen_calculator.run_full_calculation(
            input_dict, mass, True, True, observations, errors, wd_timescales
        )
        if not suppress_graphical_output:
    ...
    def make_corner_plot(self, model, observation_number, wd_name_tuple, chains_dir):
        stats, weightpost, best_fit_lnz, best_fit_params = (
            self.get_stats_weightpost_and_best_fit(
                model, observation_number, chains_dir
            )
        )

        parameter_indices = mp.parameter_indices(ld._live_model)
        t_sinceaccretion_index = parameter_indices[mp.ModelParameter.t_sinceaccretion]
        t_sinceaccretion_scaled = np.log10(
            weightpost[:, t_sinceaccretion_index] * 1000000
        )
        ...
        for dfp in model.get_model_params():
            if "Δ" in dfp:
                ...
                delta_free_params.append(dfp.replace("Δ", ""))
                # The corner function doesn't handle ^^^this character when
                # plt.rcParams['text.usetex'] = 'True' in dict_plotter
            else:
                if (
                    dfp
                    == mp.model_parameter_strings[mp.ModelParameter.t_sinceaccretion]
                ):
    ...
    def make_composition_plot(
        ...
        consider_thermohaline,
        bonus_mk2_fits=None,
        bonus_mk2_error_lows=None,
        bonus_mk2_error_highs=None,
        suppress_graphical_output=False,
    ):
        normalisation_element = ci.Element.Mg
        reference_elements = [
            normalisation_element,
            white_dwarf.get_atmospheric_type().value,
        ]
        # ^ Having both of these is a bit redundant for the Fragment case so that could be
        # improved!

        fixed_pressure_version = False
        # ^Set this to True to produce a version of this plot with an extra line
        # corresponding to a certain fixed pressure
        fixed_pressure = 0
        ...
        samples = np.random.choice(
            number_of_possible_samples, number_of_samples, replace=False
        )
        ...
        for element in reference_elements:
            ...
            sampled_fragment_compositions[element] = np.zeros(
                shape=(number_of_samples, len_x_axis)
            )
        ...
        if fixed_pressure_version:
            ...
            for element in reference_elements:
                fp_model_output[element] = np.zeros(
                    shape=(number_of_samples, len_x_axis)
                )
        ...
        for i in range(number_of_samples):
            ...
            fe_star = (
                ...
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.metallicity
                )
                else mp.default_values[enhancement_model][mp.ModelParameter.metallicity]
            )
            t_sinceaccretion = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.t_sinceaccretion]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.t_sinceaccretion
                )
                else mp.default_values[enhancement_model][
                    mp.ModelParameter.t_sinceaccretion
                ]
            )
            d_formation = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.formation_distance]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.formation_distance
                )
                else mp.default_values[enhancement_model][
                    mp.ModelParameter.formation_distance
                ]
            ...
            z_formation = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.feeding_zone_size]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.feeding_zone_size
                )
                else mp.default_values[enhancement_model][
            ...
            N_c = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.parent_core_frac]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.parent_core_frac
                )
                else mp.default_values[enhancement_model][
            ...
            N_o = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.parent_crust_frac]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.parent_crust_frac
                )
                else mp.default_values[enhancement_model][
            ...
            f_c = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.fragment_core_frac]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.fragment_core_frac
                )
                else mp.default_values[enhancement_model][
            ...
            f_o = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.fragment_crust_frac]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.fragment_crust_frac
                )
                else mp.default_values[enhancement_model][
            ...
            fragment_mass = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.fragment_mass]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.fragment_mass
                )
                else mp.default_values[enhancement_model][
            ...
            log_t_disc = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.accretion_timescale]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.accretion_timescale
                )
                else mp.default_values[enhancement_model][
                    mp.ModelParameter.accretion_timescale
                ]
            ...
            pressure = (
                weightpost[samples[i], parameter_indices[mp.ModelParameter.pressure]]
                if mp.model_uses_parameter(ld._live_model, mp.ModelParameter.pressure)
                else mp.default_values[enhancement_model][mp.ModelParameter.pressure]
            )
            fO2 = (
                weightpost[
                    samples[i], parameter_indices[mp.ModelParameter.oxygen_fugacity]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.oxygen_fugacity
                )
                else mp.default_values[enhancement_model][
            ...
            for ref_element in reference_elements:
                scaled_abundances = sa.scale_abundances_to_solar(
                    expressions, ref_element
                )
                model_output[ref_element][i, :] = [
                    scaled_abundances[element] for element in ci.usual_elements
                ]
            try:
                eoc_comp = geo_model_for_solar_abundances.get_earth_mix(
                    f_c, diagnostics["Enhancements"]["Abundances"]
                )
            except (TypeError, KeyError):
            ...
            sampled_disc_abundances[i, :] = [
                diagnostics["DiscAbundances"][element] for element in ci.usual_elements
            ]
            if (
                diagnostics["Enhancements"] is not None
                and "ParentCoreNumberFraction" in diagnostics["Enhancements"]
            ):
                sampled_pcnfs.append(
                    diagnostics["Enhancements"]["ParentCoreNumberFraction"]
                )
            if (
                diagnostics["Enhancements"] is not None
                and "Abundances" in diagnostics["Enhancements"]
            ):
                sampled_bulk_compositions[i, :] = [
                    diagnostics["Enhancements"]["Abundances"][element][gi.Layer.bulk]
                    for element in ci.usual_elements
                ]
                sampled_core_compositions[i, :] = [
                    diagnostics["Enhancements"]["Abundances"][element][gi.Layer.core]
                    for element in ci.usual_elements
                ]
                sampled_mantle_compositions[i, :] = [
                    diagnostics["Enhancements"]["Abundances"][element][gi.Layer.mantle]
                    for element in ci.usual_elements
                ]
            if (
                diagnostics["Enhancements"] is not None
                and "Ds" in diagnostics["Enhancements"]
            ):
                sampled_partition_compositions[i, :] = [
                    diagnostics["Enhancements"]["Ds"][element]
                    for element in ci.usual_elements
                ]
            if (
                diagnostics["Enhancements"] is not None
                and "FragmentLogAbundances" in diagnostics["Enhancements"]
            ):
                for ref_element in reference_elements:
                    scaled_fragment_composition = sa.scale_abundances_to_solar(
                        diagnostics["Enhancements"]["FragmentLogAbundances"],
                        ref_element,
                        normalisation_element,
                    )
                    sampled_fragment_compositions[ref_element][i, :] = [
                        scaled_fragment_composition[element]
                        for element in ci.usual_elements
                    ]
            if fixed_pressure_version:
                for ref_element in reference_elements:
                    scaled_abundances = sa.scale_abundances_to_solar(
                        fp_expressions, ref_element
                    )
                    fp_model_output[ref_element][i, :] = [
                        scaled_abundances[element] for element in ci.usual_elements
                    ]
        disc_abundance_medians = dict(
            zip(
                ci.usual_elements, self.confidence_intervals(sampled_disc_abundances)[3]
            )
        )
        bulk_composition_medians = dict(
            zip(
                ci.usual_elements,
                self.confidence_intervals(sampled_bulk_compositions)[3],
            )
        )
        core_composition_medians = dict(
            zip(
                ci.usual_elements,
                self.confidence_intervals(sampled_core_compositions)[3],
            )
        )
        mantle_composition_medians = dict(
            zip(
                ci.usual_elements,
                self.confidence_intervals(sampled_mantle_compositions)[3],
            )
        )
        partition_coefficient_medians = dict(
            zip(
                ci.usual_elements,
                self.confidence_intervals(sampled_partition_compositions)[3],
            )
        )
        pcnf_median = (
            np.percentile(sampled_pcnfs, 50) if len(sampled_pcnfs) > 0 else None
        )
        for ref_element in reference_elements:
            (
                model_low3,
                ...
                model_high2,
                model_high3,
            ) = self.confidence_intervals(model_output[ref_element])
            fit_data[ref_element]["Model median"] = (
                dict(zip(ci.usual_elements, model_median)),
                dict(zip(ci.usual_elements, model_low1)),
                dict(zip(ci.usual_elements, model_high1)),
            )
            (
                model_low3,
                model_low2,
                ...
                model_high2,
                model_high3,
            ) = self.confidence_intervals(sampled_fragment_compositions[ref_element])
            fit_data[ref_element]["Fragment"] = (
                dict(zip(ci.usual_elements, model_median)),
                dict(zip(ci.usual_elements, model_low1)),
                dict(zip(ci.usual_elements, model_high1)),
            )
            ...
            if fixed_pressure_version:
                (
                    ...
                    fp_model_high2,
                    fp_model_high3,
                ) = self.confidence_intervals(fp_model_output[ref_element])
                fit_data[ref_element][fixed_pressure_name] = (
                    dict(zip(ci.usual_elements, fp_model_median)),
                    dict(zip(ci.usual_elements, fp_model_low1)),
                    dict(zip(ci.usual_elements, fp_model_high1)),
                )
        ...
        if include_best_fit:
            fe_star = (
                best_fit_params[parameter_indices[mp.ModelParameter.metallicity]]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.metallicity
                )
                else mp.default_values[enhancement_model][mp.ModelParameter.metallicity]
            ...
            t_sinceaccretion = (
                best_fit_params[parameter_indices[mp.ModelParameter.t_sinceaccretion]]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.t_sinceaccretion
                )
                else mp.default_values[enhancement_model][
            ...
            d_formation = (
                best_fit_params[parameter_indices[mp.ModelParameter.formation_distance]]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.formation_distance
                )
                else mp.default_values[enhancement_model][
            ...
            t_formation = 1.5
            z_formation = (
                best_fit_params[parameter_indices[mp.ModelParameter.feeding_zone_size]]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.feeding_zone_size
                )
                else mp.default_values[enhancement_model][
            ...
            N_c = (
                best_fit_params[parameter_indices[mp.ModelParameter.parent_core_frac]]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.parent_core_frac
                )
                else mp.default_values[enhancement_model][
            ...
            N_o = (
                best_fit_params[parameter_indices[mp.ModelParameter.parent_crust_frac]]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.parent_crust_frac
                )
                else mp.default_values[enhancement_model][
            ...
            f_c = (
                best_fit_params[parameter_indices[mp.ModelParameter.fragment_core_frac]]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.fragment_core_frac
                )
                else mp.default_values[enhancement_model][
            ...
            f_o = (
                best_fit_params[
                    parameter_indices[mp.ModelParameter.fragment_crust_frac]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.fragment_crust_frac
                )
            ...
            fragment_mass = (
                best_fit_params[parameter_indices[mp.ModelParameter.fragment_mass]]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.fragment_mass
                )
                else mp.default_values[enhancement_model][
            ...
            log_t_disc = (
                best_fit_params[
                    parameter_indices[mp.ModelParameter.accretion_timescale]
                ]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.accretion_timescale
                )
            ...
            pressure = (
                best_fit_params[parameter_indices[mp.ModelParameter.pressure]]
                if mp.model_uses_parameter(ld._live_model, mp.ModelParameter.pressure)
                else mp.default_values[enhancement_model][mp.ModelParameter.pressure]
            )
            fO2 = (
                best_fit_params[parameter_indices[mp.ModelParameter.oxygen_fugacity]]
                if mp.model_uses_parameter(
                    ld._live_model, mp.ModelParameter.oxygen_fugacity
                )
                else mp.default_values[enhancement_model][
    ...
    def get_untransformed_parameter_stats(
        self, chains_dir, model, observation_number, model_param
    ):
        stats, weightpost, best_fit_lnz, best_fit_params = (
            self.get_stats_weightpost_and_best_fit(
                model, observation_number, chains_dir
    ...
    def make_pressure_plot(
        self, chains_dir, model, model_name, wd_name_tuple, observation_number
    ):
        p_stats = self.get_untransformed_parameter_stats(
            chains_dir, model, observation_number, mp.ModelParameter.pressure
        )
        if p_stats is None:
        ...
        heights, bins2 = np.histogram(p_stats, bins, density=True)
        geo_model = gi.GeologyModel()
        ...
        for p in pressure_vals:
            if p == 0:
                ...
                rm_tuple = geo_model.calculate_radius_and_mass(
                    p, geo_model.element_info
                )
    ...
    def make_eo_distribution_plot(
        self, model, model_name, wd_name_tuple, observation_number, eo_stats
    ):
        half_bin_size = 0.025
        for ox_strat, stats in eo_stats.items():
            for abs_or_frac in [self.absolute_string, self.fractional_string]:
                ...
                if eo_stats_to_use is not None:
                    ...
                    eo_stats_no_nans = [
                        eo
                        for eo in eo_stats_to_use
                        if eo is not None and not np.isnan(eo)
                    ]
                    ...
                    while min_bin > min(eo_stats_no_nans):
                        min_bin -= 0.1
                    bins, x_bar_centres = self.generate_bins_and_bar_centres(
                        min_bin, max_bin, half_bin_size
                    )

                    heights, bins2 = np.histogram(eo_stats_no_nans, bins, density=True)
    ...
    def make_semisampled_eo_distibution_plot(
        self,
        chains_dir,
        model_name,
        model,
        ...
        # medians = self.get_param_vals_by_percentile(model, weightpost, 50)
        # predicted_median_abundances, diagnostics, input_values_used = (
        #     self.get_predicted_abundances(model, medians)
        # )

        # I think really the underlying problem here is that the median abundances, and
        # the abundances calculated by forward modelling with the medians might not be
        # the same!
        # We should really just reuse the median abundances calculated from the
        # composition resampling, which should hopefully never be None
        ...
        t_obs_data = weightpost[:, t_sinceaccretion_index] * 1000000  # Myr -> yr
        t_event_data = 10 ** weightpost[:, accretion_timescale_index]  # log yr -> yr
        ...
        els_to_iterate = list(
            self.excess_oxygen_calculator.oxidation_dict[
                eoc.OxidationStrategy.default
            ].keys()
        )
        ...
        # Really the model name should not be hardcoded as a magic value like this!
        # It basically just needs to be the same name that we saved the resampled median
        # fits to in make_composition_plot
        descaled_elements_results_dict = sa.scale_abundances_to_solar(
            resampled_fit_data[white_dwarf.get_atmospheric_type().value][
                "Model median"
            ][0],
            white_dwarf.get_atmospheric_type().value,
            None,
        ...
        for el in ci.usual_elements:
            ...
            if (
                relevant_data is None
                or relevant_data.data_point_type
                != wd.WhiteDwarfDataPointType.measurement
            ):
            ...
            # else:
            #     # If that element didn't have an abundance, or was only an upper bound,
            #     # we'll use the abundance predicted by the model for the median
            #     # parameter values
        ...
        for el in els_to_iterate:
            ...
            t_el = white_dwarf.atmosphere_data.atmosphere_data_dict[
                timescale_type
            ].timescale_dict[el]
            N_el = 10**log_el_hx / (
                t_el
                * (
                    np.exp((np.minimum(t_event_data, t_obs_data) - t_obs_data) / t_el)
                    - np.exp(-t_obs_data / t_el)
                )
            )
        ...
        bins_many, x_bar_centres = self.generate_bins_and_bar_centres(
            np.floor(min(min(O_excess_def), min(O_excess_cons), min(O_excess_def_ss))),
            # ^ This is sometimes unnecessarily (and stupidly) low, but truncating it
            # will change the later maths. Solution is to remove unnecessary bins prior
            # to sending them over to the graph factory
            np.ceil(max(max(O_excess_def), max(O_excess_cons), max(O_excess_def_ss))),
            half_bin_size,
        )
        plot_scaling_factor = 100  # We want to plot % rather than the raw fraction
        x_bar_centres_percent = [plot_scaling_factor * x for x in x_bar_centres]

        heights_default, bins2 = np.histogram(O_excess_def, bins_many, density=True)

        n_cons, edges_def = np.histogram(O_excess_cons, bins=bins_many)

        lower_sigma_def = (
            np.median(O_excess_def)
            - bins_many[
                np.where(np.cumsum(heights_default) > 0.158 * np.sum(heights_default))
            ][0]
        )
        upper_sigma_def = bins_many[
            np.where(np.cumsum(heights_default) > 0.841 * np.sum(heights_default))
        ][0] - np.median(O_excess_def)
        # stat.norm.ppf(x) returns y such that the area under the curve of
        # Gaussian(mu=0, sigma=1) between -inf and y is x
        # Satisfies e.g. stat.norm.ppf(0.5) = 0, i.e. the sigma significance of a 50:50
        # event is 0
        sig_excess_def = abs(
            stat.norm.ppf(
                len(O_excess_def[np.where(O_excess_def > 0)]) / len(O_excess_def)
            )
        )
        p_excess_def = len(O_excess_def[np.where(O_excess_def > 0)]) / len(O_excess_def)
        p_deficit_def = len(O_excess_def[np.where(O_excess_def < 0)]) / len(
            O_excess_def
        )
        ...
        lower_sigma_cons = (
            np.median(O_excess_cons)
            - bins_many[np.where(np.cumsum(n_cons) > 0.158 * np.sum(n_cons))][0]
        )
        upper_sigma_cons = bins_many[
            np.where(np.cumsum(n_cons) > 0.841 * np.sum(n_cons))
        ][0] - np.median(O_excess_cons)

        median_def_ss = np.median(O_excess_def_ss)
        n_def_ss, edges_ss = np.histogram(O_excess_def_ss, bins=bins_many)
        lower_sigma_def_ss = (
            np.median(O_excess_def_ss)
            - bins_many[np.where(np.cumsum(n_def_ss) > 0.158 * np.sum(n_def_ss))][0]
        )
        upper_sigma_def_ss = bins_many[
            np.where(np.cumsum(n_def_ss) > 0.841 * np.sum(n_def_ss))
        ][0] - np.median(O_excess_def_ss)
        ...
        if filter_out_low_x:
            # Sometimes there are a huge number of bins at absurdly low x values that we
            # will never see on the graph - this logic gets rid of the most egregious
            # cases, which can seriously slow down run time.
            distances_to_centres = [
                abs(ignorable_x_min - x) for x in x_bar_centres_percent
            ]
    ...
    def build_model(self, hierarchy_name, hierarchy_level_list):
        ...
        # Make a pollution model consisting of the parameters in the relevant hierarchy
        ...
        for potential_param in mp.ModelParameter:
            mp.model_definitions_dict[model_name][potential_param] = (
                potential_param in parameters_to_use
            )
    ...
    def recreate_model(self, observation_number, timescale_type):
        ...
        return pm.PollutionModel(
            model_name, timescale_type, enhancement_model_name, prior_name, live_points
        )

    def make_variable_distribution_plot(
        self,
        variable_name,
        ...
        for comparison_dist_name, comparison_dist in comparison_dist_dict.items():
            ...
            heights, bins2 = np.histogram(comparison_dist, bins, density=True)
            all_heights.append(heights)
    ...
    def make_multisystem_pressure_plot(
        self,
        chains_dir,
        observation_numbers,
        timescale_type,
        ...
        hist_plot = self.graph_fac.make_histogram(
            x_bar_centres,
            all_heights,
            used_observation_numbers_list if labels is None else labels,
            prefix,
            half_bin_size * 2,
            0.8,
            "Pressure /GPa",
    ...
    def get_temperature_stats(self, chains_dir, model, observation_number):
        ...
        try:
            ...
            # If there was no formation_distance parameter, this calculation is
            # irrelevant
            return None
        T = np.zeros(len(weightpost))
        for i in range(0, len(weightpost)):
            T[i] = dm.T_disc(10 ** (weightpost[i, distance_index]), 1.5)
            # TODO: Consider having a variable like default_t_formation = 1.5 in some
            # high level module
        return T

    def generate_bins_and_bar_centres(self, min_bin_edge, max_bin_edge, half_bin_size):
        max_num_bins = 1000000
        # ^ Otherwise it's just silly (and can easily use ALL your memory!)
    ...
    def make_temperature_plot(
        ...
        heights, bins2 = np.histogram(temp_stats, bins, density=True)
        text_dict = {
            "icy_text": {
    ...
    def make_mass_plot(
        ...
        heights, bins2 = np.histogram(logmassofpollutant, bins, density=True)
        ...
        # ^ This is just so that the later functions don't need to recalculate it!

    def make_accretion_rate_plot(
        self,
        chains_dir,
        model,
        model_name,
        ...
        massofpollutant = 1000 * (
            10 ** weightpost[:, fragment_mass_index]
        )  # log(kg)->g
        t_event_samples = pc.seconds_per_year * (
            10 ** weightpost[:, t_event_index]
        )  # yr->s
        ...
        heights, bins2 = np.histogram(accretion_rate, bins, density=True)
    ...
    def make_time_plot(
        ...
        self.graph_fac.plot_timesince_v_accretiontime(
            x_data,
            y_data,
            wd_name_tuple[0],
            wd_name_tuple[1],
    ...
    def make_scaled_tevent_plot(
        ...
        steady_state_scaling_factor = 1
        # ^ The steady state panel takes up steady_state_scaling_factor * t_ss width on
        # the plot. Marc used 1
        half_bin_size = 0.1
        ...
        t_element = white_dwarf.atmosphere_data.atmosphere_data_dict[
            timescale_type
        ].timescale_dict[critical_element]

        t_ss = t_ss_scaling_factor * t_element  # Time to reach steady state

        # This bit calculates a scaled time coordinate for all possible phases of
        # accretion (apart from build-up because that's just t_obs_data).
        # We will pick the right one to use later
        scaled_ss_times = (
            (steady_state_scaling_factor * t_ss)
            * ((t_obs_data - t_ss) / (t_event_data - t_ss))
        ) + t_ss
        # ^ i.e., what fraction of the way through steady state are we?
        # Then rescale by arbitrary multiplicative factor so it looks ok on the plot,
        # then add a non-arbitrary offset so that it goes after the build-up phase
        scaled_declining_times = (
            (t_obs_data - t_event_data) + t_ss + (steady_state_scaling_factor * t_ss)
        )
        # ^i.e., how far into declining phase are you?
        # Now add the size of the build-up and steady state panels

        # This bit picks out the indices to which each possible accretion phase applies
        i_buildup = np.where((t_obs_data <= t_ss) & (t_obs_data <= t_event_data))
        # ^ build-up defined as t_ss after start.
        i_ss = np.where((t_obs_data > t_ss) & (t_obs_data <= t_event_data))
        i_declining = np.where(t_obs_data > t_event_data)

        scaled_times = np.concatenate(
            (
                t_obs_data[i_buildup] / t_element,
                scaled_ss_times[i_ss] / t_element,
                scaled_declining_times[i_declining] / t_element,
            ),
            axis=None,
        )
        # ^ This picks out the correct phase for each sample and puts them all together
        # in one array

        try:
            max_bin = max(
                np.amax(scaled_declining_times[i_declining] / t_element),
                (2 + steady_state_scaling_factor) * t_ss_scaling_factor,
            )
        except ValueError:
            # Probably means there were no declining phase samples
            max_bin = (2 + steady_state_scaling_factor) * t_ss_scaling_factor
        bins, x_bar_centres = self.generate_bins_and_bar_centres(
            0, max_bin, half_bin_size
        )
        heights, bins2 = np.histogram(scaled_times, bins, density=True)
        ...
        text_dict = {
            ...
            "ss_text": {
                "x_pos": t_ss_scaling_factor
                + steady_state_scaling_factor * t_ss_scaling_factor * 0.5,
                "text_string": str(
        ...
        line_dict = {
            "vline1": {"x_start": t_ss_scaling_factor},
            "vline2": {
        ...
        x_axis_dict = {
            "supress_second_axis": True,
            "x_tick_locations_override": [
                0,
                t_ss_scaling_factor,
                t_ss_scaling_factor * (1 + steady_state_scaling_factor),
                t_ss_scaling_factor * (2 + steady_state_scaling_factor),
            ],
    ...
    def make_p_v_fO2_plot(
        self, chains_dir, model, model_name, wd_name_tuple, observation_number
    ):
        x_data = self.get_untransformed_parameter_stats(
            chains_dir, model, observation_number, mp.ModelParameter.pressure
        )
        y_data = self.get_untransformed_parameter_stats(
            chains_dir, model, observation_number, mp.ModelParameter.oxygen_fugacity
        )
        if x_data is None or y_data is None:
            print(
                "Abandoning pressure v fO2 plot (no pressure and/or fO2 stats available)"
            )
            return
        self.graph_fac.plot_pressure_v_oxygen_fugacity(
            x_data, y_data, wd_name_tuple[0], wd_name_tuple[1], model.get_prefix()
        )
