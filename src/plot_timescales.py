...
def get_logg_values(grid_steps=100, grid_range=(None, None), timescale_types=list()):
    # Bedard goes from 7.5 to 9, Koester from 7.5 to 8.5
    # grid_range = min logg, max logg
    min_logg = grid_range[0]
    max_logg = grid_range[1]
    if min_logg is None:
        min_logg = max(
            [logg_ranges[timescale_type][0] for timescale_type in timescale_types]
        )
    if max_logg is None:
        max_logg = min(
            [logg_ranges[timescale_type][1] for timescale_type in timescale_types]
        )
    print(min_logg)
    print(max_logg)
...
def get_Teff_values(grid_steps=100, grid_range=(None, None), timescale_types=list()):
    # Bedard goes from 11000 to 20000 (actually it varies!
    # 5000 to 30000 by default, or 11000 to 20000 for 3O), Koester from 5000 to 20000
    min_teff = grid_range[0]
    max_teff = grid_range[1]
    if min_teff is None:
        min_teff = max(
            [teff_ranges[timescale_type][0] for timescale_type in timescale_types]
        )
    if max_teff is None:
        max_teff = min(
            [teff_ranges[timescale_type][1] for timescale_type in timescale_types]
        )
    print(min_teff)
    print(max_teff)
...
def get_elements_for_model_comparison():
    # needs to be a subset of get_elements_for_2D_plot().
...
def generate_timescales():
    ...
    for element in elements_we_care_about:
        ...
        timescale_vals["He"][element] = np.zeros(
            (len(logg_vals), len(Teff_vals), len(CaHe_vals))
        )
    for i in range(len(logg_vals)):
        logg = logg_vals[i]
        for j in range(len(Teff_vals)):
            Teff = Teff_vals[j]
            H_timescales = timescale_interpolator.get_wd_timescales(
                "H", logg, Teff, None
            )
            for element in elements_we_care_about:
            ...
            for k in range(len(CaHe_vals)):
                CaHe = CaHe_vals[k]
                He_timescales = timescale_interpolator.get_wd_timescales(
                    "He", logg, Teff, CaHe
                )
                for element in elements_we_care_about:
...
def extract_2D_vals_to_plot(
    timescale_vals, HorHe, logg, Teff, CaHe, elements_to_extract
):
...
def plot_2D_timescale(timescale_vals, HorHe, logg, Teff, CaHe):
    to_plot, variable = extract_2D_vals_to_plot(
        timescale_vals, HorHe, logg, Teff, CaHe, get_elements_for_2D_plot()
    )
    graph_factory = gf.GraphFactory()
    graph_factory.plot_2D_timescales(
        to_plot, variable, get_variable_vals(variable), HorHe, logg, Teff, CaHe
    )


def plot_model_comparison(timescale_vals, HorHe, logg, Teff, CaHe):
    to_plot_new, variable = extract_2D_vals_to_plot(
        timescale_vals, HorHe, logg, Teff, CaHe, get_elements_for_model_comparison()
    )
    if variable == "logg":
        # These are at (or near) Teff = 6250 according to Simon
        # (but we take the timescales themselves from Hollands)
        # (also excluding those where logg was just assumed to be 8 by default)
        # System      logg     Ca       Fe
        ...
        # These are at (or near) logg = 8 according to Simon
        # (but we take the timescales themselves from Hollands)
        # (also excluding those where logg was just assumed to be 8 by default)
        # System   Teff     Ca       Fe
    ...
    if HorHe != "He":
        ...
        to_plot_old = None
    graph_factory = gf.GraphFactory()
    graph_factory.plot_timescale_model_comparison(
        to_plot_new,
        to_plot_old,
        variable,
        get_variable_vals(variable),
        x_vals_old,
        HorHe,
        logg,
        Teff,
        CaHe,
        get_elements_for_model_comparison(),
    )
...
def extract_timescale_ratios(element1, element2, Hx, logg_vals, Teff_vals):
    ...
    timescale_vals = {
        tt: {
            element1: np.zeros((len(logg_vals), len(Teff_vals))),
            element2: np.zeros((len(logg_vals), len(Teff_vals))),
            "SS": np.zeros((len(logg_vals), len(Teff_vals))),
            "Dec": np.zeros((len(logg_vals), len(Teff_vals))),
        }
        for tt in ti.TimescaleType
    }
    for i in range(len(logg_vals)):
        logg = logg_vals[i]
        for j in range(len(Teff_vals)):
            ...
            for tt in timescales:
                if timescales[tt] is not None:
                    ...
                    # Let's say accretion lasts for 10 sinking timescales of elements
                    # 1 and 2, so that the system has reached steady state
                    t_e = 10 * max(t1, t2)
                    # ..and let's say you observe it a few sinking timescales after that
                    t_obs = 15 * max(t1, t2)
                    timescale_vals[tt]["Dec"][i, j] = (
                        t1
                        / t2
                        * np.exp(t_obs / t2 - t_obs / t1)
                        * ((np.exp(t_e / t1) - 1) / (np.exp(t_e / t2) - 1))
                    )
    return timescale_vals


def plot_timescale_ratios(
    element1,
    element2,
    ...
    correction_types=["SS"],
    grid_steps=100,
    grid_range=((None, None), (None, None)),
):
    # grid_range is a 2-tuple of 2-tuples:
    # ( (min Teff, max Teff) , (min logg, max logg) )
    ...
    if include_wd_markers:
        if Hx == ci.Element.H:
            ...
            reference_systems = {
                ...
                for wd in pss.pick_out_sample(name_of_sample)
            }
            full_sample = {
                wd.full_name(): (wd.get_teff().value, wd.get_logg().value)
                for wd in pss.pick_out_all_das()
            }
            ...
            full_sample = {
                wd.full_name(): (wd.get_teff().value, wd.get_logg().value)
                for wd in pss.pick_out_all_dbs()
            }
        else:
            print(Hx)
            raise
    ...
    print(
        [
            ti.teff_ranges[Hx][timescale_type][0]
            for timescale_type in all_timescale_types
        ]
    )
    print(
        min(
    ...
    min_teff = (
        ...
        if grid_range[0][0] is None
        else grid_range[0][0]
    )
    max_teff = (
        max(
            [
                ti.teff_ranges[Hx][timescale_type][1]
                for timescale_type in all_timescale_types
            ]
        )
        if grid_range[0][1] is None
        else grid_range[0][1]
    )
    min_logg = (
        min(
            [
                ti.logg_ranges[Hx][timescale_type][0]
                for timescale_type in all_timescale_types
            ]
        )
        if grid_range[1][0] is None
        else grid_range[1][0]
    )
    max_logg = (
        max(
            [
                ti.logg_ranges[Hx][timescale_type][1]
                for timescale_type in all_timescale_types
            ]
        )
        if grid_range[1][1] is None
        else grid_range[1][1]
    )
    ...
    data_pool = extract_timescale_ratios(
        element1, element2, Hx, full_logg_range, full_teff_range
    )
    overshoot_test_timescale_types = [
    ...
    for correction_type in correction_types:
        ...
        for timescale_type1, timescale_type2 in timescale_type_pairs:
            ...
            #     grid_steps,
            #     grid_range[0],
            #     [timescale_type_1, timescale_type2]
            # )
            # logg_vals = get_logg_values(
            #     grid_steps,
            #     grid_range[1],
            ...
            grid_range_to_plot = (
                (
                    # Find the teff/logg range that is within BOTH of the interpolation
                    # limits (ie for both timescale types)
                    (
                        max(
                            [
                                ti.teff_ranges[Hx][timescale_type][0]
                                for timescale_type in [timescale_type1, timescale_type2]
                            ]
                        )
                        if grid_range[0][0] is None
                        else grid_range[0][0]
            ...
            data_to_plot = np.log10(
                data_pool[timescale_type1][correction_type]
                / data_pool[timescale_type2][correction_type]
            )
            plot_dict = graph_fac.plot_timescale_type_ratios(
                element1,
                element2,
                timescale_type1,
                timescale_type2,
            ...
            if (
                timescale_type1 in overshoot_test_timescale_types
                and timescale_type2 in overshoot_test_timescale_types
            ):
...
def plot_timescale_comparison_benchmarked(
    element1,
    element2,
    timescale_type_pairs,
    timescale_benchmark,
    ...
    for correction_type in correction_types:
        ...
        for timescale_type1, timescale_type2 in timescale_type_pairs:
            ...
            #     diff_t1b < diff_t2b, diff_t2b-diff_t1b, diff_t2b-diff_t1b
            # ) # Introduce sign convention that t2 is negative, t1 is positive
            # ^ I realised this just simplifies to this:
            data_to_plot = diff_t2b - diff_t1b
            plot_dict = graph_fac.plot_timescale_type_benchmark(
                element1,
                element2,
                timescale_type1,
                timescale_type2,
...
def plot_therm_factors():
    ...
    reference_systems = {
        wd.full_name(): (wd.get_teff().value, wd.get_logg().value)
        for wd in pv.pick_out_thermohaline_das()
    }
    full_sample = {
        wd.full_name(): (wd.get_teff().value, wd.get_logg().value)
        for wd in pv.pick_out_all_das()
    }
    graph_fac = gf.GraphFactory()
    for logMdot in logMdots:
        therm_factors = extract_therm_factors(
            therm_factor_interpolator, logg_vals, Teff_vals, logMdot
        )
        # reference_systems = {
        #    # 'WD2058+181': (17308, 7.92),
        ...
        #    # 'G29-38': (11800, 8.4)
        # }
        plot_dict = graph_fac.plot_therm_factors(
            Teff_vals, logg_vals, logMdot, therm_factors, reference_systems, full_sample
        )
...
def main():
    ...
    plot_timescale_comparison_benchmarked(
        ci.Element.Ca,
        ci.Element.Fe,
        [(ti.TimescaleType.BedardNoOvershoot, ti.TimescaleType.BedardOvershoot)],
    ...
    if plot_timescale_pairs:
        ...
        plot_timescale_ratios(
            ci.Element.Ca, ci.Element.Mg, timescale_type_pairs, ci.Element.H
        )
        # plot_timescale_ratios(
        #     ci.Element.Fe,
        #     ci.Element.Mg,
        ...
        plot_timescale_ratios(
            ci.Element.Ca, ci.Element.Mg, timescale_type_pairs, ci.Element.He, ["SS"]
        )
        plot_timescale_ratios(
            ci.Element.Fe, ci.Element.Mg, timescale_type_pairs, ci.Element.He, ["SS"]
        )
        plot_timescale_ratios(
            ci.Element.Si, ci.Element.Mg, timescale_type_pairs, ci.Element.He, ["SS"]
        )
        plot_timescale_ratios(
            ci.Element.O, ci.Element.Mg, timescale_type_pairs, ci.Element.He, ["SS"]
        )
        plot_timescale_ratios(
            ci.Element.Fe, ci.Element.Ca, timescale_type_pairs, ci.Element.He, ["SS"]
        )
        plot_timescale_ratios(
            ci.Element.Ca, ci.Element.Fe, timescale_type_pairs, ci.Element.He, ["SS"]
        )
        plot_timescale_ratios(
            ci.Element.Ca, ci.Element.Fe, timescale_type_pairs, ci.Element.H
        )
        plot_timescale_ratios(
            ci.Element.Ca,
            ci.Element.Fe,
        ...
        plot_timescale_ratios(
            ci.Element.Mg, ci.Element.Fe, timescale_type_pairs, ci.Element.H
        )
        plot_timescale_ratios(
            ci.Element.Fe, ci.Element.Mg, timescale_type_pairs, ci.Element.H
        )
        plot_timescale_ratios(
            ci.Element.Mg, ci.Element.Fe, timescale_type_pairs, ci.Element.He, ["SS"]
        )
        plot_timescale_ratios(
            ci.Element.Ca, ci.Element.Mg, timescale_type_pairs, ci.Element.H
        )
        ...
        plot_timescale_ratios(
            ci.Element.Si, ci.Element.Fe, timescale_type_pairs, ci.Element.H
        )
        ...
        #    # plot_model_comparison(timescale_vals, 'H', None, 6250, -9.5)
        #    # ^ At the moment, we're limited to Teff == 6250 only
        #    #  (the only val. I got old model values for)
        #    # plot_model_comparison(timescale_vals, 'He', None, 6250, -9.5)
        #    # ^ At the moment, we're limited to Teff == 6250 only
        #    #  (the only val. I got old model values for)
        ...
        #    # plot_model_comparison(timescale_vals, 'H', 8, None, -9.5)
        #    # At the moment, we're limited to logg == 8 only
