...
def load_marc_distributions():
    ...
    for file_name in file_names:
        ...
        CNF = [
            geology_model.convert_core_mass_fraction_to_core_number_fraction(cmf)
            for cmf in CMF
        ]
...
def load_tidal_distributions():
    geology_model = gi.GeologyModel()
    file_names = ["CMF_tidal_disruption"]
    # ^ This is a list of core mass fractions from Marc Brouwers - need to be converted
    # to number fractions and binned
    # WARNING: The caching (synthetic_population.py, cache_inverse_cdf_from_table) seems
    # to have a strange bug where it's possible to sample above the maximum input value
    # if you supply it with bins that go above that value (even if there's nothing in
    # those bins).
    # Workaround: make sure that your bins extend as far as the values and no further!
    # Hence the precisely chosen upper limit of 0.321 here.
...
def load_amy_distributions():
    ...
    # Ignore everything heavier than 0.1 Earth mass (see Bonsor+ 2020)
    mass_cutoff = 0.1
...
def load_mwdd_distributions():
    ...
    dadb_logg_bins = {"DA": np.linspace(7, 9.4, 16), "DB": np.linspace(7.85, 9, 6)}
    ...
    for stellar_suffix in ["DA", "DB"]:
        ...
        with open(
            ...
            for row in csv.reader(config_csv, delimiter=","):
                ...
                if logg is not None:
                    loggs.append(logg)
        teff_counts, teff_edges = np.histogram(
            teffs, bins=dadb_teff_bins[stellar_suffix]
        )
        teff_tuples = [
            (teff_edges[i], teff_edges[i + 1]) for i in range(0, len(teff_edges) - 1)
        ]
        predefined_distributions[teff_dist_name] = (teff_tuples, teff_counts)
        logg_counts, logg_edges = np.histogram(
            loggs, bins=dadb_logg_bins[stellar_suffix]
        )
        logg_tuples = [
            (logg_edges[i], logg_edges[i + 1]) for i in range(0, len(logg_edges) - 1)
        ]
    ...
    with open(f"../data/{gspcwd_filename}", encoding="utf-8") as config_csv:
        ...
        for row in csv.reader(config_csv, delimiter=","):
            ...
            if min_teff <= teff <= max_teff and min_logg <= logg <= max_logg:
                if spt in ["DA", "DA:"]:
                    # Luckily for us, the DAs and DBs fall neatly into H and He
                    # dominated here
...
def load_hollands_distributions():
    ...
    logg_bins = np.linspace(7.1, 8.7, 9)
    # ^ Important to have a bin centred on 8: a lot of log(g)s are equal to 8 exactly
...
def collisional_cascade_distribution(mass):
    ...
    # The sanity check here is that for a uniform distribution we would have alpha = 0
    # And indeed, setting alpha = 0 gives a uniform distribution
    return mass ** (-alpha)
...
def log_collisional_cascade_distribution(log_mass):
    ...
    # (Don't need to normalise here, it will be normalised later) - so technically we
    # might not actually need the base conversion prefactor
    # The sanity check here is that if you set alpha to zero, sample from this, and take
    # 10**(answer), that should be uniformly distributed, and indeed it is
    mass = base**log_mass
...
def load_distributions():
    ...
    # TODO: This should be called during main execution so that the file name can be set
    # dynamically from the configuration file
...
def load_functions():
    ...
    predefined_functions["LogCollisionalCascadeWyatt"] = (
        log_collisional_cascade_distribution_wyatt
    )
...
wd_configurations = {
    ...
    "RealisticDAs": {
        ...
        mp.WDParameter.temperature: (
            ...
            [
                "MWDD_DA_Teffs_40pc",
                predefined_distributions["MWDD_DA_Teffs_40pc"][0],
                predefined_distributions["MWDD_DA_Teffs_40pc"][1],
            ],
        ),
        mp.WDParameter.logg: (
            Distribution.CustomDistribution,
            [
                "MWDD_DA_Loggs_40pc",
                predefined_distributions["MWDD_DA_Loggs_40pc"][0],
        ...
        # an input, which depends on M_WD via logq
    ...
    "RealisticDBs": {
        ...
        mp.WDParameter.temperature: (
            ...
            [
                "MWDD_DB_Teffs_40pc",
                predefined_distributions["MWDD_DB_Teffs_40pc"][0],
                predefined_distributions["MWDD_DB_Teffs_40pc"][1],
            ],
        ),
        mp.WDParameter.logg: (
            Distribution.CustomDistribution,
            [
                "MWDD_DB_Loggs_40pc",
                predefined_distributions["MWDD_DB_Loggs_40pc"][0],
                predefined_distributions["MWDD_DB_Loggs_40pc"][1],
            ],
        ),
        mp.WDParameter.mass: (Distribution.Normal, [0.6, 0.02]),
        # ^ This doesn't actually matter
    ...
    "HollandsDBs": {
        ...
        mp.WDParameter.temperature: (
            ...
            [
                "HollandsTeffs",
                predefined_distributions["HollandsTeffs"][0],
                predefined_distributions["HollandsTeffs"][1],
            ],
        ),
        mp.WDParameter.logg: (
            Distribution.CustomDistribution,
            [
                "HollandsLoggs",
                predefined_distributions["HollandsLoggs"][0],
                predefined_distributions["HollandsLoggs"][1],
            ],
        ),
        mp.WDParameter.mass: (Distribution.Normal, [0.6, 0.02]),
        # ^ This doesn't actually matter
    ...
    #         [
    #             "NS20_Teffs_DA",
    #             predefined_distributions["NS20_Teffs_DA"][0],
    #             predefined_distributions["NS20_Teffs_DA"][1],
    #         ],
    #     ),
    #     mp.WDParameter.logg: (
    #         Distribution.CustomDistribution,
    #         [
    #             "NS20_Loggs_DA",
    #             predefined_distributions["NS20_Loggs_DA"][0],
    #             predefined_distributions["NS20_Loggs_DA"][1],
    #         ],
    #     ),
    #     mp.WDParameter.mass: (
    #         Distribution.Normal,
    #         [0.6, 0.02],
    #     ),  # This doesn't actually matter UPDATE/WARNING: It actually does now! The new atmosphere model takes M_cvz as an input, which depends on M_WD via logq
    #     mp.WDParameter.timescale_type: ti.TimescaleType.KoesterOvershoot,
    #     mp.WDParameter.consider_thermohaline: False,
    #     mp.WDParameter.distance: (
    #         Distribution.Delta,
    #         [40],
    #     ),  # This doesn't actually matter
    # },
    ...
    #         [
    #             "NS20_Teffs_DB",
    #             predefined_distributions["NS20_Teffs_DB"][0],
    #             predefined_distributions["NS20_Teffs_DB"][1],
    #         ],
    #     ),
    #     mp.WDParameter.logg: (
    #         Distribution.CustomDistribution,
    #         [
    #             "NS20_Loggs_DB",
    #             predefined_distributions["NS20_Loggs_DB"][0],
    #             predefined_distributions["NS20_Loggs_DB"][1],
    #         ],
    #     ),
    #     mp.WDParameter.mass: (Distribution.Normal, [0.6, 0.02]),
    #     # ^ This doesn't actually matter
...
pollution_configurations = {
    "TestPollutionConfig": {
        ...
        # ^ kg, 10^20kg corresponds to an asteroid ish type mass
        # (Vesta-esque, so a large asteroid)
    ...
    "DummyPollutionConfig": {
        ...
        #     Distribution.Uniform, [999999, 1000001]
        # ),
    ...
    "Amy035f6nogas": {
        ...
        mp.ModelParameter.fragment_core_frac: (
            ...
            [
                "m_cf_035f6nogas",
                predefined_distributions["m_cf_035f6nogas"][0],
                predefined_distributions["m_cf_035f6nogas"][1],
            ],
        ),
        # mp.ModelParameter.fragment_core_frac: (Distribution.Delta, [0.5]),
        mp.ModelParameter.fragment_crust_frac: (Distribution.Delta, [0]),
    ...
    "DATidal": {
        ...
        mp.ModelParameter.fragment_core_frac: (
            Distribution.CustomDistribution,
            [
                "CMF_tidal_disruption",
                predefined_distributions["CMF_tidal_disruption"][0],
                predefined_distributions["CMF_tidal_disruption"][1],
            ],
        ),
        mp.ModelParameter.fragment_crust_frac: (Distribution.Delta, [0]),
        mp.ModelParameter.fragment_mass: (Distribution.Uniform, [10, 25]),
    ...
    "Wyatt2014": {
        ...
        mp.ModelParameter.fragment_mass: (
            Distribution.CustomFunction,
            [
                "LogCollisionalCascadeWyatt",
                predefined_functions["LogCollisionalCascadeWyatt"],
                10,
                21.50515,
            ],
    ...
    "TestCollCasc": {
        ...
        mp.ModelParameter.fragment_mass: (
            ...
            [
                ...
                predefined_functions["LogCollisionalCascade"],
                15.7,
                np.log10(pc.M_Earth),
            ],
...
def plot_predefined_distributions(
    reference_dist_for_bins,
    dist_names,
    dist_label_names,
    file_prefix,
    file_suffix,
    ...
    xbar = [
        (edges[0] + edges[1]) / 2
        for edges in predefined_distributions[reference_dist_for_bins][0]
    ]
    all_heights = list()
