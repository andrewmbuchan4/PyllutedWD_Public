...
def collect_system_stats(path=None):
    ...
    volatile_poor_temp = 1400  # Above this temp, we're volatile poor
    # (including moderate volatiles) (loosely basing this on Lodders 2003 table 8)
    for stats_file in find_stats_files(path):
            ...
            for row in csv.reader(stats_csv):
                if len(row) > 1:
                    if row[0] == "Results from model:":
                        # This logic is new and super important! The code now writes
                        # results from multiple models in the same csv underneath each
                        # other,
                        # So we need to keep track of which model results we are
                        # currently reading, we only care about one of them (for now)
                        ...
                        if best_model_index is not None:
                            try:
                                if row[best_model_index] == "True":
                                    # This should happen precisely once
                                    good_fit = row[good_fit_index]
                                    best_model = row[0]
                            except IndexError:
                                # The row was one of the short ones which we can ignore
                                # (we're looking for the row which has a True entry in
                                # the Best model? column)
                                pass
                        if row[0].startswith("Diff Sigma"):
                            diff_sigma = row[1]
                        if row[0] == "Mg":
                            # Then this must be the Mg sinking timescale - need this for
                            # later!
                            t_Mg = float(row[1])
        ...
        volatile_depleted = (
            temperature >= volatile_rich_temp and temperature <= volatile_poor_temp
        )
...
def sort_latex_rows(list_of_rows):
    toret = list_of_rows
    toret.sort(key=lambda x: (x[0]))
    # ^ This is stable (i.e. retains original order in case of ties) but could be made
    # more complicated?
    return toret
            ...
            to_write.writerow(
                [
                    ...
                    (
                        ...
                        if vals["Medians"]["Stellar metallicity indices"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["Stellar metallicity indices"]
                        if vals["LowerErrors"]["Stellar metallicity indices"]
                        is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["Stellar metallicity indices"]
                        if vals["UpperErrors"]["Stellar metallicity indices"]
                        is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["Time since Accretion/Myrs"]
                        if vals["Medians"]["Time since Accretion/Myrs"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["Time since Accretion/Myrs"]
                        if vals["LowerErrors"]["Time since Accretion/Myrs"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["Time since Accretion/Myrs"]
                        if vals["UpperErrors"]["Time since Accretion/Myrs"] is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["log(Formation Distance/AU)"]
                        if vals["Medians"]["log(Formation Distance/AU)"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["log(Formation Distance/AU)"]
                        if vals["LowerErrors"]["log(Formation Distance/AU)"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["log(Formation Distance/AU)"]
                        if vals["UpperErrors"]["log(Formation Distance/AU)"] is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["Feeding Zone Size/AU"]
                        if vals["Medians"]["Feeding Zone Size/AU"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["Feeding Zone Size/AU"]
                        if vals["LowerErrors"]["Feeding Zone Size/AU"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["Feeding Zone Size/AU"]
                        if vals["UpperErrors"]["Feeding Zone Size/AU"] is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["Parent Core Fraction"]
                        if vals["Medians"]["Parent Core Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["Parent Core Fraction"]
                        if vals["LowerErrors"]["Parent Core Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["Parent Core Fraction"]
                        if vals["UpperErrors"]["Parent Core Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["Parent Crust Fraction"]
                        if vals["Medians"]["Parent Crust Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["Parent Crust Fraction"]
                        if vals["LowerErrors"]["Parent Crust Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["Parent Crust Fraction"]
                        if vals["UpperErrors"]["Parent Crust Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["Fragment Core Fraction"]
                        if vals["Medians"]["Fragment Core Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["Fragment Core Fraction"]
                        if vals["LowerErrors"]["Fragment Core Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["Fragment Core Fraction"]
                        if vals["UpperErrors"]["Fragment Core Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["Fragment Crust Fraction"]
                        if vals["Medians"]["Fragment Crust Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["Fragment Crust Fraction"]
                        if vals["LowerErrors"]["Fragment Crust Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["Fragment Crust Fraction"]
                        if vals["UpperErrors"]["Fragment Crust Fraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["log(Pollution Fraction)"]
                        if vals["Medians"]["log(Pollution Fraction)"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["log(Pollution Fraction)"]
                        if vals["LowerErrors"]["log(Pollution Fraction)"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["log(Pollution Fraction)"]
                        if vals["UpperErrors"]["log(Pollution Fraction)"] is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["log(Accretion Event Timescale/Yrs)"]
                        if vals["Medians"]["log(Accretion Event Timescale/Yrs)"]
                        is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["log(Accretion Event Timescale/Yrs)"]
                        if vals["LowerErrors"]["log(Accretion Event Timescale/Yrs)"]
                        is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["log(Accretion Event Timescale/Yrs)"]
                        if vals["UpperErrors"]["log(Accretion Event Timescale/Yrs)"]
                        is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["Pressure /GPa"]
                        if vals["Medians"]["Pressure /GPa"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["Pressure /GPa"]
                        if vals["LowerErrors"]["Pressure /GPa"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["Pressure /GPa"]
                        if vals["UpperErrors"]["Pressure /GPa"] is not None
                        else "N/A"
                    ),
                    (
                        vals["Medians"]["Oxygen Fugacity /ΔIW"]
                        if vals["Medians"]["Oxygen Fugacity /ΔIW"] is not None
                        else "N/A"
                    ),
                    (
                        vals["LowerErrors"]["Oxygen Fugacity /ΔIW"]
                        if vals["LowerErrors"]["Oxygen Fugacity /ΔIW"] is not None
                        else "N/A"
                    ),
                    (
                        vals["UpperErrors"]["Oxygen Fugacity /ΔIW"]
                        if vals["UpperErrors"]["Oxygen Fugacity /ΔIW"] is not None
                        else "N/A"
                    ),
                    vals["GoodFit"],
                    vals["Primitive"],
                    ...
                    (
                        vals["ParentCoreNumberFraction"]
                        if vals["ParentCoreNumberFraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["FragmentCoreMassFraction"]
                        if vals["FragmentCoreMassFraction"] is not None
                        else "N/A"
                    ),
                    (
                        vals["FragmentCoreMassFractionLowerError"]
                        if vals["FragmentCoreMassFractionLowerError"] is not None
                        else "N/A"
                    ),
                    (
                        vals["FragmentCoreMassFractionUpperError"]
                        if vals["FragmentCoreMassFractionUpperError"] is not None
                        else "N/A"
                    ),
        ...
        for sys, vals in system_stats_dict.items():
            ...
            if sys in systems_in_sample_including_dupes:
                list_of_vals = [
                    ...
                    (
                        str(int(round(vals["Temperature"])))
                        if vals["Temperature"] != 0
                        else "N/A"
                    ),
                    (
                        str(round(vals["ParentCoreNumberFraction"], 2))
                        if vals["ParentCoreNumberFraction"] is not None
                        else "N/A"
                    ),
                    (
                        str(round(vals["DifferentiationSigma"], 1))
                        if vals["DifferentiationSigma"] is not None
                        and vals["DifferentiationSigma"] != "N/A"
                        else "N/A"
                    ),
...
def compile_stats_summary(system_stats_dict, path=None):
    ...
    classifications = {
        "VolatileRich": {"Primitive": 0, "CoreRich": 0, "MantleRich": 0},
        "VolatileDepleted": {"Primitive": 0, "CoreRich": 0, "MantleRich": 0},
        "ModerateVolatileDepleted": {"Primitive": 0, "CoreRich": 0, "MantleRich": 0},
    }
    for sys, vals in system_stats_dict.items():
        if sys in whitelist:
            if not vals["GoodFit"]:
    ...
    with open(path_to_use, "w", newline="", encoding="utf-8") as f:
        to_write = csv.writer(f)
        to_write.writerow(["Unclassified:", unclassified])
        to_write.writerow([])
        to_write.writerow(["", "Primitive", "Core Rich", "Mantle Rich"])
...
def read_old_output_files():
    # This was partially done at the time of abandonment: The only major logic needed
    # was a way to find the model number (17 in this example) from the model params
    # stored in model_params_dict.
    # NB: The model names in the code are named differently from how they are named in
    # the output file, for some reason.
    # Also, just for further confusion, these names are also different from the names
    # ultimately given in John's paper (there are 3 sets of names)
    import xlrd
    ...
    model_params_dict = {
        ...
        "M3 = (M1 + Formation Distance + Feeding Zone)": [
            "Stellar metallicity indices",
            "Time since Accretion/Myrs",
            "log(Formation Distance/AU)",
            "Feeding Zone Size/AU",
        ...
        "M5 = (M1 + Fragment Core Fraction + Fragment Crust Fraction)": [
            "Stellar metallicity indices",
            "Time since Accretion/Myrs",
            "Fragment Core Fraction",
            "Fragment Crust Fraction",
            "log(Pollution Fraction)",
            "log(Accretion Event Timescale/Yrs)",
        ],
        "M6 = (M2 + Fragment Core Fraction)": [
            "Stellar metallicity indices",
            "Time since Accretion/Myrs",
            "log(Formation Distance/AU)",
            "Fragment Core Fraction",
        ...
        "M7 = (M2 + Fragment Core Fraction + Fragment Crust Fraction)": [
            "Stellar metallicity indices",
            "Time since Accretion/Myrs",
            "log(Formation Distance/AU)",
            "Fragment Core Fraction",
        ...
        "M9 = (M3 + Fragment Core Fraction + Fragment Crust Fraction)": [
            "Stellar metallicity indices",
            "Time since Accretion/Myrs",
            "log(Formation Distance/AU)",
            "Feeding Zone Size/AU",
        ...
        "M10 = (M7 + Parent Core Fraction)": [
            "Stellar metallicity indices",
            "Time since Accretion/Myrs",
            "log(Formation Distance/AU)",
            "Parent Core Fraction",
            "Fragment Core Fraction",
            "Fragment Crust Fraction",
            "log(Pollution Fraction)",
            "log(Accretion Event Timescale/Yrs)",
        ],
        "M11 = (M7 + Parent Crust Fraction)": [
            "Stellar metallicity indices",
            "Time since Accretion/Myrs",
            "log(Formation Distance/AU)",
            "Parent Crust Fraction",
            "Fragment Core Fraction",
            "Fragment Crust Fraction",
            "log(Pollution Fraction)",
            "log(Accretion Event Timescale/Yrs)",
        ],
        "M12 = (M7 + Parent Core Fraction + Parent Crust Fraction)": [
            "Stellar metallicity indices",
            "Time since Accretion/Myrs",
            "log(Formation Distance/AU)",
            "Parent Core Fraction",
        ...
        "M15 = (M9 + Parent Core Fraction + Parent Crust Fraction)": [
            "Stellar metallicity indices",
            "Time since Accretion/Myrs",
            "log(Formation Distance/AU)",
            "Feeding Zone Size/AU",
    ...
    # For outputs from John's code, need to load up the pymultinest outputs and read
    # them to generate things like bu/ss/dec percentages
    ...
    for obs_number in [185]:
        ...
        a = pn.Analyzer(
            n_params=number_of_params, outputfiles_basename=path + file_prefix
        )
        stats = a.get_stats()
        print(stats)
        print(a.get_equal_weighted_posterior())
        weightpost = a.get_equal_weighted_posterior()[:, 0:number_of_params]
