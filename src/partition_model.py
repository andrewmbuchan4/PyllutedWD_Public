...
# General:
# In calculate_abundances in geology_info, comment / uncomment the relevant
#   normalisation section
# Remove Si/O from reading fischer es if not using fischer Si/O
# Change ci.Element.Placeholder to ci.Element.O in this file (and calculate_abundances
#   in geology_info)
# Change geology model config name
# Change o_cap in geology_info

# Li et al 2023 has a parameterisation for the ratio of D_C to D_N - could be useful for
#   future (experiments up 3GPa, fO2 up to IW)
# Both are siderophilic - also see the references therein, they include higher pressure
#   experiments
# Huang+ 2020 and Huang+ 2021 have parametrisations for Mo, W, Nb and Ta (helpfully
#   compiled in Gu+ 2023, albeit without interactions)
...
class PartitionModel:
    ...
    def __init__(self, file_params, file_interactions, file_composition, file_es=None):
        ...
        # fixed activity coeff of Fe in silicate (Rudge 2010, just above equation G.3)
        ...
        self.partitioners = [
            ...
            ci.Element.Cr,
            ci.Element.Cu,
            # ci.Element.Ti,   # Removing Ti
            # ^ it's extremely erratic and ought to be essentially non-partitioning
            # anyway.
            # NB This should be revisited if Sulfur included, as the Ti/S interaction is
            # strong
            ci.Element.Fe,
            ci.Element.W,
            ci.Element.P,
            ...
            for row in read:
                ...
                if i > 0:
                    for j in range(1, len(row) - 1):
                        # This logic is a fancy way of setting the class member variable
                        # names to match the column headings, but a bit pointless given
                        # that in mkd we assume they're called self.a etc anyway...
                        getattr(self, self.params[j - 1])[ci.Element[row[0]]] = float(
                            row[j]
                        )
                    getattr(self, self.params[len(row) - 2])[ci.Element[row[0]]] = str(
                        row[len(row) - 1]
                    )
                i += 1
        ...
        with open(file_interactions, encoding="utf-8") as csvfile:
            ...
            for row in read:
                if i == 0:
                    ...
                    # It might be more readable to make self.eps a dict with
                    # (Element, Element) keys, but this is slow
    ...
    def load_fischer_es(self, file_es=None):
        ...
        if file_es is None:
            return
        # Ignore es for these elements! We will stick with the default interactions
        forbidden_e_els = [ci.Element.Si, ci.Element.O]
            ...
            for row in read:
                ...
                if i > 0:
                    for j in range(1, len(row)):
                        ...
                        el_j = eles[j - 1]
                        if (el_i not in forbidden_e_els) and (
                            el_j not in forbidden_e_els
                        ):
    ...
    def calculate_gammas(self, T=None, composition=None):
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            ...
            two = -eps * xk * xj * (
                1 + log_1_minus_xj_over_xj + log_1_minus_xk_over_xk
            ...
            three = eps * xj * xk * (
                1 + log_1_minus_xk_over_xk - 1 / (1 - xj)
            ) - eps_xj2_xk2 * (
                one_over_1_minus_xj
                + one_over_1_minus_xk
                + xj / (2.0 * (1 - xj) ** 2)
            ...
            one = -eps * xk * (
                ...
                one_over_1_minus_xj + one_over_1_minus_xk + xj / (2 * (1 - xj) ** 2) - 1
            )
            ...
            logg = (
                loggFe
                + ((self.T0 / T) * self.logg0)
                - np.transpose(np.diag(eps)) * np.log(1 - x)
                + np.transpose(one)
            )
            gammas = dict(zip(self.logg0name, np.exp(logg)))
            gammas[ci.Element.Fe] = np.exp(loggFe)
            # gammas[ci.Element.O] *= np.exp(4.29 - (16500/T))
            # ^ This is a hack to account for Badro's gamma parametrisation for O
            return gammas
    ...
    def construct_eps_table(self, T, alloy_composition_without_Fe):
        ...
        damping = False
        if damping:
            si_index = self.alloy_els.index(ci.Element.Si) - 1
            # ^ minus 1 because alloy_composition_without_Fe is the alloy without the
            # first element (i.e. Fe)
    ...
    def calculate_D(
        ...
        nbot,
        log10_gammaFe_sil,
        core_composition=None,
        mantle_composition=None,
        params=None,
    ):
        ...
        if core_composition is None:
            ...
            alloy_composition = self.convert_core_composition_to_alloy_composition(
                core_composition
            )

        gammas = self.calculate_gammas(T, alloy_composition)
        els_to_iterate_over = [
            el for el in self.partitioners if el in self.logg0name + [ci.Element.Fe]
        ]  # Filter out elements that we don't have partitioning info for
        d_dict = dict()
        for e in els_to_iterate_over:
            p_v = params[4] if params is not None else self.v
            if e != ci.Element.Fe:
                logkd_app, sigma_logkd_app_2 = self.calculate_logKD_app(
                    P, T, nbot, e, params
                )  # When iterating over composition, we don't need to recalculate this
                loggamma = np.log10(gammas[e])
                loggammaFe = np.log10(gammas[ci.Element.Fe])
                if e == ci.Element.Placeholder:
                    # ^ This used to be Oxygen;
                    # for Fischer O parametrisation we need to use this logic!
                    logX_met = (
                        logkd_app
                        - loggamma
                        - log10_gammaFe_sil
                    ...
                    # mkdSd_o[e] = (
                    #     10**(logX_met + sigma_logX_met),
                    #     10**(logX_met - sigma_logX_met)
                    # )
                elif e == ci.Element.S:
                    # See Boujibar+ 2014 eqs 6 and 11
                    # gammas not needed, but they are used for how other elements
                    # interact with S

                    # Maybe replace this with Terry-Ann Suer's 2017 paper! (equation 8
                    # of www.sciencedirect.com/science/article/pii/S0012821X17301954)

                    if mantle_composition is None:
                        # Use a very rough approximation of Earth.
                        # Doesn't matter much - this is just an initial guess
                        X_FeO = 0.06
                        ...
                        # The logic here is that I'm assuming all these elements in the
                        # silicate are fully oxidised.
                        # (And that this accounts for all the Oxygen)
                        # So to get X for FeO, it's just the fraction of Fe out of
                        # everything that's not oxygen
                        # So X_FeO = X_Fe/(1 - X_O)
                        # This can probably afford to be v. rough: it's only to do part
                        # of the calculation for Sulfur after all
                        X_O = mantle_composition[ci.Element.O]
                        oxygen_modifier = 1 / (1 - X_O)

                        # If these are not present in the dict at this stage, it means
                        # they are not present in the bulk composition. Hence a default
                        # value of 0

                        X_FeO = oxygen_modifier * mantle_composition.get(
                            ci.Element.Fe, 0
                        )
                        X_CaO = oxygen_modifier * mantle_composition.get(
                            ci.Element.Ca, 0
                        )
                        X_MgO = oxygen_modifier * mantle_composition.get(
                            ci.Element.Mg, 0
                        )
                        X_TiO2 = oxygen_modifier * mantle_composition.get(
                            ci.Element.Ti, 0
                        )
                        # Dividing the next ones by 2 because there's half as many
                        # Na2(O) as Na(O)
                        X_Na2O = (
                            0.5
                            * oxygen_modifier
                            * mantle_composition.get(ci.Element.Na, 0)
                        )
                        X_K2O = (
                            0.5
                            * oxygen_modifier
                            * mantle_composition.get(ci.Element.K, 0)
                        )
                        # I'm assuming the FeO wt % is directly proportional to the
                        # molar %
                        # and taking the constant from cells K3 and G3 here:
                        # www.researchgate.net/publication/
                        #   308404683_Sulfur_partitioning_calculator
                        # This is potentially inaccurate
                        X_FeO_sil_wt_percent = 127.5862069 * X_FeO

                    core_composition_by_mass = self.convert_composition_to_mass(
                        core_composition
                    )
                    ...
                    logCs = self.calculate_logCs(
                        X_FeO, X_CaO, X_MgO, X_TiO2, X_Na2O, X_K2O
                    )
                    S_interactions = self.calculate_S_interactions(
                        X_Si, X_C, X_Fe, X_Ni, X_O
                    )

                    logD = (
                        np.log10(X_FeO_sil_wt_percent)
                        - logCs
                        + logkd_app
                        + S_interactions
                    )
                    ...
                    additional_term = (
                        ((1 - (0.5 * p_v[e])) * loggammaFe)
                        if self.source[e] in ["f1", "f2", "b"]
                        else 0
                    )
                    # ^ We take care of gamma_0 elsewhere by setting it to 0 in the input file
                    logD = (
                        logkd_app
                        - loggamma
                        + (0.5 * p_v[e] * (log10_gammaFe_sil - (0.5 * dIW)))
                        + additional_term
                    )
                    # sigma_log_dfe_2 = ((self.sigv[e]/4.)**2)*dIW**2
                ...
                loggammaFe = np.log10(gammas[ci.Element.Fe])
                logdfe = self.calculate_logDFe(log10_gammaFe_sil, dIW, loggammaFe)
                # ^ When iterating over composition, we don't need to recalculate this
                # sigma_log_dfe_2 = ((self.sigv[e]/4.)**2)*dIW**2  # This isn't v nice
                d_dict[e] = 10**logdfe
                # mkdSd_o[e] = (
                #     10**(logdfe + sigma_log_dfe_2),
                #     10**(logdfe - sigma_log_dfe_2)
                # )
        return d_dict

    # NB: Corgne+ 2008 parametrise KD_app as shown here, but Fischer+ 2015 actually
    # parametrise KD instead, which is why the fischer_update corrections exist
    ...
    def calculate_logDFe(self, log10_gammaFe_sil, dIW, loggammaFe):
        ...
        # dIW := 2log10(gammaFe_sil/gammaFe) + 2log10(1/D_Fe)
        # => 0.5dIW = log10(gammaFe_sil) - loggammaFe - logD_Fe
        # => logD_Fe = log10(gammaFe_sil) - 0.5*dIW - loggammaFe
        logdfe = log10_gammaFe_sil - ((0.5 * dIW) + loggammaFe)
        return logdfe

    def calculate_logCs(self, X_FeO, X_CaO, X_MgO, X_TiO2, X_Na2O, X_K2O):
        # Args are all number fractions in the silicate
        # Boujibar+ 2014 eq 6
        return (
            (3.15 * X_FeO)
            + (2.65 * X_CaO)
    ...
    def calculate_S_interactions(self, X_Si, X_C, X_Fe, X_Ni, X_O):
        # Args are all mass fractions in metal
        d = 32
        e = 181
        f = 305
        g = 30.2
    ...
    def peridotite_liquidus(self, P):
        ...
        # built on fitting Hirschmann 2000 data
        # (high P, >3.8785673242673817 GPa, corresponds to 100km depth)
        ...
        # an update and assessment of PT conditions for metal–silicate equilibrium
        ...
        # return 1621 + (38.415*P) + (0.00038369*P*P*P) - (0.1958*P*P)
        # ^ eqn. 7 in Badro 2015 appendix
    ...
    def calculate_partition_coefficients(
        self,
        pressure,
        fO2,
        core_composition=None,
        ...
        Ds = self.calculate_D(
            ...
            fO2,
            nbot_to_use,
            self.log10_gammaFe_sil,
            core_composition,
            mantle_composition,
        )
        return Ds

    def get_all_partition_coefficients(
        self, pressure, fO2, abundances=None, temp=None, nbot=None
    ):
        core_composition = self.convert_abundance_dict_to_layer_composition(
            gi.Layer.core, abundances
        )
        mantle_composition = self.convert_abundance_dict_to_layer_composition(
            gi.Layer.mantle, abundances
        )
        Ds = self.calculate_partition_coefficients(
            pressure, fO2, core_composition, mantle_composition, temp, nbot
        )
    ...
    def convert_abundance_dict_to_layer_composition(self, layer, abundances=None):
        ...
        for element in self.ele_set:
            try:
                ...
                # This means that the layer information is missing for some/all elements
                # i.e. there is None
                return None
        return composition
    ...
    def convert_core_composition_to_alloy_composition(self, core_composition):
        alloy = np.array(
            [core_composition.get(element, 0) for element in self.alloy_els]
        )
        return alloy
...
def example():
    ...
    fO2 = -2
    abundances = {}
    # ^ Needed for interaction parameters, but can just leave blank apparently
    temp = None
    # ^ None will make it calculate a temperature based on peridotite liquidus
    nbot = 2.7
