...
# For now, ignore the crust. Treat parent_crust_number_fraction and
# fragment_crust_number_fraction as if they're always 0
...
class EnhancementModel:
    ...
    def find_enhancements_nonearthlike(
        self,
        geo_model,
        disc_abundances,
        elements,
        ...
        for element in elements:
            ...
            final = (fragment_mantle_number_fraction * fragment_mantle_abundance) + (
                fragment_core_number_fraction * fragment_core_abundance
            )
    ...
    def find_enhancements_with_fixed_light_element(
        self,
        geo_model,
        disc_abundances,
        elements,
        ...
        # add S to the mantle - Not modelled properly yet, so let's randomise it up to
        # 5% (Earth is 0% McDonough 2003, Mars is 2% Yoshizaki 2020, so we're covering
        ...
        # Here's the new bit: we take any light element out of the calculated core, and
        # replace it with the equivalent amount of the elements in the following dict:
        light_elements = {
            # The number is what fraction of the light element content corresponds to
        ...
        heavy_element_content = sum(
            [number_abundances[el][gi.Layer.core] for el in heavy_elements]
        )
        ...
        for el in number_abundances:
            if el in heavy_elements:
                ...
                number_abundances[el][gi.Layer.core] = (
                    light_elements[el] * light_element_content
                )
            else:
                number_abundances[el][gi.Layer.core] = 0.0
        # And normalise (although, as long as the light_elements dict is normalised, I
        # don't think this is actually necessary for the core)
        total_core_abundance = sum(
            [number_abundances[el][gi.Layer.core] for el in number_abundances]
        )
        total_mantle_abundance = sum(
            [number_abundances[el][gi.Layer.mantle] for el in number_abundances]
        )
        ...
        for element in elements:
            ...
            fragment_mantle_abundance = number_abundances[element][gi.Layer.mantle]
            # But the bulk is different if it has different mantle/core fractions:
            enhancements[element] = (
                fragment_mantle_number_fraction * fragment_mantle_abundance
            ) + (fragment_core_number_fraction * fragment_core_abundance)
        diagnostics_dict = {
            "Abundances": number_abundances,
            "ParentCoreNumberFraction": None,
            # ^ We changed it when we overwrote the light element, so would need to
            # recalculate if we need this
            "Ds": Ds,
        }
        return enhancements, diagnostics_dict

    def find_enhancements_earthlike(
        self,
        geo_model_ignore,
        disc_abundances,
        elements,
        ...
        if (
            (parent_core_number_fraction is None)
            or (parent_crust_number_fraction is None)
            or (fragment_core_number_fraction is None)
            or (fragment_crust_number_fraction is None)
        ):
            # This means no differentiation took place
            return disc_abundances, {
                "ParentCoreNumberFraction": parent_core_number_fraction
        ...
        parent_is_physical = (
            0.19 >= parent_core_number_fraction >= 0
            and parent_crust_number_fraction >= 0
            and parent_core_number_fraction + parent_crust_number_fraction <= 1
        )
        fragment_is_physical = (
            fragment_core_number_fraction >= 0
            and fragment_crust_number_fraction >= 0
            and fragment_core_number_fraction + fragment_crust_number_fraction <= 1
        )
        fragment_is_physical = fragment_is_physical and not (
            fragment_core_number_fraction > parent_core_number_fraction
            and fragment_crust_number_fraction > parent_crust_number_fraction
        )
        fragment_is_physical = fragment_is_physical and not (
            fragment_core_number_fraction
            > (parent_core_number_fraction / (1 - parent_crust_number_fraction))
            and fragment_crust_number_fraction > 0.01
        )
        fragment_is_physical = fragment_is_physical and not (
            (parent_core_number_fraction / (1 - parent_crust_number_fraction))
            >= fragment_core_number_fraction
            >= parent_core_number_fraction
            and fragment_core_number_fraction - parent_core_number_fraction
            > (parent_crust_number_fraction - fragment_crust_number_fraction)
            * (parent_core_number_fraction / (1 - parent_crust_number_fraction))
        )
        ...
        if not (fragment_is_physical and parent_is_physical):
            ...
            # Ignore the input geo_model because it could be non-Earth-like, instead
            # make a new one:
            earthlike_geo_model = gi.GeologyModel(None, normalise_abundances)
            fragment_mantle_number_fraction = 1 - (
                fragment_core_number_fraction + fragment_crust_number_fraction
            )
            parent_mantle_number_fraction = 1 - (
                parent_core_number_fraction + parent_crust_number_fraction
            )
            for element in elements:
                parent_bulk_abundance = earthlike_geo_model.get_bulk_abundance(element)
                parent_crust_abundance = earthlike_geo_model.get_crust_abundance(
                    element
                )
                parent_core_abundance = earthlike_geo_model.get_core_abundance(element)
                parent_mantle_abundance = (
                    parent_bulk_abundance
                    - (
                        (parent_crust_abundance * parent_crust_number_fraction)
                        + (parent_core_abundance * parent_core_number_fraction)
                    )
                ) / parent_mantle_number_fraction
                ...
                if parent_mantle_abundance >= 0:
                    ...
                    if self.earthlike_info[element]["fill_core_first"]:
                        ...
                        fragment_crust_abundance = parent_crust_abundance
                        fragment_core_abundance = (
                            parent_bulk_abundance
                            - (parent_crust_abundance * parent_crust_number_fraction)
                        ) / parent_core_number_fraction

                fragment_bulk_abundance_term1 = (
                    fragment_crust_number_fraction * fragment_crust_abundance
                )
                fragment_bulk_abundance_term2 = (
                    fragment_mantle_number_fraction * fragment_mantle_abundance
                )
                fragment_bulk_abundance_term3 = (
                    fragment_core_number_fraction * fragment_core_abundance
                )
                fragment_bulk_abundance = (
                    fragment_bulk_abundance_term1
                    + fragment_bulk_abundance_term2
                    + fragment_bulk_abundance_term3
                )

                enhancements[element] = disc_abundances[element] * (
                    fragment_bulk_abundance / parent_bulk_abundance
                )
