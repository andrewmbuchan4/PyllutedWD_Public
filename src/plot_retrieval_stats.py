...
def find_stats_files(path, system_prefixes):
    ...
    for system_prefix in system_prefixes:
        for directory_or_file in os.listdir(path):
            if os.path.isdir(path + directory_or_file) and directory_or_file.startswith(
                system_prefix
            ):
...
def extract_retrieval_data(path, system_prefixes):
    # This function could do with some revision
...
def plot_retrieved_stats():
    ...
    for variable in [
        ...
        earth_plot["retrieved_variable_plot"]["subplots"]["subplot1"][
            "y_hide_ticks"
        ] = [0]
