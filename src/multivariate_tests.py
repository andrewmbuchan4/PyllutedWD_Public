...
try:
    ...
    # _fasano_franceschini = importr('fasano.franceschini.test')
    # ^ Feel free to add this back in, it seemed insensitive to the differences I saw in
    # the data though
...
def cramer_test_p_value(
    data1, data2, confidence_level=0.95, replicate_count=1000, bootstrap_type="ordinary"
):
...
def fasano_franceschini_test_p_value(data1, data2, n_permutations=100):
    ...
    result = _fasano_franceschini.fasano_franceschini_test(
        data1, data2, **{"nPermute": n_permutations, "verbose": verbose}
    )
    result_dict = convert_rlistvector_to_dict(result)
    return result_dict["p.value"]
...
def demo():
    data_set1 = [[1, 2, 3, 4, 5], [10, 20, 30, 40, 50], [11, 21, 31, 41, 51]]
    data_set2 = [[2, 1, 6, 1.7], [11, 28, 20, 50], [11, 22, 31, 42]]
    data_set1_r = convert_data_to_r_format(data_set1)
    data_set2_r = convert_data_to_r_format(data_set2)
    p_value_cramer = cramer_test_p_value(data_set1_r, data_set2_r)
...
def get_mv_test_method_dict():
    # return {
    #     MultiVariateTest.cramer: cramer_test_p_value,
    #     MultiVariateTest.ff: fasano_franceschini_test_p_value
    # }
