            ...
            with open("../data/" + file_dict[HorHe][g]) as csvfile:
                ...
                for row in read:
                    if i == 0:
                        ...
                        q_vals = row[1:]
                        for j in range(1, len(row)):
                            toret[HorHe][g][int(T_vals[j - 1])]["logq"] = float(
                                q_vals[j - 1]
                            )
                    else:
...
def perform_interpolation(data, x1, x2, y1, y2, x, y):
    toret = dict()
    for key in data[x1][y1].keys():
        # This is just a way of looping through all the elements (and q)
        if x1 == x2:
            ...
            # Bilinear interpolation in x and y, following notation of
...
def extract_timescales(data, HorHe, Teff, logg):
    ...
    for g_test in data[HorHe].keys():
        if g_test <= logg:
            ...
            if (current_best_lower_diff is None) or (
                lower_diff < current_best_lower_diff
            ):
                current_best_lower_diff = lower_diff
                g_lower_bound = g_test
        if g_test >= logg:
            upper_diff = g_test - logg
            if (current_best_upper_diff is None) or (
                upper_diff < current_best_upper_diff
            ):
                current_best_upper_diff = upper_diff
                g_upper_bound = g_test
    # At this point, if either of the g bounds is None, then we are outside the range of
    # the data, so just use the min/max value of g as appropriate
    ...
    g_value = g_upper_bound if g_lower_bound is None else g_lower_bound
    # ^ This doesn't matter, it's just to read the T values which should be the same
    # for all g
    t_lower_bound = None
    current_best_lower_diff = None
    t_upper_bound = None
    ...
    if (g_lower_bound == g_upper_bound) and (t_lower_bound == t_upper_bound):
        # This means that one of the parameters was out of bounds and the other was in
        # the keys already (or both out of bounds?)
        # Can now just look up the appropriate value
        return data[HorHe][g_lower_bound][t_lower_bound]
    return perform_interpolation(
        data[HorHe],
        g_lower_bound,
        g_upper_bound,
        t_lower_bound,
...
def main():
    timescale_data = load_detlev_data()
    # ^ Structure:
    # data[g][Teff] = { 'logq': <q>, Element.Li: <t_Li>, ... Element.Zn: <t_Zn> }
