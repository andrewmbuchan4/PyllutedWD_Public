...
# This script is designed to be used on xlsx outputs from the old version of the code
# (in original_codebase) - it's essentially deprecated now and included just for
# completeness
...
def collect_best_fits():
    ...
    volatile_poor_temp = 1400
    # ^ Above this temp, we're volatile poor (including moderate volatiles) (loosely
    # basing this on Lodders 2003 table 8)
    for xlsx_file in find_xlsx_files():
        system = xlsx_file.split("PWDOutputs")[0]
        if (
            system.startswith("Gaia")
            or system.startswith("SDSSJ2047")
            or system.startswith("PG")
            or system.startswith("WD")
        ):
        ...
        # True: we will determine whether something is core/crust/mantle rich based on
        #  whether the fragment is enriched in those layers relative to its parent
        # False: we will determine whether something is core/crust/mantle rich based on
        #  whether the fragment is primarily core/crust/mantle
        ...
        if not primitive:
            ...
            if measure_layers_relative_to_parent:
                ...
                if delta_crust == "":
                    ...
                    # This fixes a bug in the original code which assumed an earth value
                    # of 0.01, not 0.001.
                    # So it subtracts 0.01 in the default case, ie the cases where a
                    # parent crust fragment is not specified.
                ...
                core_rich = (
                    fragment_core_frac > fragment_crust_frac
                    and fragment_core_frac > fragment_mantle_frac
                )
                crust_rich = (
                    fragment_crust_frac > fragment_core_frac
                    and fragment_crust_frac > fragment_mantle_frac
                )
                mantle_rich = (
                    fragment_mantle_frac > fragment_crust_frac
                    and fragment_mantle_frac > fragment_core_frac
                )
        ...
        buildup_phase = (
            steady_state_sigma == na_string and declining_phase_sigma == na_string
        )
...
def write_summary_table(best_fit_dict):
    ...
    phases = {"BuildUpPhase": 0, "SteadyStatePhase": 0, "DecliningPhase": 0}
    for sys, vals in best_fit_dict.items():
        if not vals["GoodFit"]:
            unclassified += 1
        else:
    ...
    with open("summary_table_hb20.csv", "w", newline="", encoding="utf-8") as f:
        ...
        to_write.writerow([])
        to_write.writerow(["", "Primitive", "Core Rich", "Mantle Rich", "Crust Rich"])
        ...
        to_write.writerow([])
        to_write.writerow(["Build Up Phase", "Steady State Phase", "Declining Phase"])
        to_write.writerow(
            [
                phases["BuildUpPhase"],
