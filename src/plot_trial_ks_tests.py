...
def trial_ks_tests():
    ...
    configs = {
        ...
        #     'half_bin_size': 0.5
        # },
        "med": {"N": 100, "half_bin_size": 0.25},
        "large": {"N": 1000, "half_bin_size": 0.2},
        "vlarge": {"N": 3000, "half_bin_size": 0.15},
        "vvlarge": {
            "N": 10001,
            # ^ 10001 is just over the asymp activation threshold. 10000 is too low
            "half_bin_size": 0.1,
        },
    }
    ...
    for config_name, config in configs.items():
        ...
        ks_statistic_original, p_value_original = pipeline.run_ks_test_on_samples(
            data1, data2, min_bin_edge, max_bin_edge, config["half_bin_size"]
        )

        new_result = st.ks_2samp(data1, data2, "two-sided")

        equiv_result = st.ks_2samp(data1, data2, "two-sided", "asymp")
        # ^ Always use the approx. version (usually deployed only for N > 10000).
        # This is supposedly equivalent to the original one but appears to be different,
        # maybe because of intermediate step I perform of converting to a cdf
