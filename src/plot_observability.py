...
def generate_data(test_args_all):
    manager = mn.Manager(
        Namespace(
            ...
            n_live_points=0,
            # ^ This argument shouldn't matter, in fact we only use the manager to
            # access observational data so nothing else matters
    ...
    for element, test_args in test_args_all.items():
        ...
        for arg_name, arg_set in test_args.items():
            ...
            for pol_frac in generate_pollution_fraction_values():
                ...
                for el in ci.usual_elements:
                    data_to_plot[element][arg_name][get_element_key(el)].append(
                        test_result[0][el]
                    )
    return data_to_plot
...
def tabulate_contour_data(contour_data_to_plot, path=None):
    ...
    pol_frac_hack = -1
    # ^ This is a hack. This number needs to be the same as the one in
    # plot_contour_observability_data labelled THIS ONE
    if path is not None:
    ...
    with open(path_to_use, "w", newline="", encoding="utf-8") as f:
        ...
        for P_fcf_key, el_abundances in contour_data_to_plot[last_element].items():
            # Arbitrarily use the last element. We checked earlier that they're all the
            # same
            list_of_vals = [
                str(P_fcf_key[1]),
                str(P_fcf_key[0]),
                str(
                    round(
                        el_abundances[ci.Element.Mg] - el_abundances[ci.Element.Fe], 2
                    )
                ),
                str(
                    round(
                        el_abundances[ci.Element.Ca] - el_abundances[ci.Element.Fe], 2
                    )
                ),
...
def plot_observability_data(fcf_data_to_plot, pressure_data_to_plot):
    ...
    cr_fcf_plot = graph_fac.plot_observability(
        fcf_data_to_plot[ci.Element.Cr],
        ci.Element.Ca,
        ci.Element.Cr,
        ci.Element.Ca,
        ci.Element.Fe,
    )
    cr_pressure_plot = graph_fac.plot_observability(
        pressure_data_to_plot[ci.Element.Cr],
        ci.Element.Ca,
        ci.Element.Cr,
        ci.Element.Ca,
        ci.Element.Fe,
    ...
    ni_fcf_plot = graph_fac.plot_observability(
        fcf_data_to_plot[ci.Element.Ni],
        ci.Element.Ca,
        ci.Element.Ni,
        ci.Element.Ca,
        ci.Element.Fe,
    )
    ni_pressure_plot = graph_fac.plot_observability(
        pressure_data_to_plot[ci.Element.Ni],
        ci.Element.Ca,
        ci.Element.Ni,
        ci.Element.Ca,
        ci.Element.Fe,
    ...
    si_fcf_plot = graph_fac.plot_observability(
        fcf_data_to_plot[ci.Element.Si],
        ci.Element.Ca,
        ci.Element.Si,
        ci.Element.Ca,
        ci.Element.Fe,
    )
    si_pressure_plot = graph_fac.plot_observability(
        pressure_data_to_plot[ci.Element.Si],
        ci.Element.Ca,
        ci.Element.Si,
        ci.Element.Ca,
        ci.Element.Fe,
    ...
    graph_fac.multipanelise(
        ...
        1,
        2,
        ["observability_multipanel_cr_only.png"],
        5,
        10,
        0,
        0,
    ...
    graph_fac.multipanelise(
        ...
        1,
        2,
        ["observability_multipanel_ni_only.png"],
        5,
        10,
        0,
        0,
    ...
    graph_fac.multipanelise(
        ...
        1,
        2,
        ["observability_multipanel_si_only.png"],
        5,
        10,
        0,
        0,
...
def extract_el_el(contour_data_to_plot_el, numerator_el, denom_el, pol_frac=-6):
    fcf_index = 0
    toret_array = np.zeros(
        (len(generate_fcf_values()), len(generate_pressure_values()))
    )
    toret_hx_array = np.zeros(
        (len(generate_fcf_values()), len(generate_pressure_values()))
    )
    while fcf_index < len(generate_fcf_values()):
        ...
        while p_index < len(generate_pressure_values()):
            ...
            toret_array[fcf_index][p_index] = (
                contour_data_to_plot_el[(p, fcf)][numerator_el]
                - contour_data_to_plot_el[(p, fcf)][denom_el]
            )
            toret_hx_array[fcf_index][p_index] = (
                contour_data_to_plot_el[(p, fcf)][numerator_el] + pol_frac
            )
            p_index += 1
        fcf_index += 1
...
def build_guideline(
    contour_data_to_plot_el, guideline_num_el, guideline_denom_el, path_to_follow
):
    toret = dict()
...
def plot_contour_observability_data(contour_data_to_plot):
    # Structure: contour_data_to_plot_el[el_of_that_run][(p, fcf)][element_of_interest]
    graph_fac = gf.GraphFactory()
    ...
    for element in contour_data_to_plot.keys():
        processed_data_tuple = extract_el_el(
            contour_data_to_plot[element], element, comparison_el, pol_frac
        )
        processed_data[element] = processed_data_tuple[0]
        processed_hx_data[element] = processed_data_tuple[1]
        guideline_hx[element] = build_guideline(
            contour_data_to_plot[element],
            guideline_num_el,
            guideline_denom_el,
            path_to_follow,
        )
    ...
    graph_fac.multipanelise(
        all_plots, 3, 1, ["observability_contour_multipanel_ss.pdf"], 15, 10, 0, None
    )
    graph_fac.multipanelise(
        all_hx_plots,
        3,
        1,
        ["observability_contour_hx_multipanel_ss.pdf"],
        15,
        10,
        0,
        None,
...
def plot_el_v_teff_data(el_v_teff_data):
    for element in [ci.Element.Ni, ci.Element.Cr, ci.Element.Si]:
        ...
        graph_fac.plot_el_v_Teff(
            element, el_vals, teff_vals, el_upper_bound_vals, teff_upper_bound_vals
        )
