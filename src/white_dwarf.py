...
class WhiteDwarfDataPoint:
    ...
    def __init__(
        self, data_point_type, value, upper_error=None, included=True, lower_error=None
    ):
        # data_point_type should be a WhiteDwarfDataPointType
        # By default, lower_error is assumed to be the same as upper_error such that
        # upper_error is the only one you need to care about
        ...
        if self.data_point_type == WhiteDwarfDataPointType.measurement:
            if upper_error is not None:
                ...
                # ^ Only applicable if data_point_type is measurement, in which case
                # this is the 1 sigma error
                if lower_error is None:
                    self.lower_error = self.upper_error
                    # ^ Can set this separately if errors are asymmetrical
                else:
                    self.lower_error = float(lower_error)
            else:
                raise ValueError(
                    "WhiteDwarfDataPointType is measurement, but no error was supplied"
                )
        else:
...
class WhiteDwarfAbundanceData:
    ...
    def __init__(self, abundance_data_dict):
        # Expecting {ci.Element: WhiteDwarfDataPoint, ... }
        self.abundance_data_dict = abundance_data_dict
        for element, abundance_data in self.abundance_data_dict.items():
            if abundance_data.value > 0:
                abundance_data.value = -abundance_data.value
                # ^ These values are always negative - now you don't need to keep typing
                # minus!

    def get_plottable_points(
        self, elements_to_plot, reference_element, included=True, scale_to_solar=False
    ):
            ...
            if el != reference_element:
                if el_data.included == included:
                    if el_data.data_point_type == WhiteDwarfDataPointType.measurement:
                        values_toret.append(
                            self.get_relative_value(
                                el, reference_element, scale_to_solar
                            )
                        )
        ...
        return (
            ...
            upper_errors_toret,
            lower_errors_toret,
            upper_bounds_bools_toret,
            lower_bounds_bools_toret,
        )

    def get_relative_value(self, element, reference_element, scale_to_solar):
        wd_rel_abundance = (
            self.abundance_data_dict[element].value
            - self.abundance_data_dict[reference_element].value
        )
        if scale_to_solar:
            solar_rel_abundance = sa.get_solar_relative_abundance(
    ...
    def get_measurements_as_lists(
        self,
        return_included=True,
        return_excluded=False,
        elements=ci.all_elements,
        ...
        for el in elements:
            try:
                ...
                if preserve_length:
                    ...
                    continue
            if el_data.data_point_type == WhiteDwarfDataPointType.measurement and (
                (el_data.included and return_included)
                or (not el_data.included and return_excluded)
            ):
                els_toret.append(el)
        ...
        for el in elements:
            try:
                ...
                if preserve_length:
                    ...
                    continue
            if el_data.data_point_type == WhiteDwarfDataPointType.upper_bound and (
                (el_data.included and return_included)
                or (not el_data.included and return_excluded)
            ):
                els_toret.append(el)
        ...
        for el in elements:
            try:
                ...
                if preserve_length:
                    ...
                    continue
            if el_data.data_point_type == WhiteDwarfDataPointType.lower_bound and (
                (el_data.included and return_included)
                or (not el_data.included and return_excluded)
            ):
                els_toret.append(el)
    ...
    def get_abundance_arrays(self, elements=ci.usual_elements):
        ...
        for element in elements:
            ...
            if data_point is None:
                ...
                abundances.append(
                    data_point.value
                    if data_point.included
                    and data_point.data_point_type
                    == WhiteDwarfDataPointType.measurement
                    else None
                )
                errors.append(
                    data_point.upper_error
                    if data_point.included
                    and data_point.data_point_type
                    == WhiteDwarfDataPointType.measurement
                    else None
                ...
                upper_bounds.append(
                    ...
                    if data_point.included
                    and data_point.data_point_type
                    == WhiteDwarfDataPointType.upper_bound
                    else None
                )
                lower_bounds.append(
                    data_point.value
                    if data_point.included
                    and data_point.data_point_type
                    == WhiteDwarfDataPointType.lower_bound
                    else None
                )
        return (
            np.array(abundances),
    ...
    def get_elements_present(self):
        ...
        for element, el_data in self.abundance_data_dict.items():
            if (
                el_data.data_point_type == WhiteDwarfDataPointType.measurement
                and el_data.included
                and el_data.value not in [None, np.nan]
            ):
    ...
    def get_modellable_detected_elements(self, detectable_elements=ci.usual_elements):
        ...
        for el in detectable_elements:
            ...
            if (
                ...
                and data_point.data_point_type == WhiteDwarfDataPointType.measurement
                and data_point.upper_error > 0
                and data_point.lower_error > 0
            ):
                toret.append(el)
        return toret
...
class WhiteDwarfPropertyData:
    ...
    def __init__(self, property_dict):
        # Expecting property_dict = {mp.WDParameter: WhiteDwarfDataPoint, ... }
        self.property_dict = property_dict
        # I'm here making the decision that a White Dwarf has a 'spectral_type'
        # ('DA' or 'DB' for now)
        # And, separately, an 'atmospheric_type' (either ci.Element.H or ci.Element.He,
        # accordingly)
        # I will further assume that DA <==> ci.Element.H and DB <==> ci.Element.He
        # (not quite true in reality - this is a simplification which I think makes
        # sense here)
    ...
    def fill_in_spectral_and_atmospheric_types(self):
        if (
            self.get_property_data(mp.WDParameter.spectral_type) is None
            and self.get_property_data(mp.WDParameter.atmospheric_type) is not None
        ):
            if (
                self.get_property_data(mp.WDParameter.atmospheric_type).value
                == ci.Element.H
            ):
                self.property_dict[mp.WDParameter.spectral_type] = WhiteDwarfDataPoint(
                    WhiteDwarfDataPointType.label, "DA"
                )
            elif (
                self.get_property_data(mp.WDParameter.atmospheric_type).value
                == ci.Element.He
                ...
                self.property_dict[mp.WDParameter.spectral_type] = WhiteDwarfDataPoint(
                    WhiteDwarfDataPointType.label, "DB"
                )
            else:
                pass
                # raise ValueError(
                #     'Must have a valid spectral type or atmospheric type! Received '
                #     + str(self.get_property_data(mp.WDParameter.spectral_type))
                #     + ' and '
                #     + str(self.get_property_data(mp.WDParameter.atmospheric_type))
                # )
        elif (
            self.get_property_data(mp.WDParameter.spectral_type) is not None
            and self.get_property_data(mp.WDParameter.atmospheric_type) is None
        ):
            if self.get_property_data(mp.WDParameter.spectral_type).value == "DA":
                self.property_dict[mp.WDParameter.atmospheric_type] = (
                    WhiteDwarfDataPoint(WhiteDwarfDataPointType.label, ci.Element.H)
                )
            elif self.get_property_data(mp.WDParameter.spectral_type).value == "DB":
                self.property_dict[mp.WDParameter.atmospheric_type] = (
                    WhiteDwarfDataPoint(WhiteDwarfDataPointType.label, ci.Element.He)
                )
            else:
                pass
                # raise ValueError(
                #     'Must have a valid spectral type or atmospheric type! Received '
                #     + self.get_property_data(mp.WDParameter.spectral_type)
                #     + ' and '
                #     + self.get_property_data(mp.WDParameter.atmospheric_type)
                # )
        else:
            pass
    ...
    def check_spectral_and_atmospheric_types(self):
        if (
            ...
            # Allow this as a special case - we don't demand that the user must supply
            # these properties
            return
        ...
        if self.get_property_data(mp.WDParameter.atmospheric_type) is None:
            raise ValueError("Spectral/atmospheric types were not valid")
        valid_da_combo = (
            self.get_property_data(mp.WDParameter.spectral_type).value == "DA"
            and self.get_property_data(mp.WDParameter.atmospheric_type).value
            == ci.Element.H
        )
        valid_db_combo = (
            self.get_property_data(mp.WDParameter.spectral_type).value == "DB"
            and self.get_property_data(mp.WDParameter.atmospheric_type).value
            == ci.Element.He
        )
        if not (valid_da_combo or valid_db_combo):
            raise ValueError(
                "Spectral type and atmospheric types are not consistent! Received "
                + str(self.get_property_data(mp.WDParameter.spectral_type).value)
                + " and "
                + str(self.get_property_data(mp.WDParameter.atmospheric_type).value)
            )

    def __str__(self):
        toret = ""
        for wd_property in mp.WDParameter:
            if wd_property not in [
                mp.WDParameter.logq,
                mp.WDParameter.timescale_type,
                mp.WDParameter.consider_thermohaline,
            ]:
                # logq is associated with atmosphere data, it's not treated as a
                # property of the white dwarf itself. Similarly, the timescale_type
                # (and consider_thermohaline) is only a WDParameter for the sake of the
                # synthetic WDs - normally it would be associated with the atmosphere
                # data
...
class WhiteDwarfAtmosphereData:
    ...
    def __init__(self, timescale_and_logq_dict):
        # Expecting input to be a dict containing both logq and all the timescales, as
        # this is what is calculated by TimescaleInterpolator
        self.logq_string = "logq"
        try:
            self.logq = WhiteDwarfDataPoint(
                WhiteDwarfDataPointType.measurement,
                timescale_and_logq_dict[self.logq_string],
                0,
            )
            # ^ For now, assume zero error on this
        except KeyError:
            self.logq = WhiteDwarfDataPoint(
                WhiteDwarfDataPointType.measurement,
                timescale_and_logq_dict[mp.WDParameter.logq],
                0,
            )
            # ^ For now, assume zero error on this
        self.timescale_dict = {
            element: timescale_and_logq_dict[element]
            for element in timescale_and_logq_dict
            if element not in [self.logq_string, mp.WDParameter.logq]
        }
...
class WhiteDwarfAtmosphereDataset:
    ...
    def __init__(self, atmosphere_data_dict):
        self.atmosphere_data_dict = atmosphere_data_dict
        # ^ Expecting {ti.TimescaleType.something: WhiteDwarfAtmosphereData}
    ...
    def get_timescales_as_array(self, timescale_type, elements=ci.usual_elements):
        try:
            return self.atmosphere_data_dict[timescale_type].get_timescales_as_array(
                elements
            )
        except KeyError:
            return None
    ...
    def get_timescale_values_dict(self, timescale_type, elements=ci.usual_elements):
        return self.atmosphere_data_dict[timescale_type].get_timescale_values_dict(
            elements
        )
...
class WhiteDwarf:
    ...
    def __init__(self, name_tuple_or_str, properties, abundances, timescales=dict()):
        ...
        # ^ Should be a WhiteDwarfPropertyData. Create a deepcopy because we want this
        # to be unique
        self.abundances = deepcopy(abundances)
        # ^ Should be a WhiteDwarfAbundanceData. Create a deepcopy because we want this
        # to be unique
        # TODO : add sinking timescales to this
        if len(self.abundances.abundance_data_dict) > 0:
            if (
                self.properties.get_property_data(mp.WDParameter.atmospheric_type)
                is None
            ):
                raise ValueError(
                    "Must supply an atmospheric type if supplying abundances"
                )
                # otherwise the abundances mean nothing
        self.abundances.abundance_data_dict[
            self.properties.get_property_data(mp.WDParameter.atmospheric_type).value
        ] = WhiteDwarfDataPoint(WhiteDwarfDataPointType.measurement, 0, 0, False)
    ...
    def abbreviated_name(self, chars_to_remove):
        ...
        while len(toret) > max_length:
            if (
                len(abbreviated_variants[variant_index]) < 3
                or variant_index == len(abbreviated_variants) - 1
            ):
            ...
            if abbreviated_variants[variant_index] is None:
                ...
                toret = (
                    abbreviated_system_names[system_name_index]
                    + "_"
                    + abbreviated_variants[variant_index]
                )
        self.abbreviated_name_as_used = toret
        return toret

    def get_abbreviated_system_names(self):
        special_prefixes = ["USNO-A2.0", "USNO-B1.0"]
        # ^ These mess around with the rest of the logic, so we'll handle them
        # separately.
        # What we're going to try here is look for coordinates that we can truncate.
        # Identify these by presence of a + or -.
        ...
        # ^ case sensitive
        # There's a slight problem with this logic because we end up with no option to
        # abbreviated the above strings and not also abbreviate the coordinate - but we
        # should probably default to this if possible!
        ...
        while len(coords[0]) > min_coord_length and len(coords[1]) > min_coord_length:
            # Now we need to know how many decimal places they each have, again assuming
            # a dot will only appear (max) once...
            dp_counts = [0, 0]
            for i in [0, 1]:
                if "." in coords[i]:
            ...
            abbreviation = (
                special_prefix + prefix + coords[0] + cs_present + coords[1] + suffix
            )
    ...
    def get_abbreviated_variants(self):
        ...
        if len(suffix) < 1:
            for ss in special_suffixes:
                ...
                # TODO in future: Rather than return, at this point we could
                # sequentially remove characters (similar to the last bit of this
                # function, but with no suffix)
                return toret
    ...
    def unpack_timescales(self, timescales):
        if isinstance(timescales, WhiteDwarfAtmosphereDataset):
            # They are already in the right format (assuming that everything inside is
            # also correct...maybe we should check? Probably overkill)
            return timescales
        toret = dict()
    ...
    def get_plottable_points(
        ...
        included=True,
        scale_to_solar=False,
    ):
        # Switch included to False to get the excluded points
        if reference_element is None:
            reference_element = self.get_atmospheric_type().value
            # ^ This should be set to zero in __init__
        (
            elements_plotted,
            values_toret,
            upper_errors_toret,
            ...
            upper_bounds_bools_toret,
            lower_bounds_bools_toret,
        ) = self.abundances.get_plottable_points(
            elements_to_plot, reference_element, included, scale_to_solar
        )
        return (
            elements_plotted,
            reference_element,
    ...
    def get_errors_for_present_elements_as_array(self, elements=ci.usual_elements):
        ...
        for el in elements:
            ...
            if relevant_data is not None and relevant_data.included:
                if (
                    relevant_data.data_point_type == WhiteDwarfDataPointType.measurement
                    and relevant_data.value not in [None, np.nan]
                ):
    ...
    def log_likelihood(self, model_result, min_likelihood):
        ...
        for el, el_result in model_result.items():
            ...
            if relevant_data is not None and relevant_data.included:
                if (
                    relevant_data.data_point_type == WhiteDwarfDataPointType.upper_bound
                    and el_result > relevant_data.value
                ):
                    return 0.9 * min_likelihood
                elif (
                    relevant_data.data_point_type == WhiteDwarfDataPointType.lower_bound
                    and el_result < relevant_data.value
                ):
                    return 0.9 * min_likelihood
                elif (
        ...
        chi2 = (
            ((data_abundances_to_use - model_abundances_to_use) ** 2)
            * (neg)
            * 2
            * np.pi
        )
    ...
    def estimate_minimum_pollution_fraction(self, elements_to_consider=ci.all_elements):
        ...
        for el in elements_to_consider:
            ...
            if (
                el_data.data_point_type == WhiteDwarfDataPointType.measurement
                and el_data.included
                and el not in ci.atmospheric_types
            ):
                list_of_abundances_for_pol_frac.append(el_data.value)
            elif (
                el_data.data_point_type == WhiteDwarfDataPointType.lower_bound
                and el_data.included
                and el not in ci.atmospheric_types
            ):
                list_of_abundances_for_pol_frac.append(el_data.value)
    ...
    def estimate_maximum_pollution_fraction(self, elements_to_consider=ci.all_elements):
        # Assume solar composition for missing elements, what's approximately the
        # highest pollution fraction we could get for this system?
        safety_margin = 1
        # ^ Guarantees that, even if the below calculation goes wrong somehow, we always
        # return at least the minimum possible pollution fraction + this margin
        ...
        for element in ci.all_elements:
            if (
                element not in reference_element_priority
                and element not in ci.atmospheric_types
            ):
        ...
        for test_ref_el in reference_element_priority:
            ...
            if (
                el_data.data_point_type == WhiteDwarfDataPointType.measurement
                and el_data.included
                and test_ref_el not in ci.atmospheric_types
                and el_data.value is not None
            ):
                ref_el = test_ref_el
        ...
        for el in elements_to_consider:
            ...
            if solar_rel_abundance is None:
                # Effectively we ignore this element if we have to add a default
                # abundance (we don't know what it is)
                solar_abundance = -np.inf
            else:
                solar_abundance = ref_el_value + sa.get_solar_relative_abundance(
                    el, ref_el
                )
            try:
                el_data = self.abundances.abundance_data_dict[el]
                if (
                    ...
                    el_data.data_point_type == WhiteDwarfDataPointType.upper_bound
                    and el_data.included
                    and el not in ci.atmospheric_types
                ):
                    # Add the minimum of either the upper bound or the solar abundance.
                    # Sometimes the upper bounds are really high
                    list_of_abundances_for_pol_frac.append(
                        min(el_data.value, solar_abundance)
                    )
                elif (
                    el_data.data_point_type == WhiteDwarfDataPointType.lower_bound
                    and el_data.included
        ...
        return max(
            estimate,
            self.estimate_minimum_pollution_fraction(elements_to_consider)
            + safety_margin,
        )
    ...
    def get_measurements_as_lists(
        ...
        return self.abundances.get_measurements_as_lists(
            return_included, return_excluded, elements, preserve_length
        )
    ...
    def get_upper_bounds_as_lists(
        ...
        return self.abundances.get_upper_bounds_as_lists(
            return_included, return_excluded, elements, preserve_length
        )
    ...
    def get_lower_bounds_as_lists(
        ...
        return self.abundances.get_lower_bounds_as_lists(
            return_included, return_excluded, elements, preserve_length
        )
    ...
    def __eq__(self, other):
        return (
            ...
            and self.abundances == other.abundances
            and self.atmosphere_data == other.atmosphere_data
        )
