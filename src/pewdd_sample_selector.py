...
dummy_timescale = 0.01  # yr
# ^ Doesn't matter much, just needs to be short relative to
# ^ assumed_accretion_event_timescale
min_thermohaline_temp = 10000
# ^ The minimum temperature that we will consider for thermohaline DAs
...
sample_selection_settings = {
    ...
    "THERMOHALINE_DA": {
        ...
        timescale_pairs_string: [
            (
                ti.TimescaleType.Bedard3DOvershootPatched,
                thermohaline_dummy_override_string,
            )
        ],
    },
}
...
def calculate_discrepancy_metric(
    wd, timescale_pairs, timescale_override_dict=None, D_override=None
):
    # This estimates the number of sigma discrepancy due to difference in timescale
    # values relative to the abundance uncertainties.
    # So, broadly speaking, if the max_discrepency is above 1, that's a sign that we
    # should maybe care
    ...
    try:
        ...
        # Really the solution to this problem is to make real and synthetic white dwarfs
        # inherit from the same class and share basic functions like this
        if wd.observed_abundances is not None:
            detected_elements = [
                el
                for el, val in wd.observed_abundances.items()
                if val not in [None, np.nan, -np.inf]
            ]
        else:
        ...
        for combo in timescale_pairs:
            ...
            if timescales1 is not None and timescales2 is not None:
                ...
                if ratio1 is not None and ratio2 is not None:
                    ...
                    metric = metric_function_for_given_element_pair_X_Y(
                        ratio1, ratio2, D, sigma_X, sigma_Y
                    )
...
def metric_function_for_given_element_pair_X_Y(ratio1, ratio2, D, sigma_X, sigma_Y):
    # This can be derived by considering the difference in log abundance (relative to
    # errors) caused by swapping to a new set of timescales and doing a neat
    # substitution (t = D*tau) to remove the awkward difference-in-reciprocals term and
    # also making a few assumptions eg that the system reaches SS, then hits declining
    # phase (such that the discrepencies just add).
    ...
    error_term = np.sqrt(sigma_X**2 + sigma_Y**2)
    # ^ This has some implicit assumptions which I don't think we need to worry about...
...
def estimate_declining_phase_depth(white_dwarf, timescale_types_to_try):
    # This is super hand wavy, but has some logic behind it! We need to take two things
    # into account:
    corr = estimate_correlation_between_elements_and_timescales(
        white_dwarf, timescale_types_to_try
    )
    chance = estimate_a_priori_chance_of_declining_phase(
        white_dwarf, timescale_types_to_try
    )
    # How can we estimate D? Firstly, it should scale with a_priori_probability in some
    # way, in order to recover D ~ 0 for systems with short sinking timescales (and low
    # a_priori_probabilities).
    # It should also increase with the strength of correlation.
    # This suggests something like D ~ a*c where a is the a_priori_probability and c is
    # the correlation.
    # Caveat: c can be negative, and in this case it seems (intuitively) that we
    # shouldn't decrease the metric (arbitrary choice!)
    # Also, the maximum value of this product is 1, when actually D can go up to
    # max_num_observable_sinking_timescales
...
def estimate_a_priori_chance_of_declining_phase(white_dwarf, timescale_types_to_try):
    # The idea here is that the longer WD sinking timescales are, the greater the chance
    # that they'll be in the declining phase
    reference_element = ci.Element.Mg  # Arbitrary, but also unimportant
    all_chances = list()
    for timescale_type in timescale_types_to_try:
        timescales = get_timescale_values_from_wd_or_dummy(
            white_dwarf, timescale_type, [reference_element]
        )
        reference_sinking_timescale = timescales[reference_element]
        observable_declining_phase_time = (
            max_num_observable_sinking_timescales * reference_sinking_timescale
        )
        max_observable_time = (
            assumed_accretion_event_timescale + observable_declining_phase_time
        )
...
def estimate_correlation_between_elements_and_timescales(
    white_dwarf, timescale_types_to_try
):
    ...
    for timescale_type in timescale_types_to_try:
        timescales = get_timescale_values_from_wd_or_dummy(
            white_dwarf, timescale_type, elements
        )
        # Makes sense to work in terms of log abundances since the log abundances change
        # linearly in time (in declining phase)
        try:
            abundances = white_dwarf.get_abundance_values_dict(elements)
            # ^ This includes upper bounds which must be removed later
        except AttributeError:
            abundances = (
                dict()
                if white_dwarf.observed_abundances is None
                else white_dwarf.observed_abundances
            )
        try:
            # It doesn't matter what the element is, as long as it's actually in this
            # dict: correlation should be invariant to an offset (which is the effect of
            # changing the reference element)
            reference_element = list(abundances)[0]
        except IndexError:
        ...
        try:
            ...
            # For Synthetic white dwarfs
            # Really the solution to this problem is to make real and synthetic white
            # dwarfs inherit from the same class and share basic functions like this
            detected_elements = [
                el
                for el, val in white_dwarf.observed_abundances.items()
                if val not in [None, np.nan, -np.inf]
            ]
        ...
        if len(elements_to_compare) < min_elements_for_correlation:
            # We don't care about these at all - any correlation here is pretty close to
            # meaningless. We want to essentially use this as a 'bonus' to pick out a
            # few extra system that really seem like they must be in the declining phase
            return 0
        ...
        try:
            slope, intercept, r_value, p_value, std_err = scipy.stats.linregress(
                timescales_to_compare, abundances_to_compare
            )
            all_corrs.append(r_value)
        except ValueError:
            # Occurs for the dummy timescales: they're all the same, so no correlation
            # can be calculated
            pass
...
def get_timescale_values_from_wd_or_dummy(
    white_dwarf, timescale_type, elements=ci.usual_elements
):
    if timescale_type == thermohaline_dummy_override_string:
        ...
        try:
            ...
            atm_type = (
                ci.Element.He
                if white_dwarf.wd_properties[mp.WDParameter.spectral_type] == "DB"
                else ci.Element.H
            )
            try:
    ...
    if score_threshold > 0:
        for j, s in enumerate(scores):
            if s >= score_threshold:
                # Broadly speaking, if this is above 1, then the discrepency due to
                # timescales is greater than the error
                indices_for_sample.append(j)
    else:
        for j, s in enumerate(scores):
            if 0 < s <= -score_threshold:
                # Excluding 0 - there's literally no point looking at those
                indices_for_sample.append(j)
...
def pick_out_sample(name_of_sample):
    ...
    filter1 = pi.filter_wds_by_num_modellable_detected_elements(
        manager.white_dwarfs, sample_selection_settings[name_of_sample][num_els_string]
    )
    filter2 = pi.filter_wds_by_atmospheric_type(
        filter1,
        sample_selection_settings[name_of_sample][mp.WDParameter.atmospheric_type],
    )
    wd_candidates = pi.filter_wds_by_teff_logg(
        filter2, min_teff, max_teff, min_logg, max_logg
    )
    for wd in wd_candidates:
        print(wd.full_name())
...
def extract_D_values_from_test_pop(test_pop):
    toret = list()
    for wd in test_pop:
        D = get_declining_phase_depth_from_synthetic_wd(
            wd, timescale_override_dict, timescale_pairs
        )
...
def get_declining_phase_depth_from_synthetic_wd(
    wd, timescale_override_dict, timescale_pairs
):
    event_duration = wd.pollution_properties[mp.ModelParameter.accretion_timescale]
    # ^ yr
    time = 1000000 * wd.pollution_properties[mp.ModelParameter.t_sinceaccretion]
    # ^ in which we convert from Myr to yr
    delta_time = max(0, time - event_duration)  # yr
    flattened_timescale_pairs = flatten_timescale_pairs(timescale_pairs)
    reference_element = ci.Element.Mg  # arbitrary
    reference_sinking_time = np.mean(
        [
            timescale_override_dict[timescale_type_to_try][reference_element]
            for timescale_type_to_try in flattened_timescale_pairs
        ]
    )
...
def get_proxy_pat(dm):
    ...
    # It doesn't reflect situations where values are tightly clustered just below the
    # threshold.
...
def plot_metric_for_synthetic_pop(name_of_pewdd_sample_to_plot_against):
    # Idea here is to, for each Teff and logg, calculate the expected metric value by
    # averaging across a synthetic population of pollutants
    # We can use the population synthesis code to do this - but we will need to override
    # the WD teff and log(g)
    ...
    atm_type = sample_selection_settings[name_of_pewdd_sample_to_plot_against][
        mp.WDParameter.atmospheric_type
    ]
    timescale_pairs = sample_selection_settings[name_of_pewdd_sample_to_plot_against][
        timescale_pairs_string
    ]
    ...
    if thermohaline_dummy_override_string in all_timescale_types:
        ...
        timescale_types_for_grid_range = [
            att
            for att in all_timescale_types
            if att != thermohaline_dummy_override_string
        ]
        min_teff = max(
            [
                ti.teff_ranges[atm_type][timescale_type][0]
                for timescale_type in timescale_types_for_grid_range
            ]
        )
        max_teff = min(
            [
                ti.teff_ranges[atm_type][timescale_type][1]
                for timescale_type in timescale_types_for_grid_range
            ]
        )
        min_logg = max(
            [
                ti.logg_ranges[atm_type][timescale_type][0]
                for timescale_type in timescale_types_for_grid_range
            ]
        )
        max_logg = min(
            [
                ti.logg_ranges[atm_type][timescale_type][1]
                for timescale_type in timescale_types_for_grid_range
            ]
        )
    ...
    if atm_type == ci.Element.He:
        D_values = [0, 0.1, 0.3, 0.6, 1, 2, 5]
        # Sampling appropriately here might be the best approach
        D_values = [1]
        # ...on second thoughts, mathematically it should be equivalent to just take the
        # mean value alone! Which is about 1 (see fig 4.7 of my thesis)
        # On third thoughts it will make a difference for the number of times above a
        # certain threshold, but we can just calculate it for each synthetic WD no?
    else:
        D_values = [0]
    ...
    if atm_type == ci.Element.He:
        ...
        synth_pop_file_name = (
            "popdump_ReferenceDB_RealisticObservererr0p2_NullModeller.csv"
        )
        # ^ this is not fully self consistent because we used VO to generate the
        # population
        ...
        synth_pop_file_name = (
            "popdump_ReferenceDA_RealisticObservererr0p2_NullModeller.csv"
        )
        # ^ this is not fully self consistent because we used 3P to generate the
        # population
    ...
    # The distribution of D should be independent of Teff and logg. I should use fig 4.7
    # in my thesis.
    # Doesn't matter if D is 'too high' for warm DAs - the 'chance' parameter will
    # filter them out later.
    # In principle it should also be exponentially decaying... can we sample, say 100
    # values from an exponentially decaying function?
    # The mean of an exponential distribution is 1/lambda (the first parameter that sets
    # the decay).
    ...
    for i, logg in enumerate(logg_values):
        for j, Teff in enumerate(Teff_values):
            ...
            while len(dm_values) < (5 * num_D_values):
                for wd in test_pop:
                    # We should also filter the WD by Teff and logg, because these might
                    # correlate with detected elements!
                    ...
                    if Teff_dist < max_Teff_dist and logg_dist < max_logg_dist:
                        ...
                        if (
                            wd.observed_abundances is not None
                            and len(wd.observed_abundances) > 1
                        ):
                            lambda_val = estimate_a_priori_chance_of_declining_phase(
                                wd,
                                [
                                    tt
                                    for tt in all_timescale_types
                                    if tt
                                    not in [None, thermohaline_dummy_override_string]
                                ],
                            )
                            D_values = np.random.exponential(lambda_val, num_D_values)
                            for D_override in D_values:
                                dm = calculate_discrepancy_metric(
                                    wd,
                                    timescale_pairs,
                                    timescale_override_dict,
                                    D_override,
                                )
                                dm_values.append(dm)
            ...
            percentage_above_threshold = (
                np.asarray(dm_values) >= metric_threshold
            ).sum() / len(dm_values)
            percentage_above_weak_threshold = (
                np.asarray(dm_values) >= weak_metric_threshold
            ).sum() / len(dm_values)
            percentage_above_strong_threshold = (
                np.asarray(dm_values) >= strong_metric_threshold
            ).sum() / len(dm_values)
    ...
    if include_wd_markers:
        reference_systems = {
            ...
            for wd in pick_out_sample(name_of_pewdd_sample_to_plot_against)
        }
        if atm_type == ci.Element.H:
            full_sample = {
                wd.full_name(): (wd.get_teff().value, wd.get_logg().value)
                for wd in pick_out_all_das()
            }
        elif atm_type == ci.Element.He:
            full_sample = {
                wd.full_name(): (wd.get_teff().value, wd.get_logg().value)
                for wd in pick_out_all_dbs()
            }
        else:
            print(atm_type)
    ...
    graph_fac.plot_discrepancy_metric(
        Teff_values,
        logg_values,
        DM_values,
        timescale_pairs[0][0],
    ...
    graph_fac.plot_discrepancy_metric_as_percentage_above_threshold(
        Teff_values,
        logg_values,
        PAT_values,
        timescale_pairs[0][0],
    ...
    graph_fac.plot_discrepancy_metric_as_percentage_above_threshold(
        ...
        reference_systems,
        full_sample,
        weak_metric_threshold,
    )
    graph_fac.plot_discrepancy_metric_as_proxy_percentage_above_threshold(
        Teff_values,
        logg_values,
        PROXYPAT_values,
        timescale_pairs[0][0],
    ...
    graph_fac.plot_modellable_wd_density(
        Teff_values, logg_values, local_modellable_wd_density, atm_type
    )
    graph_fac.plot_discrepancy_metric(
        Teff_values,
        logg_values,
        DM_values * local_modellable_wd_density,
        timescale_pairs[0][0],
        timescale_pairs[0][1],
        atm_type,
        reference_systems,
    ...
    graph_fac.plot_discrepancy_metric_as_percentage_above_threshold(
        Teff_values,
        logg_values,
        PAT_values * local_modellable_wd_density,
        timescale_pairs[0][0],
        timescale_pairs[0][1],
        atm_type,
        reference_systems,
    ...
    graph_fac.plot_discrepancy_metric_as_percentage_above_threshold(
        Teff_values,
        logg_values,
        PAST_values * local_modellable_wd_density,
        timescale_pairs[0][0],
        timescale_pairs[0][1],
        atm_type,
        reference_systems,
    ...
    graph_fac.plot_discrepancy_metric_as_percentage_above_threshold(
        Teff_values,
        logg_values,
        PAWT_values * local_modellable_wd_density,
        timescale_pairs[0][0],
        timescale_pairs[0][1],
        atm_type,
        reference_systems,
    ...
    graph_fac.plot_discrepancy_metric_as_proxy_percentage_above_threshold(
        Teff_values,
        logg_values,
        PROXYPAT_values * local_modellable_wd_density,
        timescale_pairs[0][0],
        timescale_pairs[0][1],
        atm_type,
        reference_systems,
