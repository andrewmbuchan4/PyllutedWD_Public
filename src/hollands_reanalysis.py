...
def calculate_original_ratio(
    observed_ratio, t_1, t_2, t_sinceaccretion_myr, t_disc_myr
):
    if observed_ratio is None:
        return None
    # Going to find what the ratio of element1 and element2 must have been in the
    # pollutant in order to match what we observe.
    # Without loss of generality, assume the abundance of element 1 is 1, and find the
    # abundance of element 2 to match the observed ratio.
    ...
    while True:
        ...
        output_abundances = wdm.process_snapshot_abundances(
            t_sinceaccretion_myr,
            1000000 * t_disc_myr,
            trial_planetesimal_abundances,
            wd_timescales,
            -6,
        )
        # ^ On a log scale
...
def plot_hollands_elel_ratios(graph_factory, correct_for_sinking=False, use_namg=False):
    ...
    cafe = [
        safe_exponentiate(10, elel)
        for elel in ha.get_hollands_el_el_values(x_el_1, x_el_2, manager)
    ]
    mgfe = [
        safe_exponentiate(10, elel)
        for elel in ha.get_hollands_el_el_values(y_el_1, y_el_2, manager)
    ]
    ...
    # 10: Crust Rich?
    # 11: Volatile Rich?
    # 12: Volatile Depleted?
    # 13: Moderate Volatile Depleted?
    # 14: Build Up Phase?
    ...
    if correct_for_sinking:
        wd_data = dict()
        wd_data["Primitive"] = extract_relevant_ratios(
            wd_names, primitive_wds, corrected_cafe, corrected_mgfe
        )
        wd_data["Core rich"] = extract_relevant_ratios(
            wd_names, core_rich_wds, corrected_cafe, corrected_mgfe
        )
        # wd_data['Mantle rich'] = extract_relevant_ratios(
        #     wd_names,
        #     mantle_rich_wds,
        #     corrected_cafe,
        #     corrected_mgfe
        # )
        wd_data["Crust rich"] = extract_relevant_ratios(
            wd_names, crust_rich_wds, corrected_cafe, corrected_mgfe
        )
        # wd_data['Volatile rich'] = extract_relevant_ratios(
        #     wd_names,
        #     volatile_rich_wds,
        #     corrected_cafe,
        #     corrected_mgfe
        # )
        wd_data["VD"] = extract_relevant_ratios(
            wd_names, volatile_depleted_wds, corrected_cafe, corrected_mgfe
        )
        wd_data["MVD"] = extract_relevant_ratios(
            wd_names, moderate_volatile_depleted_wds, corrected_cafe, corrected_mgfe
        )
        # wd_data['Build-up'] = extract_relevant_ratios(
        #     wd_names,
        #     build_up_wds,
        #     corrected_cafe,
        #     corrected_mgfe
        # )
        # wd_data['Steady State'] = extract_relevant_ratios(
        #     wd_names,
        #     steady_state_wds,
        #     corrected_cafe,
        #     corrected_mgfe
        # )
        # wd_data['Declining'] = extract_relevant_ratios(
        #     wd_names,
        #     declining_wds,
        #     corrected_cafe,
        #     corrected_mgfe
        ...
        wd_data = dict()
        wd_data["Primitive"] = extract_relevant_ratios(
            wd_names, primitive_wds, cafe, mgfe
        )
        wd_data["Core rich"] = extract_relevant_ratios(
            wd_names, core_rich_wds, cafe, mgfe
        )
        # wd_data['Mantle rich'] = extract_relevant_ratios(
        #     wd_names,
        #     mantle_rich_wds,
        ...
        # )
        wd_data["Crust rich"] = extract_relevant_ratios(
            wd_names, crust_rich_wds, cafe, mgfe
        )
        # wd_data['Volatile rich'] = extract_relevant_ratios(
        #     wd_names,
        #     volatile_rich_wds,
        ...
        # )
        wd_data["VD"] = extract_relevant_ratios(
            wd_names, volatile_depleted_wds, cafe, mgfe
        )
        wd_data["MVD"] = extract_relevant_ratios(
            wd_names, moderate_volatile_depleted_wds, cafe, mgfe
        )
        # wd_data['Build-up'] = extract_relevant_ratios(
        #     wd_names,
        #     build_up_wds,
    ...
    plot_dict = graph_factory.make_hollands_elel_scatter_plot(
        wd_data, stellar_data, x_el_1, x_el_2, y_el_1, y_el_2, graph_name
    )
    return plot_dict
...
def analyse_population_material():
    ...
    graph_fac.multipanelise(
        ...
        2,
        [
            "hollands_material_cafemgfe_multipanel.pdf",
            "hollands_material_cafemgfe_multipanel.png",
        ],
        5,
        10,
        None,
    ...
    graph_fac.multipanelise(
        ...
        2,
        [
            "hollands_material_cafenamg_multipanel.pdf",
            "hollands_material_cafenamg_multipanel.png",
        ],
        5,
        10,
        None,
...
def estimate_accretion_phase(
    t_sinceaccretion_samples,
    accretion_timescale_samples,
    time_to_reach_steady_state_myr,
):
    # t_sinceaccretion will be in Myr, accretion_timescales AKA t_event AKA t_disc
    # in log(yr)
    ...
    return {
        ...
        "ss_dec": ss_dec_count,
        "total": i,
        "t_ss": time_to_reach_steady_state_myr,
...
def reclassify_systems():
    ...
    for wd_name in sorted(wd_names):
        (
            ...
            weightpost,
            log_likelihood,
            best_fit_param_values,
            model_parameters,
            model_number,
        ) = ha.extract_posteriors(wd_name)
        print()
        print(model_parameters)
        print(model_number)
        print(best_fit_param_values)

        accretion_timescale_index = model_parameters.index(
            "log(Accretion Event Timescale/Yrs)"
        )
        ...
        # I worry that this is horribly biased towards declining phase solutions, since
        # we sample the accretion timescale in log space, so the alternative below just
        # looks at the single best fit solution.
        # But I feel like really the bias isn't a problem - it just reflects the
        # Bayesian nature of what we're doing (ie the priors favour declining phase)
        ...
        dec_count = (
            accretion_phase_estimate_dict["bu_dec"]
            + accretion_phase_estimate_dict["ss_dec"]
        )
        ...
        # best_chi_square_M1 = -2.0 * (log_likelihood - norm_log)
        # Probably easier to reuse old calculation rather than calculate norm_log

        primitive = check_if_primitive(model_parameters)
        # In the end, I just manually adjusted the original table in the cases where it
        # was apparently primitive and (moderate) volatile depleted

        volatile_poor = "log(Formation Distance/AU)" in model_parameters
        # New logic: if the model invoked heating, its volatile poor

        classification[wd_name] = {
            "Model": model_parameters,
            "Evidence": best_model_evidence,
            # ^ This parameter is basically just here to make me comfortable that we
            ...
            # Not separating VD and MVD in this analysis - can reuse from previous
...
def extract_accretion_lifetime_info(accretion_phase_estimate_dict, threshold=0.67):
    ...
    print(lower_bounds)
    print(upper_bounds)
    # These two numbers define a range of possible values of t_event that would be
    # consistent with all the (meaningful) constraints.
    # There's no guarantee that this would even give a sensible range, but it happens to
    # come out to 0.2-0.7 Myr.
...
def analyse_accretion_lifetime():
    ...
    for wd_name in wd_names:
        ...
        accretion_timescale_median_dict[wd_name] = np.percentile(
            accretion_timescale_samples, 50
        )
        percentile_16 = np.percentile(accretion_timescale_samples, 16)
        percentile_84 = np.percentile(accretion_timescale_samples, 84)
        # Remove anything above the 84th percentile
        filter_1 = np.extract(
            accretion_timescale_samples <= percentile_84, accretion_timescale_samples
        )
        # Remove anything below the 16th percentile. What's left is the one sigma
        # distribution
        filter_2 = np.extract(filter_1 >= percentile_16, filter_1)
        accretion_timescale_onesigma_dict[wd_name] = filter_2
        accretion_timescale_dict[wd_name] = accretion_timescale_samples

        t_sinceaccretion_index = model_parameters.index("Time since Accretion/Myrs")
        t_sinceaccretion_samples = weightpost[:, t_sinceaccretion_index]
        accretion_phase_estimate_dict[wd_name] = estimate_accretion_phase(
            t_sinceaccretion_samples,
            accretion_timescale_samples,
            0.000005 * timescales[wd_name][ci.Element.Mg],
        )
    ...
    scaled_median_delta_times = [
        accretion_phase_estimate_dict[wd_name]["median_delta_time"]
        / (0.000001 * timescales[wd_name][ci.Element.Mg])
        for wd_name in accretion_phase_estimate_dict
    ]
    print()
    print()
    print()
    ...
    delta_time_bins, delta_time_bar_centres = (
        model_analyser.generate_bins_and_bar_centres(0, 5, half_delta_bin_size)
    )

    median_delta_time_heights, dtbins2 = np.histogram(
        scaled_median_delta_times, delta_time_bins, density=True
    )
    ...
    raise
    all_medians = [
        accretion_timescale_median_dict[wd_name]
        for wd_name in accretion_timescale_median_dict
    ]
    print(all_medians)

    individual_systems_to_plot_by_index = [20, 50, 71, 154, 180]
    individual_systems_to_plot_by_name = [
        wd_names[i] for i in individual_systems_to_plot_by_index
    ]
    ...
    bins_small, bar_centres_small = model_analyser.generate_bins_and_bar_centres(
        0, 8, small_half_bin_size
    )
    big_half_bin_size = 0.2
    bins_big, bar_centres_big = model_analyser.generate_bins_and_bar_centres(
        0, 8, big_half_bin_size
    )

    medians_heights, bins2 = np.histogram(all_medians, bins_big, density=True)
    ...
    all_onesigma_heights, bins2 = np.histogram(
        all_onesigma_samples, bins_small, density=True
    )
    ...
    all_heights, bins2 = np.histogram(all_samples, bins_small, density=True)
    ...
    for name, samples in accretion_timescale_dict.items():
        if name in individual_systems_to_plot_by_name:
            system_heights, bins3 = np.histogram(samples, bins_small, density=True)
