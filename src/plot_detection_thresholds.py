...
def build_wd_data(element, DA=True):
    if DA:
        ...
        wd_data["DA"], wd_upper_bounds["DA"] = mwdd.get_mwdd_abundances(
            element, ci.Element.H, True, True
        )
    else:
        wd_data = {"DB": list(), "DA": None}
        wd_upper_bounds = {"DB": list(), "DA": None}
        # wd_data['DB'], wd_upper_bounds['DB'] = ha.get_hollands_abundances_for_dt_plot(
        #     element
        # )
        wd_data["DB"], wd_upper_bounds["DB"] = mwdd.get_mwdd_abundances(
            element, ci.Element.He, True, True
        )
...
def plot_detection_thresholds(DA=True):
    ...
    for i, element in enumerate(elements_to_plot):
        ...
        new_plot = graph_fac.make_detection_thresholds_plot(
            to_plot,
            element,
            wd_data,
            wd_upper_bounds,
