...
def load_points_files(file_prefix, output_dir="output"):
    ...
    model_name = file_prefix.split("_lp")[0]
    # ^ Assume _lp doesn't appear in the model name...
    live_points = int(file_prefix.split("_lp")[1].split("_")[0])
    param_list = list()
    if model_name.startswith("Hierarchy"):
        hierarchy_name = model_name.split("_levels")[0]
        # ^ Assume _levels or levels_ doesn't appear in the hierarchy name...
...
def plot_ellipses(
    file_prefix, active_points, rejected_points, sorted_param_list, live_points
):
    param_index_1 = 0
    param_index_2 = 0
                    ...
                    "LastRejected": {
                        ...
                        "series": {
                            "last_rejected": {
                                ...
                                "z_data": rejected_points[
                                    -live_points:, len(sorted_param_list)
                                ],
                                "fill": False,
                                "legend": False,
                                "cbar_label": "log(Z)",
...
def plot_system(file_prefix):
    active_points, rejected_points, sorted_param_list, live_points = load_points_files(
        file_prefix
    )
...
def main():
    # progress_plotter_nonlive(
    #     'Hierarchy_Default_levels_0_lp1000_obs95_phys_live.points',
    #     4
    # )
