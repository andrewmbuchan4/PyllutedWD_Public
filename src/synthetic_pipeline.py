...
class Pipeline:
    ...
    def __init__(self, name, population_parameter_dict, observer_dict, modeller_dict):
        ...
        self.manager = mn.Manager()
        # ^ Load manager once here, and send it to hollands_abundances, rather than load
    ...
    def create_mv_plots(
        ...
        # For this to be remotely practical, assume that the modeller name is always the
        # same, and we're only interested in 'Modelled' output
        # (Unless populations are the same!)
        # Let's require that the obs names we are interested in should all end in
        # errxpyz which is parsed as error = x.yz dex
        ...
        # Also assume that we change error size OR threshold offsets, not both.
        # Need to detect which mode we're using:
        ...
        for N in N_values:
            for et in errors_or_thresholds:
                ...
                if not pool:
                    ...
                    # Then assume we are being directed to resample a pop of size N from
                    # a larger pool
                    # Assuming this becomes the standard protocol, can assume that the
                    # populations will no longer be named ...Nxyz
                    ...
                    if pop2_name.startswith("Hollands"):
                        ...
                        hollands_data = ha.get_hollands_common_el_values(
                            el_list_to_compare
                        )
                        mv_results_dict, mv_distinguishable_dict = (
                            self.run_mv_tests_against_comparison(
                                mv_combination1,
                                "Hollands",
                                hollands_data,
                                resample_count,
                        ...
                        mv_combination1 = (modeller, obs_name, pop1_name, "Observed", N)
                        mv_combination2 = (modeller, obs_name, pop2_name, "Observed", N)
                        # ^ This is sort of assuming we will have the same observer for
                        # each...
                        # el_list_to_compare = [
                        #     ci.Element.Ca,
                        #     ci.Element.Mg,
                        #     ci.Element.Fe
                        # ] # This should be exposed at a higher level
                        # el_list_to_compare = [
                        #     ci.Element.C,
                        #     ci.Element.O,
                        #     ci.Element.Si
                        ...
                        mv_results_dict, mv_distinguishable_dict = (
                            self.run_mv_tests_between_populations(
                                mv_combination1,
                                mv_combination2,
                                resample_count,
                                most_polluted,
                ...
                for test_type in mv.get_mv_test_method_dict():
                    if mv_distinguishable_dict[test_type][1] > 0:
                        mv_dprob_values_dict[test_type][N].append(
                            mv_distinguishable_dict[test_type][0]
                            / mv_distinguishable_dict[test_type][1]
                        )
                        mv_dprob_values_dict_v_N[test_type][et].append(
                            mv_distinguishable_dict[test_type][0]
                            / mv_distinguishable_dict[test_type][1]
                        )
                    else:
        ...
        y_label = (
            "P(Input != Output)"
            if comparison_et_value is None
            else "P(Output != No Noise)"
        )
        graph_fac.make_dprob_v_N_plot(
            N_values,
            mv_dprob_values_dict_v_N,
            None,
            pop1_output_name,
            pop2_output_name,
            y_label,
        ...
        graph_fac.make_dprob_v_error_plot(
            errors_or_thresholds,
            mv_dprob_values_dict,
            None,
            pop1_output_name,
            pop2_output_name,
            error_mode,
        ...
        graph_fac.make_ks_v_error_plot(
            errors_or_thresholds,
            mv_p_values_dict,
            self.p_threshold,
            None,
    ...
    def run_mv_tests_between_populations(
        self,
        combination1,
        combination2,
        resample_count=1,
        ...
        # 1: obs_name
        # 2: pop_name
        # 3: variable_type ('Input', 'Pollution', 'Observed', 'Modelled')
        # 4: population size (can be omitted)
        print(combination1)
        print(combination2)
        normalise_abundance_values = True

        pop1_including_unobserved = self.results_dict[combination1[0]][combination1[1]][
            combination1[2]
        ]
        pop2_including_unobserved = self.results_dict[combination2[0]][combination2[1]][
            combination2[2]
        ]
        if filter_on_elements is not None:
            # Take all systems that are have detections of the requested elements
            pop1 = pop1_including_unobserved.get_subset_with_detected_elements(
                filter_on_elements
            )
            pop2 = pop2_including_unobserved.get_subset_with_detected_elements(
                filter_on_elements
            )
        else:
        ...
        #     comparison_values_normalised = self.normalise_abundance_lists(
        #        comparison_values
        #     )
        ...
        while number_of_times_resampled < resample_count:
            if most_polluted is not None:
                # Then we will use (as our 1 sample) the most polluted white dwarfs
            ...
            if pop1_sample is not None and pop2_sample is not None:
                if normalise_abundance_values:
                    sample1_values_normalised = self.normalise_abundance_lists(
                        pop1_sample.get_common_el_values(filter_on_elements)
                    )
                    sample2_values_normalised = self.normalise_abundance_lists(
                        pop2_sample.get_common_el_values(filter_on_elements)
                    )
                    ...
                    sample1_values = pop1_sample.get_common_el_values(
                        filter_on_elements
                    )
                    sample2_values = pop2_sample.get_common_el_values(
                        filter_on_elements
                    )
                ...
                break
        distinguishable_dict = self.report_mv_test_results(
            combination1, combination2, pop1_size, pop2_size, results_dict, True
        )
    ...
    def run_mv_tests_against_comparison(
        self,
        combination,
        comparison_name,
        comparison_values,
        ...
        # 1: obs_name
        # 2: pop_name
        # 3: variable_type ('Input', 'Pollution', 'Observed', 'Model
        # 4: population size (can be omitted)

        normalise_abundance_values = True

        pop_including_unobserved = self.results_dict[combination[0]][combination[1]][
            combination[2]
        ]
        if filter_on_elements is not None:
            # Take all systems that are have detections of the requested elements
            pop = pop_including_unobserved.get_subset_with_detected_elements(
                filter_on_elements
            )
        else:
            # Take all systems that are actually observed:
        ...
        if normalise_abundance_values:
            ...
            data_set1_r = mv.convert_data_to_r_format(comparison_values)
        max_no_ternary_series = 2
        ternary_abundance_dict = {"Real": dict()}
        for i, el in enumerate(filter_on_elements):
            if normalise_abundance_values:
                ternary_abundance_dict["Real"][el] = comparison_values_normalised[i]
            else:
        ...
        while number_of_times_resampled < resample_count:
            ...
            if pop_sample is not None:
                if normalise_abundance_values:
                    sample_values_normalised = self.normalise_abundance_lists(
                        pop_sample.get_common_el_values(filter_on_elements)
                    )
                    assert len(comparison_values_normalised[0]) == len(
                        sample_values_normalised[0]
                    )
                    # ^ Just a quick sanity check in case I forget to set the synthetic
                    # sample size to match the real sample size
                    ...
                    assert len(comparison_values[0]) == len(sample_values[0])
                    # ^ Just a quick sanity check in case I forget to set the synthetic
                    # sample size to match the real sample size
                ...
                if len(ternary_abundance_dict.keys()) < max_no_ternary_series:
                    ...
                    for i, el in enumerate(filter_on_elements):
                        if normalise_abundance_values:
                            ternary_abundance_dict["Synthetic"][el] = (
                                sample_values_normalised[i]
                            )
                        else:
        ...
        if len(filter_on_elements) == 3:
            graph_fac = gf.GraphFactory(
                self.get_pipeline_dir(combination[2], combination[1], combination[0])
            )
            graph_fac.make_ternary_plot(
                filter_on_elements,
                ternary_abundance_dict,
    ...
    def normalise_abundance_lists(self, abundance_lists):
        ...
        number_of_data_vectors = len(abundance_lists[0])
        # ^ Hopefully this is the same for all the lists
    ...
    def create_dprob_ks_v_error_plot(
        self,
        base_pop1_name,
        base_pop2_name,
        errors,
        ...
        ks_values_dict = dict()
        # Also assume that we change error size OR threshold offsets, not both. Need to
            ...
            for et in errors_or_thresholds:
                ...
                if not pool:
                    ...
                    if pop2_name.startswith("Hollands"):
                        ...
                        ks_results, distinguishable_count, total_count = (
                            self.run_ks_test_against_comparison(
                                ks_combination1,
                                "Hollands",
                                ha.get_hollands_el_el_values(el1, el2, self.manager),
                                variable,
                                plot_descriptions[variable],
                                most_polluted,
                                filter_on_elements,
                            )
                        )
                        ...
                        ks_results, distinguishable_count, total_count = (
                            self.run_ks_test_against_comparison(
                                ks_combination1,
                                "40pc",
                                fp.get_40pc_el_el_values(el1, el2, self.manager),
                                variable,
                                plot_descriptions[variable],
                                most_polluted,
                                filter_on_elements,
                        ...
                        # Then we're being told to compare all outputs to the output of
                        # one specific run
                        ...
                        ks_results, distinguishable_count, total_count = (
                            self.run_ks_test_on_results(
                                ks_combination1,
                                ks_combination2,
                                variable,
                                plot_descriptions[variable],
                                element_pair_to_test,
                            )
                        )
                    else:
                        if use_modelled_output:
                            ks_combination1 = (
                                (modeller, obs_name, pop1_name, "Modelled")
                                if pop1_name != pop2_name
                                else (modeller, obs_name, pop1_name, "Input")
                            )
                            ks_combination2 = (
                                modeller,
                                obs_name,
                            ...
                            ks_combination1 = (
                                (modeller, obs_name, pop1_name, "Observed")
                                if pop1_name != pop2_name
                                else (modeller, obs_name, pop1_name, "Pollution")
                            )
                            ks_combination2 = (
                                modeller,
                                obs_name,
                    ...
                    if pop2_name.startswith("Hollands"):
                        ...
                        ks_results, distinguishable_count, total_count = (
                            self.run_ks_test_against_comparison(
                                ks_combination1,
                                "40pc",
                                fp.get_40pc_el_el_values(el1, el2),
                                variable,
                                plot_descriptions[variable],
                                resample_count,
                                most_polluted,
                        ...
                        if use_modelled_output:
                            ks_combination1 = (
                                (modeller, obs_name, pop1_name, "Modelled", N)
                                if pop1_name != pop2_name
                                else (modeller, obs_name, pop1_name, "Input", N)
                            )
                            ks_combination2 = (
                                modeller,
                                obs_name,
                            ...
                            ks_combination1 = (
                                (modeller, obs_name, pop1_name, "Observed", N)
                                if pop1_name != pop2_name
                                else (modeller, obs_name, pop1_name, "Pollution", N)
                            )
                            ks_combination2 = (
                                modeller,
                                obs_name,
        ...
        graph_fac.make_dprob_v_error_plot(
            errors_or_thresholds,
            dprob_values_dict,
            plot_descriptions[variable][0][0],
            pop1_output_name,
            pop2_output_name,
            error_mode,
            y_label,
    ...
    def create_dprob_ks_v_N_plot(
        ...
        for error in errors:  # can generalise to errors or thresholds later!
            ...
            for N in N_values:
                # Assume that we are always resampling a pop of size N from a larger
                # pool
                if use_modelled_output:
                    ks_combination1 = (
                        (modeller, obs_name, base_pop1_name, "Modelled", N)
                        if base_pop1_name != base_pop2_name
                        else (modeller, obs_name, base_pop1_name, "Input", N)
                    )
                    ks_combination2 = (
                        modeller,
                        obs_name,
                    ...
                    ks_combination1 = (
                        (modeller, obs_name, base_pop1_name, "Observed", N)
                        if base_pop1_name != base_pop2_name
                        else (modeller, obs_name, base_pop1_name, "Pollution", N)
                    )
                    ks_combination2 = (
                        modeller,
                        obs_name,
        ...
        if len(N_values) > 1 and len(errors) > 1:
            graph_fac.make_dprob_N_error_heatmap(
                N_values,
                errors,
                dprob_grid,
                plot_descriptions[variable][0][0],
    ...
    def create_dprob_ks_v_N_multipop_plot(
        self,
        base_pop1_name,
        base_pop2_name,
        base_obs1_name,
        ...
        for error in errors:  # can generalise to errors or thresholds later!
            ...
            for N in N_values:
                # Assume that we are always resampling a pop of size N from a larger
                # pool
                ks_combination_pop1_1 = (
                    (modeller1, obs1_name, base_pop1_name, "Input", N)
                    if use_modelled_output
                    else (modeller1, obs1_name, base_pop1_name, "Pollution", N)
                )
                ks_combination_pop1_2 = (
                    (modeller1, obs1_name, base_pop1_name, "Modelled", N)
                    if use_modelled_output
                    else (modeller1, obs1_name, base_pop1_name, "Observed", N)
                )
                ks_results_pop1, distinguishable_count_pop1, total_count_pop1 = (
                    self.run_ks_test_on_results(
                        ks_combination_pop1_1,
                        ks_combination_pop1_2,
                        variable,
                ...
                dprob_values_dict[pop1_key][error].append(
                    distinguishable_count_pop1 / total_count_pop1
                )
                ...
                ks_combination_pop2_1 = (
                    (modeller2, obs2_name, base_pop2_name, "Input", pop2_sample_size)
                    if use_modelled_output
                    else (
                        modeller2,
                        obs2_name,
                ...
                ks_combination_pop2_2 = (
                    (modeller2, obs2_name, base_pop2_name, "Modelled", pop2_sample_size)
                    if use_modelled_output
                    else (
                        modeller2,
                        obs2_name,
                ...
                ks_results_pop2, distinguishable_count_pop2, total_count_pop2 = (
                    self.run_ks_test_on_results(
                        ks_combination_pop2_1,
                        ks_combination_pop2_2,
                        variable,
                ...
                dprob_values_dict[pop2_key][error].append(
                    distinguishable_count_pop2 / total_count_pop2
                )
                ks_values_dict[pop2_key][error].append(ks_results_pop2)
                N_values_dict[pop2_key][error].append(pop2_sample_size)
        for error in errors:
            graph_fac.make_dprob_v_N_multipop_plot(
                N_values_dict,
                dprob_values_dict,
                plot_descriptions[variable][0][0],
                pop1_key,
            ...
            graph_fac.make_ks_v_N_multipop_plot(
                N_values_dict,
                ks_values_dict,
                self.p_threshold,
                plot_descriptions[variable][0][0],
    ...
    def run_ks_test_on_samples(
        ...
        max_bin_edge,
        half_bin_size,
        precalculated_weights1=None,
        precalculated_weights2=None,
    ):
        n1 = len(data1)
        n2 = len(data2)
    ...
    def convert_sample_to_cdf(self, data, bins, precalculated_weights=None):
        ...
        if precalculated_weights is not None:
            # Assume data and precalculated_weights are already the vals and
            # weights_to_use we need
            vals = data
            weights_to_use = precalculated_weights
        else:
            for val in data:
                # Expect each item to be a list. Need to collapse this list of lists
                # into a single list, but keep track of the weight each should be
                # assigned (eg if a system has 3 possible output values, they need to
                # each by weighted by 1/3 to avoid distorting the histogram)
        ...
        heights, bins_ignore = np.histogram(
            vals, bins, weights=weights_to_use, density=False
        )
        cdf = np.cumsum(heights)
        # ^ Ideally we would avoid this by setting density to True, but when I tested
        # that, it sometimes weighted things incorrectly
    ...
    def run_ks_test_on_cdf(self, cdf1, cdf2, n1, n2):
        ...
        # From 1.3.0 onwards, it only uses the approximation for large samples
        # ( > 10,000) and calculates an exact value for smaller samples.
        # One of the tests (test_ks_tests) checks the output of run_ks_test_on_cdf
        # against the output of ks_2samp (on a small sample).
        # Therefore, if the installed version of scipy is >= 1.3.0, there will be a
        # discrepancy between run_ks_test_on_cdf and ks_2samp, because
        # run_ks_test_on_cdf will calculate an approximate value and ks_2samp will
        # calculate the exact value.
        # The release notes (https://docs.scipy.org/doc/scipy/release/1.3.0-notes.html)
        # claim that calling ks_2samp with alternative="two-sided", method="asymp"
        # should replicate the old, approximate method.
        # But when I tried this, there was still a discrepancy. I imagine probably
        # caused by different internal cdf conversion logic?
        # (also note there is a separate bug in ks_2samp for versions between 1.3.0 and
        # 1.5.0 which results in wildly incorrect p-values sometimes - avoid these
        # versions!!!)
        ...
        # So the far bigger caveat to these tests is the arbitrary significance
        # threshold.
        # If anything, overestimating the p-values is conservative.
        # And when the samples are actually from the same dist, ks_samp does seem to
        # sometimes produce p-values that look too low by eye.

        # For future reference: the reason I didn't just use ks_2samp in the first place
        # is that it takes, as input, 2 samples of data, and it is not expecting any of
        # these data points to be multi-valued.
        # But various outputs from the synthetic modeller are lists, e.g. if there are
        # multiple best fit nebular compositions, the inferred Fe/H index is a list of
        # all the best fit indices, assumed to be equally likely.
        # So I dug into the source code and realised that it firstly converts to cdf,
        # then performs the ks test on the cdf.
        # So my solution was to write some logic to convert my outputs to a cdf, then
        # duplicate the cdf-based ks test code from scipy.
        if n1 == 0 or n2 == 0:
            return np.nan, np.nan
        d = np.max(np.absolute(cdf1 - cdf2))
        en = np.sqrt(n1 * n2 / (n1 + n2))
        prob = st.distributions.kstwobign.sf((en + 0.12 + 0.11 / en) * d)
        # The above line looks odd. But see
        # https://stats.stackexchange.com/questions/264445/
        # constants-in-scipy-implementation-of-kolmogorov-smirnov-two-sample-test
    ...
    def find_sample_size_for_given_distinguishability_probability(
        self, combination1, combination2, variable, plot_description, p_threshold=None
    ):
        # combinations should be a tuple containing:
        # 0: mod_name
        # 1: obs_name
        ...
        pop1 = pop1_including_unobserved.get_subset_with_modelled_properties(
            [mp.ModelParameter.fragment_core_frac]
        )  # we only care about systems which have an fcf estimate
        pop2 = pop2_including_unobserved.get_subset_with_modelled_properties(
            [mp.ModelParameter.fragment_core_frac]
        )

        sample_size = 5
        # ^ For now, to keep simple, I'll just increase this until P goes above 90%
        ...
        while True:
            ...
            while number_of_times_sampled < max_number_of_times_to_resample:
                ...
                if (
                    combination1[0] == combination2[0]
                    and combination1[1] == combination2[1]
                    and combination1[2] == combination2[2]
                ):
                    # Then they're actually the same population and we're comparing
                    # input and output, so the sample should be the same
                    pop2_sample = pop1_sample
                else:
                    pop2_sample = pop2.get_random_subset(sample_size, True)
                if pop1_sample is not None and pop2_sample is not None:
                    variable_dict1, weights_dict1, variable_below1, variable_above1 = (
                        self.extract_plottables(
                            combination1[2], pop1_sample, {variable: plot_description}
                        )
                    )
                    variable_dict2, weights_dict2, variable_below2, variable_above2 = (
                        self.extract_plottables(
                            combination2[2], pop2_sample, {variable: plot_description}
                        )
                    )
                    ...
                    # NB sample1 and sample 2 could be longer than sample_size if a
                    # system has multiple multiple values, should probably rather check
                    # the sum of weights
                    assert abs(sum(weights1) - sample_size) < 0.00001
                    ...
                    result = self.run_ks_test_on_samples(
                        sample1,
                        sample2,
                        min_bin_edge,
                        max_bin_edge,
                    ...
                    # This will occur when we run out of WDs to sample (and the
                    # get_random_subset function returns None). We now want to break ou
                    # of the loop and report the results
                    break
            distinguishable_count, total_count = self.report_ks_test_results(
                combination1,
                combination2,
                variable,
                sample_size,
            ...
            if (
                distinguishable_count / total_count
                > distinguishability_probability_threshold
            ):
                break
            else:
                # Some logic to increase the sample size more if we're really far away
                # from making the threshold
                # sample_size /= 0.48*(
                #     np.tanh(((distinguishable_count/total_count) * 6) - 3) + 1.05
                # )
        ...
        pop2.reset_unsampled_indices()
        return sample_size
        # ^ Note this will always be an overestimate (by up to ~10%), but it's non-
        # deterministic (and boundary can be inherently fuzzy) so we're only in the game
        # of making v rough estimates anyway. I just round down to a roughly round
        # number

    def run_ks_test_against_comparison(
        self,
        combination,
        comparison_name,
        comparison_values,
        ...
        # mv_tests affects the functionality massively and should maybe not have a
        # default value? If True, it doesn't actually run a ks test but instead pulls
        # out "observed" Ca, Mg and Fe data and runs multivariate statistical tests on
        # those
        pop_including_unobserved = self.results_dict[combination[0]][combination[1]][
            combination[2]
        ]
        ...
        while number_of_times_resampled < resample_count:
            if most_polluted is not None:
                ...
                pop_sample = pop.get_random_subset(pop_size, True)
            if pop_sample is not None:
                variable_dict, weights_dict, variable_below, variable_above = (
                    self.extract_plottables(
                        combination[2], pop_sample, {variable: plot_description}
                    )
                )
    ...
    def run_ks_test_on_results(
        self, combination1, combination2, variable, plot_description, resample_count=1
    ):
        # combinations should be a tuple containing:
        # 0: mod_name
        # 1: obs_name
        ...
        # Need to pick out the right subset - bear in mind that extract_plottables can
        # return a smaller subset of values than expected if it receives systems that it
        # shouldn't. This is bad (it throws off the KS test)
        if combination1[3] == "Modelled":
            pop1 = pop1_including_unobserved.get_subset_with_modelled_properties(
                [plot_description[0][0]]
            )
            ...
            # Because all systems have pollution - whether that pollution is observed is
            # irrelevant in this case
            pop1 = pop1_including_unobserved.get_duplicate_population()
        else:
            raise NotImplementedError(
        ...
        while number_of_times_resampled < resample_count:
            ...
            if (
                ...
                # Then they're actually the same population and we're comparing input
                # and output, so the sample should be the same
                pop2_sample = pop1_sample
            else:
                pop2_sample = pop2.get_random_subset(pop2_size, True)
            ...
            if pop1_sample is not None and pop2_sample is not None:
                ...
                # This will occur when we run out of WDs to sample (and the
                # get_random_subset function returns None). We now want to break out of
                # the loop and report the results
                break
        distinguishable_count, total_count = self.report_ks_test_results(
            combination1,
            combination2,
            ...
            for result in results_dict[test_type]:
                ...
                if result is not None and not np.isnan(result):
                    if result > self.p_threshold:
                        ...
                        to_write.append(
                            "Samples appear to be from different distributions"
                        )
                        distinguishable_count += 1
                    total_count += 1
                else:
    ...
    def generate_populations(self, population_parameter_dict, obs_name, mod_name):
        ...
        default_min_mdot_dict = {
            # By default, there will be no filter on which sets of parameters we forward
            # model. Potentially inefficient - could throw away anything with negligible
            # chance of being detectable (by adding a key to the dictionary in detection
            # thresholds)
            "DA": {"gradient": 0, "yintercept": -np.inf, "leeway": 0},
            "DB": {"gradient": 0, "yintercept": -np.inf, "leeway": 0},
        }
        for pop_name, params_dict in population_parameter_dict.items():
            # 3 cases: Never seen this combo before, seen pop_name before but not
            # one/both of obs and mod, seen exact combo before.
            # In the 1st and 3rd cases, load up with no premade pop - in the 1st case it
            # should realise it's seen this before and load the right pop
            # In the 2nd case, need to be careful. We have got pop data saved but must
            ...
            if not os.path.exists(file_to_look_for) and pop_name in self.results_dict:
                ...
                premade_pop = self.results_dict[pop_name][dummy_obs][
                    dummy_mod
                ].extract_raw_population()
            ...
            if "DA" in params_dict[sp.PopulationParameter.wd_config]:
                ...
                # In this case, need to just rename the wd config so we know what mdot
                # bank to look in
            ...
            # toret[pop_name].check_sinking_timescales()
            # ^ This is an optional diagnostic (time consuming!)
        return toret
    ...
    def plot_results(
        ...
        # mwdd_hx refers to the reference element, i.e. either ci.Element.H or
        # ci.Element.He
        # Warning: if this is supplied, some of the plots will then implicitly assume
        # that all the white dwarfs in the synthetic pops are DAs/DBs respectively
        # I anticipate we probably don't want to do every combo, so comment stuff out!

        # most_polluted indicates whether to only pull out the most heavily polluted WDs
        # for analysis (in which case most_polluted should be an (int, element) tuple
        # indicating how many to pull out and what element to use to determine pollution
        # level. Can be None if using all elements)
        # filter_on_elements is a list of elements (or None). If not None, we only pick
        # out WDs for analysis with detections of the specified elements.
            ...
            for obs_name, results in mod_results.items():
                ...
                for pop_name, pop in results.items():
                    (
                        variable_dict,
                        weights_dict,
                        variable_below_threshold_dict,
                        variable_above_threshold_dict,
                    ) = self.extract_plottables(
                        pop_name,
                        pop,
                        plot_descriptions,
                    ...
                    model_analyser = ma.ModelAnalyser(
                        self.get_pipeline_dir(pop_name, obs_name, mod_name)
                    )
                    individual_plots_for_multipanel = (
                        self.plot_individual_population_variables(
                            model_analyser,
                            plot_descriptions,
                            variable_dict,
                            weights_dict,
                    ...
                    self.plot_multipanels(
                        model_analyser,
                        individual_plots_for_multipanel,
                        plot_descriptions,
                    )
                    io_comparison_plots_dict[mod_name][obs_name][pop_name] = (
                        self.plot_individual_population_io_comparison(
                            model_analyser,
                            pop_name,
                            plot_descriptions,
                            variable_dict,
                    ...
                    self.plot_individual_population_io_scatter(
                        model_analyser,
                        pop,
                        pop_name,
                        plot_descriptions,
                        add_excluded_systems_to_scatter_plot,
                        mwdd_hx,
                    )
    ...
    def perform_knn_comparisons(self):
        ...
        for mod_name, mod_results in self.results_dict.items():
            for obs_name, results in mod_results.items():
                for pop_name, pop in results.items():
                    relevant_subset = pop.get_subset_with_detected_elements(
                        [ci.Element.Ca, ci.Element.Mg, ci.Element.Fe]
                    )
                    random_subset = relevant_subset.get_random_subset(
                        len(hollands_pop), True
                    )
                    correct, incorrect, p_value = (
                        random_subset.knn_comparison_against_other_pop(hollands_pop)
                    )
                    print()
                    print(pop_name)
                    print(obs_name)
    ...
    def plot_ternary_comparisons(
        self,
        most_polluted_tuple=None,
        comparison_pop_name="Hollands",
        comparison_elements=[ci.Element.Ca, ci.Element.Mg, ci.Element.Fe],
    ):
            ...
            for obs_name, results in mod_results.items():
                for pop_name, pop in results.items():
                    ...
                    if most_polluted_tuple is not None:
                        ...
                        pop_to_use = pop.get_most_polluted_subset(
                            pop_size, element_to_use
                        )
                    else:
                        pop_to_use = pop
                    for wd in pop_to_use:
                        ...
                        if all_elements_observed:
                            for element in el_list:
                                el_abundance_dict[pop_name][element].append(
                                    10 ** wd.observed_abundances[element]
                                )
                    # Now add in comparison abundances
                    el_abundance_dict[comparison_pop_name] = dict()
                    if comparison_pop_name == "Hollands":
                        comparison_values = ha.get_hollands_common_el_values(
                            comparison_elements, manager_for_hollands_abundances
                        )
                    elif comparison_pop_name == "PEWDD_thermohaline":
                        comparison_values = pv.get_common_thermohaline_el_values(
                            comparison_elements
                        )
                    else:
                        pass
                    for i, element in enumerate(el_list):
                        el_abundance_dict[comparison_pop_name][element] = [
                            10**hav for hav in comparison_values[i]
                        ]
                        # TODO Add some logic here that more elegantly switches to
                        # different abundances as a function of comparison_pop_name
                        # if comparison_pop_name == 'Hollands':
                        #     el_abundance_dict[comparison_pop_name][element] = [
                        #         10**hav for hav in ha.get_hollands_abundance_values(
                        #             element, manager_for_hollands_abundances
                        #         )
                        #     ]
                        # elif comparison_pop_name == 'PEWDD_thermohaline':
                        #    el_abundance_dict[comparison_pop_name][element] = [
                        #        10**av for av in pt.get_abundance_values(
                        #            element, manager_for_hollands_abundances
                        #         )
                        #     ]
                        # else:
                    ...
                    if run_mv_tests:
                        synthetic_data_for_mv_test = [
                            el_abundance_dict[pop_name][c_el]
                            for c_el in comparison_elements
                        ]
                        # synthetic_data_for_mv_test = [
                        #    el_abundance_dict[pop_name][ci.Element.Ca],
                        #    el_abundance_dict[pop_name][ci.Element.Mg],
                        #    el_abundance_dict[pop_name][ci.Element.Fe]
                        # ]
                        # Things to check: does it matter if I just use 2 of these (as
                        # long as its normalised), does the scaling matter, does the
                        # test type matter
                        # Also, these need to be normalised anyway: we only care about
                        # the relative quantities surely? Maybe this is another thing to
                        # test
                        hollands_data_for_mv_test = [
                            el_abundance_dict[comparison_pop_name][c_el]
                            for c_el in comparison_elements
                        ]
                        synthetic_converted = mv.convert_data_to_r_format(
                            synthetic_data_for_mv_test
                        )
                        hollands_converted = mv.convert_data_to_r_format(
                            hollands_data_for_mv_test
                        )
                        ...
                        p_value_cramer = mv.cramer_test_p_value(
                            synthetic_converted, hollands_converted
                        )
                        print("Done")
                        # print('Performing Fasano Franceschini test...')
                        # p_value_fasano_franceschini = (
                        #     mv.fasano_franceschini_test_p_value(
                        #         synthetic_converted,
                        #         hollands_converted
                        #     )
                        # )
                        ...
                        additional_text_dict = {
                            "Cramér": {
                                "x_pos": -0.08,
                                "y_pos": 0.87,
                                "text_string": "Cramér p-value: "
                                + str(round(p_value_cramer, 7)),
                            }
                            # 'FF': {
                            #    'x_pos': -0.08,
                            #    'y_pos': 0.82,
                            #    'text_string': (
                            #        'FF p-value: '
                            #        + str(round(p_value_fasano_franceschini, 7))
                            #    )
                            # }
                        }
                    graph_fac.make_ternary_plot(
    ...
    def plot_specific_io_comparisons(self, io_comparison_plots_dict):
        ...
        if activate_fcf_comparison:
            ...
            io_comparison_plots_dict["StandardModeller"]["RealisticObservererr0"][
                "DATidal"
            ]["fcf"]["pdf"]["hist_plot"]["subplots"]["subplot1"]["series"]["test"] = {
            ...
            io_comparison_plots_dict["StandardModeller"]["RealisticObservererr0p2"][
                "DATidal"
            ]["fcf"]["pdf"]["hist_plot"]["subplots"]["subplot1"]["series"]["test"] = {
                "type": dp.PlotType.text,
                "x_pos": 0.5,
            ...
            io_comparison_plots_dict["StandardModeller"]["RealisticObservererr0p2"][
                "DACollisional"
            ]["fcf"]["pdf"]["hist_plot"]["subplots"]["subplot1"]["x_hide_ticks"] = [0]
            io_comparison_plots_dict["StandardModeller"]["RealisticObservererr0"][
                "DATidal"
            ]["fcf"]["pdf"]["hist_plot"]["subplots"]["subplot1"][
                "legend_text_size"
            ] = 26
            io_comparison_plots_dict["StandardModeller"]["RealisticObservererr0p2"][
                "DATidal"
            ]["fcf"]["pdf"]["hist_plot"]["subplots"]["subplot1"]["legend"] = False
            io_comparison_plots_dict["StandardModeller"]["RealisticObservererr0"][
                "DACollisional"
            ]["fcf"]["pdf"]["hist_plot"]["subplots"]["subplot1"]["legend"] = False
            io_comparison_plots_dict["StandardModeller"]["RealisticObservererr0p2"][
            ...
            graph_fac.multipanelise(
                ...
                2,
                2,
                ["tidalvcollisional_multipanel.pdf"],
                15,
                18,
                0,
                0,
        ...
        if activate_strategy_comparison:
            combined_plot = io_comparison_plots_dict["StandardModeller"][
                "ShortObservation"
            ]["DAControlPopLarge"]["fcf"]["pdf"]
            combined_plot["hist_plot"]["filenames"] = ["strategy_io_comparisons.pdf"]
            combined_plot["hist_plot"]["subplots"]["subplot1"]["y_max"] = max(
                combined_plot["hist_plot"]["subplots"]["subplot1"]["y_max"],
                io_comparison_plots_dict["StandardModeller"]["LongObservation"][
                    "DAControlPopSmall"
                ]["fcf"]["pdf"]["hist_plot"]["subplots"]["subplot1"]["y_max"],
            )
            combined_plot["hist_plot"]["subplots"]["subplot1"]["legend_text_size"] = 11
            combined_plot["hist_plot"]["subplots"]["subplot1"]["series"][
                "Large Sample Initial"
            ] = combined_plot["hist_plot"]["subplots"]["subplot1"]["series"].pop(
                "Initial"
            )
            combined_plot["hist_plot"]["subplots"]["subplot1"]["series"][
                "Large Sample Final"
            ...
            for plot_name, description in plot_descriptions.items():
                ...
                if vl in ["Input", "Pollution"]:
                    ...
                    if filter_on_elements is None:
                        ...
                        # filter_on_elements should be a list of elements. We only want
                        # the subset where those elements are detected
                        population = pop.get_subset_with_detected_elements(
                            filter_on_elements
                        )
                    if most_polluted is not None:
                        pop_size = most_polluted[0]
                        element_to_use = most_polluted[1]
                        population = population.get_most_polluted_subset(
                            pop_size, element_to_use
                        )
                if vl == "Input":
                    ...
                    if len(vfilter[0]) == 2:
                        # Then we're dealing with a tuple of 2 elements, and we want to
                        # return the ratio
                        prefilter = population.pollution_abundance_log_ratios(
                            vfilter[0][0], vfilter[0][1]
                        )
                    else:
                    ...
                    if len(vfilter[0]) == 2:
                        # Then we're dealing with a tuple of 2 elements, and we want to
                        # return the ratio
                        prefilter = population.observed_abundance_log_ratios(
                            vfilter[0][0], vfilter[0][1]
                        )
                    else:
                    ...
                    for i, rmv in enumerate(raw_modelled_values):
                        # Expect each item to be a list. Need to collapse this list of
                        # lists into a single list, but keep track of the weight each
                        # should be assigned
                        # (eg If a system has 3 possible output values, they need to
                        # each be weighted by 1/3 to avoid distorting the histogram)
                        weight = 1 / len(rmv)
                        added_weight = list()
                        for entry in rmv:
                ...
                for i, val in enumerate(prefilter):
                    try:
                        if val is not None and ~np.isnan(val):
                            ...
                            if val < vfilter[1]:
                                variable_below_threshold_dict[vl][plot_name][
                                    pop_name
                                ] += weights[i]
                                print(val)
                                print(vfilter[1])
    ...
    def plot_individual_population_io_comparison(
        self,
        model_analyser,
        pop_name,
        plot_descriptions,
            ...
            for pop_name, values in variable_dict["Input"][plot_name].items():
                ...
                if not paper_version:
                    io_text_dict[f"{pop_name} Initial excluded"] = {
                        ...
                        + " below, "
                        + str(
                            variable_above_threshold_dict["Input"][plot_name][pop_name]
                        )
                        + " above",
            ...
            for pop_name, values in variable_dict["Modelled"][plot_name].items():
                ...
                if not paper_version:
                    io_text_dict[f"{pop_name} Final excluded"] = {
                        ...
                        + " total, "
                        + str(
                            variable_below_threshold_dict["Modelled"][plot_name][
                                pop_name
                            ]
                        )
                        + " below, "
                        + str(
                            variable_above_threshold_dict["Modelled"][plot_name][
                                pop_name
                            ]
                        )
                        + " above",
    ...
    def plot_individual_population_io_scatter(
        self,
        model_analyser,
        population,
        pop_name,
        ...
        for plot_name, plot_description in plot_descriptions.items():
            ...
            modelled_input_vals, modelled_output_vals, unmodelled_input_vals = (
                observed_pop.modelled_io_pairs(vfilter[0])
            )
            bins, x_bar_centres = model_analyser.generate_bins_and_bar_centres(
                vfilter[1], vfilter[2], vfilter[3]
            )
            heights, bins2 = np.histogram(unmodelled_input_vals, bins, density=False)
            io_scatter_plot = model_analyser.graph_fac.make_io_scatter_plot(
                modelled_input_vals,
        ...
        if plot_elel_scatter:
            ...
            # dummy_observer = so.Observer(
            #     so.ObservationType.TeffIndividualElementCutoff
            # )
            # spectral_type = 'DA' if hx == ci.Element.H else 'DB'
            # teff = 5000
            # el1_threshold = dummy_observer.calculate_element_cutoff_threshold(
            #     el1,
            #     spectral_type,
            #     teff
            # )
            # el2_threshold = dummy_observer.calculate_element_cutoff_threshold(
            #     el2,
            #     spectral_type,
            #     teff
            # )
            ...
            io_scatter_plot = model_analyser.graph_fac.make_elel_scatter_plot(
                el1_values,
                el2_values,
    ...
    def plot_individual_population_variables(
        self,
        model_analyser,
        plot_descriptions,
        variable_dict,
        ...
        for vl in variable_list:
            ...
            for plot_name, description in plot_descriptions.items():
                ...
                if isinstance(vfilter[0], tuple) and len(vfilter[0]) == 2:
                    # Assume that it's a 2-tuple of 2 elements: comment one of these out
                    # as appropriate!
                    comparison_dist_dict["Hollands"] = ha.get_hollands_el_el_values(
                        vfilter[0][0], vfilter[0][1], self.manager
                    )
                    # comparison_dist_dict['40pc'] = fp.get_40pc_el_el_values(
                    #     vfilter[0][0],
                    #     vfilter[0][1]
                    # )
                ...
                for mode in hist_modes:
                    ...
                    if vl in vls_to_make_standalone:
                        standalone_plot = (
                            model_analyser.make_variable_distribution_plot(
                                variable_name,
    ...
    def plot_multipanels(
        self, model_analyser, individual_pop_plots_dict, plot_descriptions
    ):
        ...
        for plot_name in ["fcf"]:
            for mode in hist_modes:
                ...
                for vl in self.get_variable_type_list_for_multipanel():
                    try:
                        plots_to_multipanelise.append(
                            individual_pop_plots_dict[vl][plot_name][mode]
                        )
                    except KeyError:
                        # We probably didn't actually have that vl specified in the plot
                        # descriptions
                        pass
    ...
    def get_vfilter(self, variable_type, plot_description):
        if variable_type in self.get_io_type_variables():
            ...
            if variable_type == "Pollution":
                new_bottom_end = -np.inf  # plot_description[1][1] - 150
                # ^ A bit of a hack - basically, element ratios for 'Pollution' can be
                # really low, and it makes sense to just drop the bottom end off
                new_top_end = np.inf  # plot_description[1][2] + 150
                # ...ditto at the top
                return (
                    plot_description[1][:1]
                    + (new_bottom_end,)
                    + (new_top_end,)
                    + plot_description[1][3:]
                )
            else:
...
def run_control_pipeline():
    ...
    observer_dict = {"IdealObservererr0": [so.ObservationType.NoCut, 0]}
    modeller_dict = {
        "StandardModeller": [
    ...
    pipeline = Pipeline(
        "control", population_parameter_dict, observer_dict, modeller_dict
    )
    ...
    plot_descriptions = {
        ...
        "fcf": (
            (mp.ModelParameter.fragment_core_frac, 0, 1, 0.025),
            (ci.Element.Fe, -20, -2, 0.1),
        ),
        "distance": (
            (mp.ModelParameter.formation_distance, -2, 1, 0.0125),
            (ci.Element.Na, -20, -4, 0.1),
        ),
        "time": (
            (mp.ModelParameter.t_sinceaccretion, 0, 100000, 1),
            (ci.Element.Ca, -20, -4, 0.1),
        ),
        "metallicity": (
            (mp.ModelParameter.metallicity, 0, 958, 24),
            (ci.Element.Ca, -20, -4, 0.1),
        ),
    }
...
def run_control_pipeline_with_realistic_observer():
    ...
    observer_dict = {
        "RealisticObservererr0": [so.ObservationType.TeffIndividualElementCutoff, 0],
    ...
    pipeline = Pipeline(
        "controlrealistic", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
...
def run_control_pipeline_with_variable_observer_offset():
    ...
    observer_dict = {
        "DefaultObserveroff2": [so.ObservationType.TeffIndividualElementCutoff, 0, 2],
        "DefaultObserveroff1p5": [
            so.ObservationType.TeffIndividualElementCutoff,
            0,
            1.5,
        ],
        "DefaultObserveroff1": [so.ObservationType.TeffIndividualElementCutoff, 0, 1],
        "DefaultObserveroff0p5": [
            so.ObservationType.TeffIndividualElementCutoff,
            0,
            0.5,
        ],
        "DefaultObserveroff0": [so.ObservationType.TeffIndividualElementCutoff, 0, 0],
        "DefaultObserveroffm0p5": [
            so.ObservationType.TeffIndividualElementCutoff,
            0,
            -0.5,
        ],
        "DefaultObserveroffm1": [so.ObservationType.TeffIndividualElementCutoff, 0, -1],
        "DefaultObserveroffm1p5": [
            so.ObservationType.TeffIndividualElementCutoff,
            0,
            -1.5,
        ],
        "DefaultObserveroffm2": [so.ObservationType.TeffIndividualElementCutoff, 0, -2],
    }
    modeller_dict = {
        "StandardModeller": [
            sm.ModellerType.AnalyticApproximation,
    ...
    pipeline = Pipeline(
        "controlvariable", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
...
def run_fcf_comparison_pipeline():
    ...
    pipeline = Pipeline(
        "fcfcomparisons", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
    ...
    if make_p_threshold_table:
        ...
        for p_threshold in p_thresholds:
            sample_size_cd = (
                pipeline.find_sample_size_for_given_distinguishability_probability(
                    [
                        "StandardModeller",
                        "RealisticObservererr0p2",
                        "DACollisional",
...
def run_delta_fcf_variable_error_pipeline():
    # For the version where Cr is included as a fourth element, uncomment/swap over the
    # lines commented and marked with 'CR'
    # Although technically there's no real reason you couldn't just do it all together
    # in one go
    population_parameter_dict = {
        "SyntheticHollandsDeltaPop": {
            sp.PopulationParameter.size: 50000,
            # ^ Should be enough to give a decent number of resamples during mv testing
    ...
    list_of_error_tuples = [
        # Doing this explicitly rather than by iteration to avoid floating point errors
        (0, "0"),
        (0.025, "0p025"),
        (0.05, "0p05"),
        (0.075, "0p075"),
    ...
    modeller_dict = {"NullModeller": [sm.ModellerType.Null, None]}
    pipeline = Pipeline(
        "dzcomparison", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
    ...
    # ...instead compare 3 elements using a multivariate test:
...
def run_strategy_comparison_pipelines():
    ...
    pipeline = Pipeline(
        "strategy_comp", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
...
def run_profiling_pipeline():
    ...
    observer_dict = {
        "ProfileObservererr0": [so.ObservationType.TeffIndividualElementCutoff, 0],
    ...
    pipeline = Pipeline(
        "profiling", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
...
def run_variable_timescale_pipeline():
    ...
    modeller_dict = {"NullModeller": [sm.ModellerType.Null, None]}
    pipeline = Pipeline(
        "variable_timescale", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
    ...
    # pops_to_iterate = [
    #     'SyntheticHollandsDeltaPopKN',
    #     'SyntheticHollandsDeltaPopBN',
    #     'SyntheticHollandsDeltaPopKO',
    #     'SyntheticHollandsDeltaPopBO'
    # ]
    # pops_to_iterate = [
    #     'SyntheticHollandsDeltaKN',
    #     'SyntheticHollandsTidalKN',
...
def run_volatile_pipeline():
    population_parameter_dict = {
        "HWOWet": {
            ...
            # ^ Setting this to a DA-specific value - doesn't matter because we're not
    ...
    modeller_dict = {"NullModeller": [sm.ModellerType.Null, None]}
    pipeline = Pipeline(
        "volatiles", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
    ...
    # water_content_wet = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWOWet'
    # ].output_water_content(excess_oxygen_calculator, oxidation_strategy)
    # water_content_dry = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWODry'
    # ].output_water_content(excess_oxygen_calculator, oxidation_strategy)
    # import csv
    ...
    # dp1 = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWOWet'
    # ].get_subset_with_detected_elements([ci.Element.O, ci.Element.Si])
    # dp2 = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWODry'
    # ].get_subset_with_detected_elements([ci.Element.O, ci.Element.Si])
    # dp3 = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWOWet'
    # ].get_observed_subset()
    # dp4 = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWODry'
    # ].get_observed_subset()
    # dp5 = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWOWet'
    # ].get_subset_with_detected_elements([ci.Element.C, ci.Element.Si])
    # dp6 = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWODry'
    # ].get_subset_with_detected_elements([ci.Element.C, ci.Element.Si])
    # print(len(dp1)) #881
    # print(len(dp2)) #1077
    ...
    # # We need a population with DAs and DBs, but I haven't programmed this
    # # functionality in yet. So what we'll do is reach inside the pipeline'
    # # populations and construct a hybrid population from what we find in there
    # # Going to add them in a 6:1 ratio, approximately accurate
    # # And repeat for both wet and dry
    # pre_made_wet_population = (
    #     pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1']['HWODAWet']
    #     .population[0:600000]
    #     + pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1']['HWODBWet'].
    #     population[0:100000]
    # )
    # pre_made_dry_population = (
    #     pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1']['HWODADry']
    #     .population[0:600000]
    #     + pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1']['HWODBDry'].
    #     population[0:100000]
    # )
    ...
    #     pre_made_dry_population
    # )
    # # Now here's the real trick: We'll dump it as a csv, then on rerunning the
    # # pipeline we can trick the code into loading this up as a normal population
    # # We'll comment out the original populations with separate DAs and DBs, and
    # # replace with a hybrid one
    # # And also comment out this code which generated the hybrid
    # # Now when we rerun, it'll think this was a population which it already ran, and
    # # will load it up
    # # One final thing: Need to go into the csv files themselves and reset all the IDs
    # # in column 1!
    # hybrid_wet_population.dump_to_csv(
    #     pipeline.get_popdump_dir()
    #     + 'popdump_HWOWet_ELB_DTObservererr0p1_NullModeller.csv'
    # )
    # hybrid_dry_population.dump_to_csv(
    #     pipeline.get_popdump_dir()
    #     + 'popdump_HWODry_ELB_DTObservererr0p1_NullModeller.csv'
    # )
    # raise

    # # Now to commit an even worse crime
    # # The two populations of wet and dry are very hard to distinguish
    # # So let's make more hybrid populations, where we only pick the ones within,
    # # say 10^1 AU
    # pre_made_wet_population = (
    #     pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1']['HWODAWet']
    #     .population[0:300000]
    ...
    # # And also comment out this code which generated the hybrid
    # # Now when we rerun, it'll think this was a population which it already ran, and
    # # will load it up - but need to be careful to load the correct number (ie the
    # # actual number available)
    # # One final thing: Need to go into the csv files themselves and reset all the IDs
    # # in column 1!
    # hybrid_wet_population.dump_to_csv(
    #     pipeline.get_popdump_dir()
    #     + 'popdump_HWOWetFilter_ELB_DTObservererr0p1_NullModeller.csv'
    # )
    # hybrid_dry_population.dump_to_csv(
    #     pipeline.get_popdump_dir()
    #     + 'popdump_HWODryFilter_ELB_DTObservererr0p1_NullModeller.csv'
    # )
    # raise
    ...
    plot_descriptions = {
        ...
        # ),
        "distance": (
            (mp.ModelParameter.formation_distance, -2, 2, 0.0125),
            ([ci.Element.O, ci.Element.Si], -3, 8, 0.1),
        ),
        # 'time': (
        #     (mp.ModelParameter.t_sinceaccretion, 0, 100000, 1),
        #     (ci.Element.Ca, -20, -4, 0.1)
...
def run_light_element_pipeline():
    # Remember to switch enhancement_model = 'FixedLightElement' in synthetic_population
    ...
    modeller_dict = {"NullModeller": [sm.ModellerType.Null, None]}
    pipeline = Pipeline(
        "light_element", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
    # dp1 = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWOSrichCore'
    # ].get_subset_with_detected_elements([ci.Element.S, ci.Element.O, ci.Element.Si])
    # dp2 = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    #     'HWOSiOrichCore'
    # ].get_subset_with_detected_elements([ci.Element.S, ci.Element.O, ci.Element.Si])
    # dp3 = pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1'][
    ...
    # print(len(dp3))  # 60038
    # print(len(dp4))  # 61755
    # So in total, we have 121793 PWDs and 1704 of those with Si, O and S - a rate of
    # 1.399%
    ...
    # # Now to commit a crime
    # # We need a population with DAs and DBs, but I haven't programmed this
    # # functionality in yet. So what we'll do is reach inside the pipeline's
    # # populations and construct a hybrid population from what we find in there
    # # Going to add them in a 6:1 ratio, approximately accurate
    # # And repeat for both wet and dry
    # pre_made_s_population = (
    #     pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1']['HWODASrichCore']
    #     .population[0:600000]
    #     + pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1']['HWODBSrichCore']
    #     .population[0:100000]
    # )
    # pre_made_sio_population = (
    #     pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1']['HWODASiOrichCore']
    #     .population[0:600000]
    #     + pipeline.results_dict['NullModeller']['ELB_DTObservererr0p1']['HWODBSiOrichCore']
    #     .population[0:100000]
    # )
    # population_size = None
    # wd_config_to_use = None
    ...
    # # One final thing: Need to go into the csv files themselves and reset all the IDs
    # # in column 1!
    # hybrid_s_population.dump_to_csv(
    #     pipeline.get_popdump_dir()
    #     + 'popdump_HWOSrichCore_ELB_DTObservererr0p1_NullModeller.csv'
    # )
    # hybrid_sio_population.dump_to_csv(
    #     pipeline.get_popdump_dir()
    #     + 'popdump_HWOSiOrichCore_ELB_DTObservererr0p1_NullModeller.csv'
    # )
    # raise
    ...
    plot_descriptions = {
        ...
        "fcf": (
            (mp.ModelParameter.fragment_core_frac, 0, 1, 0.025),
            ([ci.Element.O, ci.Element.S], -5, 5, 0.1),
        ),
        # 'distance': (
        #     (mp.ModelParameter.formation_distance, -2, 2, 0.0125),
        #     ([ci.Element.O, ci.Element.Si], -3, 4, 0.1)
        # ),
        # 'time': (
        #     (mp.ModelParameter.t_sinceaccretion, 0, 100000, 1),
        #     (ci.Element.Ca, -20, -4, 0.1)
    ...
    pipeline.create_mv_plots(
        ...
        [0.1],
        [0],
        # np.arange(5, 101, 5).tolist(),
        np.arange(5, 71, 1).tolist(),
        "NullModeller",
        "ELB_DTObserver",
        True,
        100,
...
def thermohaline_pipeline():
    ...
    modeller_dict = {"NullModeller": [sm.ModellerType.Null, None]}
    pipeline = Pipeline(
        "thermohaline", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()

    print(
        pipeline.results_dict["NullModeller"]["DefaultObservererr0p1"][
            "ThermohalineOn"
        ].population[0]
    )
    print(
    ...
    premade_hot_t_on_population = [
        wd
        for wd in pipeline.results_dict["NullModeller"]["DefaultObservererr0p1"][
            "ThermohalineOn"
        ].population
        if wd.wd_properties[mp.WDParameter.temperature] > 15000
    ]
    ...
    pollution_config_to_use = None
    output_filename = None
    pipeline.results_dict["NullModeller"]["DefaultObservererr0p1"]["ThermohalineOn"] = (
    ...
    plot_descriptions = {
        ...
        "fcf": (
            (mp.ModelParameter.fragment_core_frac, 0, 1, 0.025),
            ([ci.Element.Fe, ci.Element.Si], -100, 100, 0.1),
        ),
        "distance": (
            (mp.ModelParameter.formation_distance, -2, 2, 0.0125),
            ([ci.Element.O, ci.Element.Si], -100, 100, 0.1),
        ),
        # 'time': (
        #     (mp.ModelParameter.t_sinceaccretion, 0, 100000, 1),
        #     (ci.Element.Ca, -20, -4, 0.1)
    ...
    pipeline.create_mv_plots(
        # Of course, what we'll want to do here is compare to real DAs
        "ThermohalineOn",
        "ThermohalineOff",
        [0.1],
        [0],
    ...
    pipeline.plot_ternary_comparisons(
        (1000, ci.Element.Ca),
        "PEWDD_thermohaline",
        [ci.Element.Fe, ci.Element.Si, ci.Element.O],
    )
    pipeline.plot_results(plot_descriptions, None)
...
def run_reference_pipeline():
    ...
    observer_dict = {
        "RealisticObservererr0p2": [so.ObservationType.TeffIndividualElementCutoff, 0.2]
    }
    modeller_dict = {"NullModeller": [sm.ModellerType.Null, None]}
    pipeline = Pipeline(
        "reference", population_parameter_dict, observer_dict, modeller_dict
    )
    pipeline.run_all_combinations()
