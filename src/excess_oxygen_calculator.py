...
class ExcessOxygenCalculator:
    ...
    def __init__(self):
        self.oxidation_dict = {
            ...
            OxidationStrategy.conservative: {
                ...
                ci.Element.Ca: 1,  # CaO
                # ci.Element.C: 2, # CO2
                # Ignoring CO2 (more volatile than H2O? more likely to form H2O than CO2
                # unless really distant?)
        ...
        # self.layer_priority = [
        #     gi.Layer.bulk, gi.Layer.mantle, gi.Layer.crust, gi.Layer.core
        # ]
    ...
    def calculate_excess_oxygen(self, normalised_abundances, favoured_layer, ox_strat):
        oxygen_assignations = self.assign_oxygen(
            normalised_abundances, favoured_layer, ox_strat
        )
        total_oxygen_assigned = sum(v for v in oxygen_assignations.values())
        excess_oxygen = (
            normalised_abundances[ci.Element.O][favoured_layer] - total_oxygen_assigned
        )
        fractional_excess_oxygen = (
            excess_oxygen / normalised_abundances[ci.Element.O][favoured_layer]
        )
        return excess_oxygen, fractional_excess_oxygen, oxygen_assignations

    def calculate_excess_oxygen_mass(
        self,
        number_abundance_dict,
        favoured_layer,
        fractional_excess_oxygen,
        total_mass=1,
    ):
    ...
    def calculate_water_abundance(
        ...
        excess_o_mass = self.calculate_excess_oxygen_mass(
            number_abundance_dict, favoured_layer, fractional_excess_oxygen, total_mass
        )
    ...
    def calculate_sigma_significance(
        self, starting_abundances, favoured_layer, ox_strat, o_error
    ):
        ...
        while converged_o is None:
            ...
            reformatted_abundances = self.reformat_abundances(
                test_abundances, favoured_layer
            )
            normalised_abundances = self.normalise_abundances(reformatted_abundances)
            excess_oxygen, fractional_excess_oxygen, oxygen_assignations = (
                self.calculate_excess_oxygen(
                    normalised_abundances, favoured_layer, ox_strat
                )
            )
    ...
    def reformat_abundances(self, abundances, favoured_layer=gi.Layer.bulk):
        # Designed to take a {Element: Value} and return {Element: {Layer.bulk: Value}}
        # if needed
    ...
    def calculate_steady_state_composition(
        self,
        wd_abundances,
        ...
        # Composition of data points, if they were in steady state (using predicted
        # composition to fill in missing elements).
        ...
        for element, default in predicted_composition.items():
            if element in wd_abundances and wd_abundances[element] != 0:
                raw_abundance = 10 ** (
                    wd_abundances[element] - wd_abundances[reference_element]
                )
                scaling_factor = wd_timescales[reference_element] / wd_timescales.get(
                    element
                )
                to_use[element] = {reference_layer: raw_abundance * scaling_factor}
            else:
                to_use[element] = {
                    reference_layer: default[reference_layer]
                    / predicted_composition[reference_element][reference_layer]
                }
        # Everything is now scaled to reference_element (unnormalised), and corrected
        # for sinking where needed.
    ...
    def run_full_calculation(
        ...
        abundances_dict,
        total_mass=1,
        include_conservative_solarFe_composition=False,
        include_dataO_composition=False,
        observations=None,
        errors={ci.Element.O: 0},
        wd_timescales=None,
    ):
        eoc_stat_dict = dict()
        ...
        for composition_name, abundances in abundances_dict.items():
            if abundances is not None:
                ...
                normalised_abundances = self.normalise_abundances(
                    reformatted_abundances
                )
                ...
                for ox_strat in self.oxidation_dict.keys():
                    if any_nans:
                        ...
                        water_mass, water_mass_fraction, excess_o_mass = (
                            self.calculate_water_abundance(
                        ...
                        sigma_significance = self.calculate_sigma_significance(
                            normalised_abundances, favoured_layer, ox_strat, o_error
                        )
                        eoc_stat_dict[composition_name][ox_strat] = (
                            excess_oxygen,
                            fractional_excess_oxygen,
                            oxygen_assignations,
                            water_mass,
                            ...
                            excess_o_mass,
                            normalised_abundances,
                            self.species_names[ox_strat],
                            sigma_significance,
                            favoured_layer,
                        )
                if include_conservative_solarFe_composition:
                    solar_Fe_comp = self.calculate_conservative_composition(
                        normalised_abundances,
                        None,
                        ci.Element.Mg,
                        gi.Layer.bulk,
                    ...
                    # additional_abundances[
                    #     composition_name + ', allow solar Fe'
                    # ] = solar_Fe_comp
                if include_dataO_composition:
                    data_O_comp = self.calculate_conservative_composition(
                        normalised_abundances,
                        observations,
                        ci.Element.Mg,
                        gi.Layer.bulk,
                ...
                if include_dataO_composition:
                    data_comp = self.calculate_conservative_composition(
                        normalised_abundances,
                        observations,
                        ci.Element.Mg,
                        gi.Layer.bulk,
                ...
                if wd_timescales is not None:
                    ss_comp = self.calculate_steady_state_composition(
                        observations,
                        normalised_abundances,
                        wd_timescales,
                        ci.Element.Mg,
        ...
        for composition_name, abundances in additional_abundances.items():
            ...
            for ox_strat in self.oxidation_dict.keys():
                excess_oxygen, fractional_excess_oxygen, oxygen_assignations = (
                    self.calculate_excess_oxygen(abundances, favoured_layer, ox_strat)
                )
                water_mass, water_mass_fraction, excess_o_mass = (
                    self.calculate_water_abundance(
                        abundances, favoured_layer, fractional_excess_oxygen, total_mass
                    )
                )
                sigma_significance = self.calculate_sigma_significance(
                    abundances, favoured_layer, ox_strat, o_error
                )
                eoc_stat_dict[composition_name][ox_strat] = (
                    excess_oxygen,
                    fractional_excess_oxygen,
    ...
    def calculate_conservative_composition(
        self,
        nominal_model,
        observations,
        reference_element,
        reference_layer,
        elements_to_alter=None,
        force_data_fit=False,
    ):
        ...
        # NB: Assuming maximum abundance for all non-O elements is definitely not the
        # most conservative composition in general (because you have to normalise
        # everything afterwards).
        # Need to prioritise maximising the more oxidising elements somehow
        toret = dict()
        assert reference_element in nominal_model
        reformatted_abundances = self.reformat_abundances(
            nominal_model, reference_layer
        )
        for element, nominal_entry in reformatted_abundances.items():
            ...
            if (
                ...
                and element in observations
                and reference_element in observations
                and observations.get(element, 0) != 0
            ):
                observed_abundance = 10 ** (
                    observations[element] - observations[reference_element]
                )
                possible_abundances.append(observed_abundance)
            if (
                element in self.solar
                and reference_element in self.solar
                and not force_data_fit
            ):
                solar_abundance = 10 ** (
                    self.solar[element] - self.solar[reference_element]
                )
                possible_abundances.append(solar_abundance)
            nominal_abundance = (
                nominal_entry[reference_layer]
                / reformatted_abundances[reference_element][reference_layer]
            )
            possible_abundances.append(nominal_abundance)
            if elements_to_alter is None or (
                elements_to_alter is not None and element in elements_to_alter
            ):
                if force_data_fit:
...
def main():
    ...
    mass = 1
    # ^ There is an inherent inconsistency in that we later assume that this mass
    # includes H, although H is not present in the dict.
    # Basically we assume M_hydrogen << M_object
    dummy_gm = gi.GeologyModel()
    earth = dummy_gm.element_info
    print(earth)
    conservative_input_dict = eoc.calculate_conservative_composition(
        input_dict, observations, ci.Element.Si, gi.Layer.bulk
    )
    conservative_input_dict_fe_only = eoc.calculate_conservative_composition(
        input_dict, observations, ci.Element.Mg, gi.Layer.bulk, [ci.Element.Fe]
    )
    ...
    eoc_stats = eoc.run_full_calculation(
        input_compositions, mass, False, False, None, {ci.Element.O: 0.1}
    )
    print(eoc_stats)
    print("Earth:")
    print(earth)
    eoc_stats = eoc.run_full_calculation(
        {"Earth": {el: earth[el] for el in ci.usual_elements}}, mass
    )
    print(eoc_stats)
