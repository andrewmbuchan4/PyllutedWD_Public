...
class Layer(Enum):
    ...
    def __hash__(self):
        ...
        # Normally python will try to hash an Enum by hashing its name (which
...
class GeologyModel:
    ...
    def __init__(self, bulk_abundance_overrides=None, normalise_abundances=True):
        ...
        self.M_Sun = pc.M_Sun
        # ^ Mass of Sun in kg. Maybe solar/stellar info could have its own class
        self.mars_abundances = {
            # Taking this from table 10 of Yoshizaki 2020
    ...
    def initialise_stellar_info_for_graphs(self):
        ...
        # solar_ratiod_to_stellar = log( (X/Mg)solar / (X/Mg)stellar )
        #                         = log(X/Mg)solar - log(X/Mg)stellar
        # => solar_abundance = log(X/Mg)solar
        #                    = solar_ratiod_to_stellar + log(X/Mg)stellar
        solar_ratiod_to_stellar = {
            ...
            ci.Element.Mg: 0,
            # ^ I guess this is technically undefined (0/0), but it's 0 for our purposes
        ...
        # upper_ratiod_to_stellar = log( (X/Mg)upper / (X/Mg)stellar )
        #                         = log(X/Mg)upper - log(X/Mg)stellar
        ...
        # These are the raw [X/H] ratios from
        # https://docs.google.com/spreadsheets/d
        # /1B2WXfFtx3KhP4Ucl2kmJGmhq73ldJqqZiofG8m10n48
        ...
        for el in self.solar_ratiod_to_H:
            ...
            self.solar_ratiod_to_H[el] -= 12
        for element in ci.usual_elements:
            self.solar_abundances[element] = (
                local_stellar_abundances[element] + solar_ratiod_to_stellar[element]
            )
            self.upper_ratiod_to_solar[element] = (
                upper_ratiod_to_stellar[element] - solar_ratiod_to_stellar[element]
            )
            self.lower_ratiod_to_solar[element] = (
                lower_ratiod_to_stellar[element] - solar_ratiod_to_stellar[element]
            )
    ...
    def apply_bulk_abundances(self, bulk_abundance_overrides=None):
        if bulk_abundance_overrides is None:
            self.element_info = {
                # McDonough 2003
                # NB: These are not normalised! Use the loaded normalised version for
                # reference
                # NB: The Fe abundance in Table 5 of McDonough 2003 was written as 0.49
                # but it should actually be 0.149
                ...
                ci.Element.C: {
                    Layer.bulk: 0.001581,
                    Layer.crust: 0,
                    Layer.core: 0.0083482,
                },
                ci.Element.N: {
                    Layer.bulk: 0.000046429,
                    Layer.crust: 0,
                    Layer.core: 0.0002684,
                },
                # ^ NB: This implies a negative mantle abundance! Luckily it doesn't
                # matter in actual runs
                ci.Element.S: {Layer.bulk: 0.005, Layer.core: 0.03},
                ci.Element.Hf: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.Hf, 0.00000019
                    )
                },
                ci.Element.U: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.U, 0.000000015
                    )
                },
                ci.Element.Ta: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.Ta, 0.000000025
                    )
                },
                ci.Element.Nb: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.Nb, 0.00000044
                    )
                },
                ci.Element.Pb: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.Pb, 0.00000023
                    )
                },
                ci.Element.Zn: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.Zn, 0.00004
                    )
                },
                ci.Element.Mn: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.Mn, 0.0008
                    )
                },
                ci.Element.Ga: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.Ga, 0.000003
                    )
                },
                ci.Element.V: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.V, 0.000105
                    )
                },
                ci.Element.Cu: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.Cu, 0.00006
                    )
                },
                ci.Element.W: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.W, 0.00000017
                    )
                },
                ci.Element.P: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.P, 0.000715
                    )
                },
                ci.Element.Co: {
                    Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                        ci.Element.Co, 0.00088
                    )
                },
                # ci.Element.Pt: {
                #     Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                #         ci.Element.Pt, 0.0000019
                #     )
                # },
                # ci.Element.Pd: {
                #     Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                #         ci.Element.Pd, 0.000001
                #     )
                # },
                # ci.Element.Rh: {
                #     Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                #         ci.Element.Rh, 0.00000024
                #     )
                # },
                # ci.Element.Re: {
                #     Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                #         ci.Element.Re, 0.000000075
                #     )
                # },
                # ci.Element.Ru: {
                #     Layer.bulk: self.convert_bulk_abundance_mass_to_number(
                #         ci.Element.Ru, 0.0000013
                #     )
                # },
            }
        else:
    ...
    def convert_bulk_abundance_mass_to_number(self, element, mass_abundance):
        return (
            mass_abundance * self.get_earth_mean_molecular_weight()
        ) / ci.get_element_mass(element)
    ...
    def get_mars_oxygen_fugacity(self):
        # See Rai/van Westrenen 2013 (although they also suggest -1.3, but then just use
        # -1 in their calcs. and conclusion)
        return -1  # i.e. 1 log unit below IW buffer.

    def get_earth_mean_molecular_weight(self):
        # Estimate based on Table 5 in McDonough 2003
        # Comparing number to weight abundances gives an estimate of
        # mmw: mass_abundance = (element_mass * number_abundance) / mmw
        return 26
    ...
    def fill_in_mantle_abundances(self):
        for element in self.element_info:
            try:
                self.element_info[element][Layer.mantle] = (
                    self.element_info[element][Layer.bulk]
                    - (
                        (
                            self.element_info[element][Layer.core]
                            * self.get_earth_layer_number_fraction(Layer.core)
                        )
                        + (
                            self.element_info[element][Layer.crust]
                            * self.get_earth_layer_number_fraction(Layer.crust)
                        )
                    )
                ) / self.get_earth_layer_number_fraction(Layer.mantle)
            except KeyError as e:
                # Then I assume we don't have enough information to calculate an
                # observed mantle abundance (eg Hf, U etc)
                pass

    def get_mu(self, element):
        # At some point this function could become an element-dependent function saying
        # what proportion of it gets a chance to react with the metal
        return 1
    ...
    def find_system_specific_abundances(self, abundances, core_number_fraction):
        ...
        # Output: The bulk abundances of a body which forms with the specified cnf and
        # core/mantle composition
        output = dict()
        mantle_number_fraction = 1 - core_number_fraction
        for element, abundance in abundances.items():
            output[element] = (
                core_number_fraction * abundance[Layer.core]
                + mantle_number_fraction * abundance[Layer.mantle]
            )
        return output
    ...
    def grid_method(
        self, Ds, core_number_fraction, pressure, fO2, abundances, temp, nbot
    ):
        important_elements = [
            ci.Element.Fe,
            ci.Element.Si,
        ...
        # If we do this for every element, it'll take way too long
                    ...
                    for ni_val in grid_points_dict[ci.Element.Ni]:
                        for si_val in grid_points_dict[ci.Element.Si]:
                            for fe_val in grid_points_dict[ci.Element.Fe]:
                                ...
                                abundances, cnf = self.calculate_abundances(
                                    test_Ds, core_number_fraction, True
                                )
                                new_Ds = self.pamela.get_all_partition_coefficients(
                                    pressure, fO2, abundances, temp, nbot
                                )
                                deviation = self.find_Ds_deviation(
                                    test_Ds, new_Ds, important_elements
                                )
    ...
    def calculate_abundances(self, Ds, core_number_fraction, calculate_mantle=False):
        ...
        for element, D in Ds.items():
            ...
            if element == ci.Element.Placeholder:
                # This used to be special logic for Oxygen
                # Then "D" is not actually D
                # This calculation assumes mu = 1...
                abundances[element][Layer.core] = D
                if calculate_mantle:
                    abundances[element][Layer.mantle] = (
                        bulk_abundance_by_number - (D * core_number_fraction)
                    ) / (1 - core_number_fraction)
            else:
                metal_abundance = (
                    mu
                    * bulk_abundance_by_number
                    * (
                        (D * core_number_fraction)
                        / (mantle_number_fraction + (D * core_number_fraction))
                    )
                )
                abundances[element][Layer.core] = metal_abundance
                if calculate_mantle:
                    # We only need to calculate mantle stuff on the final iteration,
                    # otherwise it just wastes time (unless S is present...)
                    sil_abundance = bulk_abundance_by_number * (
                        1 - (
                            mu * (
                                1 - (
                                    mantle_number_fraction / (
                                        mantle_number_fraction + (
                                            D * core_number_fraction
                                        )
                                    )
        ...
        # Fischer O
        # This logic keeps O the same, normalises everything else. Reinstate if using
        # old O calculation
        # for e in abundances.keys():
        #     if e != ci.Element.O:
        #         total_core_abundance += abundances[e][Layer.core]
        ...
        # for e in abundances.keys():
        #     if e != ci.Element.O:
        #         abundances[e][Layer.core] *= (
        #             1 - abundances[ci.Element.O][Layer.core]
        #         ) * inv_core_abundance
        #         if calculate_mantle:
        #             abundances[e][Layer.mantle] *= (
        #                 1 - abundances[ci.Element.O][Layer.mantle]
        #             ) * inv_mantle_abundance
    ...
    def form_a_planet_iteratively(
        ...
        # -----------------------------
        # Every so often, nudge the algorithm in the right direction to help convergence
        nudge_iterations = 1000
        # Invoke grid solution after this many iterations (if O increases 2 times in a
        # row)
        grid_iterations = 1001
        # Give up eventually
        max_iterations = 1000
        ...
        o_cap = 0.3  # v
        si_cap = 1  # v
        # ^ Justification: O and Si should never be highly siderophilic (i.e. D much
        # greater than 1) otherwise no silicate is left and the whole exercise loses its
        # meaning! To be conservative, set D_Si <= 1 and D_O <= 1.
        # But actually D_O = 1 is still a bit silly and leads to sudden X_O spikes so I
        # tuned it to 0.3

        # NB: if using a parametrisation with direct calculation of X_O rather than D_O,
        # the o_cap should be dropped! (to something in the region of 0.2 ish)

        movement_fraction = 1
        # ^ Setting this to 0.5 is the same as setting nudge_iterations to 1, in
        # principle
        while not converged:
            # If using the old Oxygen logic: the Ds value for oxygen is NOT actually
            # D_O, it's just skipped straight to abundances[O][Layer.core] and that
            # value is stored in Ds
            Ds = self.pamela.get_all_partition_coefficients(
                pressure, fO2, abundances, temp, nbot
            )
            if movement_fraction != 1 and prev_Ds is not None:
                for element in Ds.keys():
                    Ds[element] = prev_Ds[element] + movement_fraction * (
                        Ds[element] - prev_Ds[element]
                    )
            ...
            if (
                double_o_detection
                and not o_cap_set
                and o_increased_last_time
                and Ds[ci.Element.O] > prev_Ds[ci.Element.O]
            ):
                cap_o = True
            ...
            if abundances is None:
                ...
                abundances[ci.Element.S].get(Layer.core, 0) != 0
                or abundances[ci.Element.S].get(Layer.mantle, 0) != 0
            ):
                calculate_mantle = True
            abundances, w_met = self.calculate_abundances(Ds, w_met, calculate_mantle)

            if not average_next_iteration:
                # To prevent 'cheating', we are only allowed to declare convergence if
                # we didn't do a nudge this iteration.
                # This way convergence is only declared if the Ds are genuinely a
                # solution (i.e. they didn't significantly change for a whole, legit,
                # iteration).
    ...
    def tabulate_output(self, abundances, w_met, Ds, outfile=None):
        ...
        if outfile is not None:
            with open(f"{outfile}.csv", "w", newline="", encoding="utf-8") as f:
                ...
                to_write.writerow([])
                to_write.writerow(
                    ["Element", "Mantle Number Fraction", "Core Number Fraction"]
                )
                for element, el_abundances in abundances.items():
                    to_write.writerow(
                        [
                            element,
                            el_abundances[Layer.mantle],
                            el_abundances[Layer.core],
                        ]
                    )
                to_write.writerow([])
    ...
    def convert_number_abundances_to_mass(self, number_abundance_dict):
        ...
        for layer in Layer:
            total_mass = 0
            for element in number_abundance_dict.keys():
                # Assume the abundance of any absent element is 0
            ...
            if total_mass > 0:
                # If total_mass was zero, then just ignore that layer (Layer was
                # probably not present in the input)
                for element in number_abundance_dict.keys():
                    X_E = number_abundance_dict[element].get(layer, 0)
                    M_E = ci.get_element_mass(element)
    ...
    def convert_mass_abundances_to_number(self, mass_abundance_dict):
        ...
        for layer in Layer:
            ...
            for element in mass_abundance_dict.keys():
                # Assume the abundance of any absent element is 0
                M_E = mass_abundance_dict[element].get(layer, 0)
                X_E = M_E / ci.get_element_mass(element)
                total_number += X_E
            # Now loop through again to build the output dict
            if total_number > 0:
                # If total_number was zero, then just ignore that layer (Layer was
                # probably not present in the input)
                for element in mass_abundance_dict.keys():
                    # Assume the abundance of any absent element is 0
                    M_E = mass_abundance_dict[element].get(layer, 0)
    ...
    def get_relative_core_mantle_masses(self, composition_override=None):
        if (
            composition_override is None
            and self.core_relative_mass is not None
            and self.mantle_relative_mass is not None
        ):
            return self.core_relative_mass, self.mantle_relative_mass
        core_relative_mass = 0
        mantle_relative_mass = 0
        for element in ci.usual_elements:
            # Only considering major elements for now. Also may need to exclude N,
            # currently its mantle content is negative...
    ...
    def convert_core_number_fraction_to_core_mass_fraction(
        self, core_number_fraction, composition_override=None
    ):
        # Need to calculate relative mass of core and mantle like material
        core_relative_mass, mantle_relative_mass = self.get_relative_core_mantle_masses(
            composition_override
        )
        mantle_number_fraction = 1 - core_number_fraction
        cmf = (core_number_fraction * core_relative_mass) / (
            (core_number_fraction * core_relative_mass)
            + (mantle_number_fraction * mantle_relative_mass)
        )
        return cmf

    def convert_core_mass_fraction_to_core_number_fraction(
        self, core_mass_fraction, composition_override=None
    ):
        # Need to calculate relative mass of core and mantle like material
        core_relative_mass, mantle_relative_mass = self.get_relative_core_mantle_masses(
            composition_override
        )
        cnf = (mantle_relative_mass * core_mass_fraction) / (
            ((mantle_relative_mass * core_mass_fraction) + core_relative_mass)
            - (core_relative_mass * core_mass_fraction)
        )
        return cnf

    def get_core_fraction_from_abundance_dict(self, abundance_dict):
        # abundance_dict could be by number, or by mass. The output will then be either
        # number or mass accordingly.
        ...
        for element, abundance_by_layer in abundance_dict.items():
            if abundance_by_layer[Layer.bulk] > 0:
                ...
                step = 0.1
                while abs(diff) >= tol:
                    implied_bulk_abundance = (
                        trial_cf * abundance_by_layer[Layer.core]
                        + (1 - trial_cf) * abundance_by_layer[Layer.mantle]
                    )
                    ...
                    if (
                        diff > 0
                        and (
                            abundance_by_layer[Layer.core]
                            > abundance_by_layer[Layer.mantle]
                        )
                    ) or (
                        diff < 0
                        and (
    ...
    def calculate_pressure_and_mass(
        self,
        planet_radius,
        iron_mass_fraction,
        lower_limit=False,
        ...
        # Planet radius in km, iron mass fraction on a scale from 0 to 1.
        # NB NOT NUMBER FRACTION!
        planet_mass = (planet_radius / (lmbda - (100 * mu * iron_mass_fraction))) ** (
            1 / nu
        )  # In Earth masses
        planet_mass_kg = planet_mass * self.M_Earth  # In kg
        core_radius = (
            alpha * ((100 * iron_mass_fraction) ** beta) * (planet_mass**gamma)
        )  # In km
        ...
        g_core = (self.G * planet_mass_kg * iron_mass_fraction) / (
            core_radius_metres**2
        )  # In m/s
        density_mantle = (3 * (1 - iron_mass_fraction) * planet_mass_kg) / (
            4 * np.pi * ((planet_radius_metres**3) - (core_radius_metres**3))
        )  # In kg/m^3
        if lower_limit:
            # Then we are going to find a lower limit on radius/mass by returning the
            # CMB pressure in this function instead of the (more realistic) mid-mantle
            # pressure
            g_mantle = (g_surface / 2) + (g_core / 2)  # In m/s
            depth_CMB_metres = planet_radius_metres - core_radius_metres
            pressure_CMB = (g_mantle * density_mantle * depth_CMB_metres) / 1000000000
            # ^ In GPa
            ...
            depth_midmantle_metres = planet_radius_metres - (
                0.5 * (planet_radius_metres + core_radius_metres)
            )
            pressure_midmantle = (
                g_uppermantle * density_uppermantle * depth_midmantle_metres
            ) / 1000000000  # In GPa
            return pressure_midmantle, planet_mass

    def calculate_radius_and_mass(
        self, pressure_in_GPa, abundance_dict, lower_limit=False
    ):
        if pressure_in_GPa <= 0:
            print("Warning! Pressure was <= 0, returning None, None")
            return None, None
        lower_radius_bound = 0
        # ^ Strictly speaking, this should be a bit bigger than 0 because Lena's model
        # breaks down for very small objects
        upper_radius_bound = 12000
        # ^ This is roughly where Lena's model will start breaking down
        ...
        if not (0 < iron_mass_fraction < 1):
            ...
            return None, None
        max_pressure, max_mass = self.calculate_pressure_and_mass(
            upper_radius_bound, iron_mass_fraction, lower_limit
        )
        if pressure_in_GPa > max_pressure:
            # Then we will never replicate the pressure and will get stuck in an
            # infinite loop
            print(
                "Warning! Pressure was too high to replicate. Considering raising the upper_radius_bound in GeologyModel.calculate_radius. Returning None, None."
            )
            return None, None
        ...
        while True:
            ...
            test_pressure, test_mass = self.calculate_pressure_and_mass(
                test_radius, iron_mass_fraction, lower_limit
            )
    ...
    def get_earth_mix(
        self,
        fragment_core_fraction,
        abundance_dict=None,
        elements_to_mix=ci.usual_elements,
    ):
        # Function to calculate the composition of a body with an Earth-like mantle and
        # Earth-like core, but arbitrary fragment core fraction
        ...
        for element in elements_to_mix:  # For now we only care about these ones
            ...
            bulk_comp = (fragment_core_fraction * core_abundance) + (
                (1 - fragment_core_fraction) * mantle_abundance
            )
            mix[element] = bulk_comp
        return mix

    def get_critical_fragment_core_fraction(self, D, parent_core_frac, dD_dV, dpcf_dV):
        # dD_dV is the derivative of the D w.r.t our variable of interest V (could be
        # pressure, fO2... anything that affects the core/mantle compositions of the
        # parent body of fixed bulk composition).
    ...
    def get_dX_dV(
        ...
        dD_dV,
        dpcf_dV,
        parent_bulk_abundance=1,
    ):
        # This calculates the rate at which we change the final (normalised) number
        # abundance of an element as we change some variable in the parent like pressure.
        # It scales linearly with parent_bulk_abundance so if we don't care about the
        # actual numbers we can just take that to be 1 and work in relative terms.
        # dD_dV is the derivative of the D w.r.t our variable of interest V (could be
        # pressure, fO2...anything that affects the core/mantle compositions of the
        # parent body of fixed bulk composition).
        # dpcf_dV is the derivative of the parent core fraction w.r.t V
        numerator = (
            parent_bulk_abundance * (fragment_core_frac - parent_core_frac) * dD_dV
        ) - (
            parent_bulk_abundance
            * dpcf_dV
            * (D - 1)
            * (1 + (fragment_core_frac * (D - 1)))
        )
    ...
    def get_dlogX_dV(self, D, parent_core_frac, fragment_core_frac, dD_dV, dpcf_dV):
        # Like get_dX_dV, except for log (base 10) X instead of X since that is actually
        # more interesting for observations (it also eliminates parent_bulk_abundance)
        numerator = ((fragment_core_frac - parent_core_frac) * dD_dV) - (
            ((fragment_core_frac * (D - 1)) + 1) * (D - 1) * dpcf_dV
        )
        denominator = (
            np.log(10)
            * ((fragment_core_frac * (D - 1)) + 1)
            * (1 + ((D - 1) * parent_core_frac))
        )
        try:
            toret = numerator / denominator
        except ZeroDivisionError:
    ...
    def convert_linear_abundances_to_log(
        self, abundance_dict, pollution_fraction=-6, layer=Layer.bulk
    ):
        # Assuming input is a normalised {element: {layer: abundance}} dict, as used
        # elsewhere in geology_info
        toret = dict()
