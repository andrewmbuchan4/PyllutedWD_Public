...
class Manager:
    ...
    def __init__(self):
        ...
        if (
            pollution_model_names is not None
            and len(pollution_model_names) == 1
            and pollution_model_names[0].startswith("Hierarchy")
        ):
            ...
            try:
                self.parameter_hierarchy = mp.hierarchy_definitions_dict[
                    self.hierarchy_name
                ]
        ...
        # Think this next bit should be removed - the concept of a comparison prior
        # actually makes no sense
        self.default_prior = "Default"
        # ^ Use this prior except for running certain comparison priors at the end of a
        # hierarchy execution. This placement isn't ideal since prior_functions.py also
        # has this as its default prior
        self.comparison_priors = list()  # ['HighPressure', 'LowPressure']

        self.load_global_data()
        if not self.use_hierarchy:
            # This is a bit ugly: we should maybe just do the logic of this function in
            # self.run_system?
            self.load_models()
    ...
    def get_timescale_output_dir(
        self, wd_name, timescale_type, consider_thermohaline=False, create=True
    ):
    ...
    def get_chains_dir(
        ...
        # Choose a short name because of the 100 character limit
        dirname = (
            self.get_timescale_output_dir(
                wd_name, timescale_type, consider_thermohaline, create
            )
            + "c/"
        )
        if create:
    ...
    def load_wd_data(self, wd_data_filename):
        ...
        subsolar_limits = {
            ci.Element.C: collections.OrderedDict(
                {
                    # Mg and Si are very similar so just setting them the same
                    ci.Element.Mg: 0.95,
                    ci.Element.Si: 0.95,
                }
            ),
            ci.Element.N: collections.OrderedDict(
                {ci.Element.Mg: 0.35, ci.Element.Si: 0.35}
            ),
        }
        with open(pu.get_path_to_data() + wd_data_filename, encoding="utf-8") as wdcsv:
            for row in csv.DictReader(wdcsv):
                ...
                try:
                    ...
                    wd_name_fullname = ""
                if (
                    wd_name_fullname is not None
                    and wd_name_fullname != ""
                    and (wd_name_tag is None or wd_name_tag == "")
                ):
                    try:
                ...
                for el in ci.metals:  # i.e., not H and He
                    ...
                    try:
                        ...
                        # Occurs if error missing, e.g. if its an upper bound - so
                        # assume 0
                        error = 0
                    pewdd_ub = False
                    if error is not None and error < 0:
                        # In PEWDD, this indicates that the data type is an upper bound
                        pewdd_ub = True
                        error = 0
                    try:
                        ...
                        value = None  # occurs if this element is absent from the table
                    except ValueError:
                        # Occurs if, for example, this is an upper bound so the value
                        # starts with <
                    ...
                    if value is not None:
                        wd_abundance_data_raw[el] = wd.WhiteDwarfDataPoint(
                            data_point_type, value, error, included
                        )
                abundance_data = wd.WhiteDwarfAbundanceData(wd_abundance_data_raw)
                # Remove C and/or N if they are significantly sub-solar
                # There is a bit of an issue here: in principle the 'sub-solar'
                # description refers to the composition at the point of condensation.
                # The numbers here are at the point of observation. Could be different.
                # No obvious fix without assuming the answer, other than perhaps making
                # the ref_limits more generous
                ...
                try:
                    ...
                    wd_mass_error = 0
                # If any timescales (or logq) are 0, then we'll try to calculate them
                # ourselves:
                try:
                    Teff = float(row["T_eff"])
                except ValueError:
                    # It's one thing to have a default logg but a default Teff doesn't
                ...
                wd_property_data_raw = {
                    mp.WDParameter.atmospheric_type: wd.WhiteDwarfDataPoint(
                        wd.WhiteDwarfDataPointType.label, wd_type
                    ),
                    # mp.WDParameter.temperature: wd.WhiteDwarfDataPoint(
                    #     wd.WhiteDwarfDataPointType.measurement,
                    #     Teff,
                    #     Teff_error
                    # ),
                    mp.WDParameter.logg: wd.WhiteDwarfDataPoint(
                        wd.WhiteDwarfDataPointType.measurement, logg, logg_error
                    ),
                    mp.WDParameter.mass: wd.WhiteDwarfDataPoint(
                        wd.WhiteDwarfDataPointType.measurement, wd_mass, wd_mass_error
                    ),
                }
                if Teff is not None:
                    wd_property_data_raw[mp.WDParameter.temperature] = (
                        wd.WhiteDwarfDataPoint(
                            wd.WhiteDwarfDataPointType.measurement, Teff, Teff_error
                        )
                    )
                    if (
                        wd_type == ci.Element.He
                        and abundance_data.get_abundance(ci.Element.Ca) is None
                    ):
                        # Then we need to intervene: technically the
                        # timescale_interpolator won't be able to calculate a timescale.
                        # So we'll just use a very small amount of Ca
                        timescales = self.timescale_interpolator.get_wd_timescales(
                            wd_type, logg, Teff, self.default_Ca_value
                        )
                    else:
                        if wd_type == ci.Element.He:
                            timescales = self.timescale_interpolator.get_wd_timescales(
                                ...
                                logg,
                                Teff,
                                abundance_data.get_abundance(ci.Element.Ca).value,
                            )
                        else:
                            timescales = self.timescale_interpolator.get_wd_timescales(
                                wd_type, logg, Teff
                ...
                white_dwarf = wd.WhiteDwarf(
                    wd_name_tuple, property_data, abundance_data, timescales
                )
                self.white_dwarfs.append(white_dwarf)
                try:
                    # If we have an ID column, we can reassure ourselves that the index
                    # in our list of white dwarfs will be what we expect it to be
    ...
    def publish_live_data(self, N_wd, timescale_type):
        ...
        all_wd_timescales = white_dwarf.get_timescales_as_array(
        ...
        abundances, errors, upper_bounds, lower_bounds = (
            white_dwarf.get_abundance_arrays(elements_to_model)
        )
    ...
    def publish_live_model(
        self, model_name, consider_thermohaline, prior_name="Default"
    ):
    ...
    def load_models(self, model_names=None):
        # If we're in this function, then we're going to run a predetermined set of
        # models for each timescale type, so we're going to set self.models to be a dict
        # where the keys are the timescale types/therm flags and the values are the
        # predetermined set
    ...
    def execute_models(self, N_wd, timescale_type, consider_thermohaline):
        white_dwarf = self.white_dwarfs[N_wd]
        if (
            self.models.get(timescale_type, dict()).get(consider_thermohaline)
            is not None
        ):
            ...
            max_filename_overflow = self.get_max_filename_overflow(
                white_dwarf, timescale_type, consider_thermohaline
            )
            for model_name, model in self.models[timescale_type][
                consider_thermohaline
            ].items():
                if model_name not in self.executed_models.get(
                    (N_wd, timescale_type, consider_thermohaline), list()
                ):
                    # Then we haven't already run this one, so must do so. Otherwise, we
                    # can ignore.
                    self.publish_live_model(
                    ...
                    if (
                        ...
                        timescale_type,
                        consider_thermohaline,
                    ) not in self.executed_models.keys():
                        self.executed_models[
                            (N_wd, timescale_type, consider_thermohaline)
                        ] = list()
                    self.executed_models[
                        (N_wd, timescale_type, consider_thermohaline)
                    ].append(model_name)

    def get_max_filename_overflow(
        self, white_dwarf, timescale_type, consider_thermohaline
    ):
        max_filename_overflow = 0
        # NB: The following logic is a bit fragile: It only works if assuming that all
        # other timescale items/thermohaline flags that we're going to run have the same
        # length as this current one (otherwise we'll find that one of those other
        # combinations may have a different length, so will get abbreviated differently,
        # and end up in a different output directory)
        ideal_output_dir = self.get_chains_dir(
            white_dwarf.full_name(), timescale_type, consider_thermohaline, False
        )
        for model_name, model in self.models[timescale_type][
            consider_thermohaline
        ].items():
    ...
    def compare(self, N_wd, timescale_type, consider_thermohaline):
        ...
        for model_name, model in self.models[timescale_type][
            ...
            # ^ Should be an OrderedDict with the base model in first position
    ...
    def register_models_with_altered_priors(
        self, best_model_name, wd_name, timescale_type, consider_thermohaline
    ):
        toret = list()
        for prior in self.comparison_priors:
            ...
            # Make a copy here because of potential for changing contents of original
            required_params = mp.model_definitions_dict[best_model_name]
            mp.model_definitions_dict[new_model_name] = required_params
            self.models[timescale_type][consider_thermohaline][new_model_name] = (
                pm.PollutionModel(
                    new_model_name,
                    timescale_type,
                    self.enhancement_model,
    ...
    def run_system(self, system_to_run):
        ...
        for timescale_type in self.timescale_types_to_run:
            ...
            for consider_thermohaline in self.thermohaline_regimes_to_run:
                ...
                if self.use_hierarchy:
                    best_model = None
                    if len(list(self.parameter_hierarchy.keys())) > 4:
                        # Too many combinations to try every one
                        accepted_hierarchy_levels = [0]
                        max_hierarchy_level = 0
                        while (
                            self.parameter_hierarchy.get(max_hierarchy_level + 1)
                            is not None
                        ):
                            base_model_name = self.build_model(
                                accepted_hierarchy_levels
                            )
                            comparison_model_name = self.build_model(
                            ...
                            successfully_ran_timescale_type = self.execute_models(
                                system_to_run, timescale_type, consider_thermohaline
                            )  # We don't do anything with this output though?
                            best_model = self.compare(
                                system_to_run, timescale_type, consider_thermohaline
                            )
                        ...
                        #     int(c) for c in best_model if c.isdigit()
                        # ]
                    ap_models = self.register_models_with_altered_priors(
                        best_model,
                    ...
                    for ap_model in ap_models:
                        self.analyser.compare_two_models(
                            self.models[timescale_type][consider_thermohaline][
                                ap_model
                            ],
                            self.models[timescale_type][consider_thermohaline][
                            ...
                            output_dir,
                            system_to_run,
                            len(white_dwarf.get_elements_present()),
                        )
                        for other_ap_model in ap_models:
                            if ap_model is not other_ap_model:
                                # Using 'is not' rather than '!=' checks that they are
                                # (not) actually the same object instance, rather than
                                # (not) objects of equal value, which is want we want
                                # here
                                self.analyser.compare_two_models(
                                    self.models[timescale_type][consider_thermohaline][
                                        ap_model
    ...
    def analyse_models(self, N_wd, timescale_type, consider_thermohaline):
        ...
        self.create_dir(
            self.get_timescale_output_dir(
                white_dwarf.abbreviated_name_as_used,
                timescale_type,
                consider_thermohaline,
            )
        )
        self.analyser.update_graph_dir(
            self.get_timescale_output_dir(
                white_dwarf.abbreviated_name_as_used,
                timescale_type,
                consider_thermohaline,
        ...
        self.publish_live_data(N_wd, timescale_type)
        hierarchy_name = (
            pu.hierarchy_abbreviations[self.hierarchy_name]
            if self.model_names is None
            else "_".join(self.model_names)
        )
        ...
        # This needs to mimic the logic from pollution_model - should probably actually
        # use that logic rather than copy it
        stats_file = (
            self.get_timescale_output_dir(
                white_dwarf.abbreviated_name_as_used,
                timescale_type,
        ...
        for model_name, model in self.models[timescale_type][
            ...
            self.analyser.dump_model_stats(model_name, model, stats_file)

        # self.analyser.find_diff_sigma(
        #     self.models,
        #     N_wd,
        #     len(ld._live_non_zero_wd_abundances),
        #     stats_file
        # )

        # TODO: This stuff still gets printed in the csv file even when the model didn't
        # run (i.e., the 3DOvershoot case for He dominated systems) with no indication
        # that the output is not valid!
        self.analyser.find_parameter_sigma(
            "Differentiation",
        ...
        self.analyser.find_best_heated_model(
            self.models[timescale_type][consider_thermohaline],
            N_wd,
            len(white_dwarf.get_elements_present()),
            stats_file,
        ...
        with open(stats_file, "a", newline="", encoding="utf-8") as f:
            ...
            try:
                ...
                for element, value in white_dwarf.get_timescale_values_dict(
                    timescale_type
                ).items():
                    to_write.writerow([str(element), str(value)])
            except KeyError:
                # Occurs if there were no timescales for this timescale_type, so there
                # is not going to be any other output from here on!
        ...
        for model_name, model in self.models[timescale_type][
            ...
            if model.best_model:
                ...
                (
                    model.best_model_without_parameter.get(
                        mp.ModelParameter.fragment_core_frac, False
                    )
                    and plot_best_nondiff
                )
                or (model.best_heated_model and plot_best_heated)
                or model_name in bonus_plots
            ):
