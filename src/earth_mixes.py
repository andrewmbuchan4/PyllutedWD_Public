...
def generate_mixes():
    geology_model = gi.GeologyModel(None, False)
    # For now, not normalising the abundances because then N in mantle becomes negative!
    fcfs = [0.75, 0.99]
    mix_dict = dict()
    for fcf in fcfs:
        ...
        # abundances, w_met, Ds, all_Ds = geology_model.form_a_planet_iteratively(
        #     geology_model.get_earth_differentiation_pressure(),
        #     geology_model.get_earth_oxygen_fugacity()
        # )
        # mix = geology_model.get_earth_mix(fcf, abundances)
        #        For what the model thinks is (roughly) Mars:
        # abundances, w_met, Ds, all_Ds = geology_model.form_a_planet_iteratively(
        #     geology_model.get_mars_differentiation_pressure(),
        #     geology_model.get_mars_oxygen_fugacity()
        # )
        # mix = geology_model.get_earth_mix(fcf, abundances)
...
def generate_observations(
    mix_dict, high_pressure_idealisation=False, low_pressure_idealisation=False
):
    idealisation_shifts = {
        "High Pressure": {
            "Mantle": {
    ...
    for fcf, mix in mix_dict.items():
        ...
        for element in ci.usual_elements:
            ...
            if high_pressure_idealisation:
                if fcf < 0.17:
                    obs_value += idealisation_shifts["High Pressure"]["Mantle"].get(
                        element, 0
                    )
                else:
                    obs_value += idealisation_shifts["High Pressure"]["Core"].get(
                        element, 0
                    )
            if low_pressure_idealisation:
                if fcf < 0.17:
                    obs_value += idealisation_shifts["Low Pressure"]["Mantle"].get(
                        element, 0
                    )
                else:
                    obs_value += idealisation_shifts["Low Pressure"]["Core"].get(
                        element, 0
                    )
...
def plot_mixes(mix_dict):
    ...
    for fcf, mix in mix_dict.items():
        ...
        for el in ci.usual_elements:  # [O, Si, Fe, Ni, Mg]:
            if mix[el] > 0.02:
                modified_mix_dict[el] = mix[el]
        modified_mix_dict[ci.Element.Placeholder] = 1 - sum(
            x[1] for x in modified_mix_dict.items()
        )
...
def reproduce_bulk_core_mantle_crust():
    test_composition = {
        ...
        "Mantle": {
            ci.Element.Al: 0.018,
            ci.Element.Ti: (1200 / 1000000) / 2,
            # division by 2 is an approxi^^mate correction for weight -> number
            ci.Element.Ca: 0.013,
            ci.Element.Ni: 0.001,
            ci.Element.Fe: 0.024,
            ci.Element.Cr: 0.001,
