...
wd_dict = {
    "SDSSJ1228+1040Corr": {  # UV, ID 365
        ...
        "UpperBounds": {ci.Element.Ni: True, ci.Element.Cr: True},
    },
    ...
    "PG1225-079Corr": {  # ID 380
        ...
        "UpperBounds": {ci.Element.Al: True, ci.Element.Na: True, ci.Element.O: True},
    },
    ...
    "PG1015+161Corr": {  # Gaensicke, ID 356
        ...
        "UpperBounds": {ci.Element.Cr: True, ci.Element.C: True},
    },
}


# Use forward = True if you know the intrinsic (ie planetesimal) composition and want to
# find out what you would observe if you put that material into a WD (in steady state)
...
def find_steady_state_adjusted_abundances(
    planetesimal_abundance_dict, HorHe, logg, Teff, CaHe=None, forward=True
):
    ...
    for el in planetesimal_abundance_dict:
        if el not in ci.usual_elements:
            all_timescales = True
    sinking_timescales = timescale_interpolator.get_wd_timescales(
        HorHe, logg, Teff, CaHe, all_timescales
    )
    total = 0
    toret = dict()
    ...
    with open(wd_data_filename, encoding="utf-8") as wdcsv:
        ...
        for row in csv.reader(wdcsv):
            ...
            if row_count == 0:  # Heading row
                ...
                output_dict[wd_name]["Properties"][mp.WDParameter.spectral_type] = row[
                    1
                ]
                output_dict[wd_name]["Properties"][mp.WDParameter.temperature] = int(
                    row[4]
                )
                ...
                for el in ci.usual_elements:
                    ...
                    try:
                        ...
                        output_dict[wd_name]["Errors"][el] = float(
                            row[input_error_index]
                        )
                    except ValueError:
                        ...
                        if row[input_abundance_index].startswith("<"):
                            ...
                            output_dict[wd_name]["Abundances"][el] = float(
                                row[input_abundance_index][1:]
                            )
...
def batch_convert(wd_dict):
    ...
    for wd_name, wd_info in wd_dict.items():
        ...
        planetesimal_abundance_dict = geo_model.convert_log_abundances_to_linear(
            wd_abundances
        )
        ca_abundance = wd_abundances.get(ci.Element.Ca, default_Ca)
        adjusted_planetesimal_dict = find_steady_state_adjusted_abundances(
            planetesimal_abundance_dict, HorHe, logg, Teff, ca_abundance, forward
        )
        ss_wd_abundances = geo_model.convert_linear_abundances_to_log(
            adjusted_planetesimal_dict, pol_frac
        )
        ...
        present_ss_abundances = {
            el: adjusted_planetesimal_dict[el]
            for el in all_wd_ss_number_abundances_dict[wd_name]
        }
        all_wd_ss_wt_linear_abundances_dict[wd_name] = (
            geo_model.convert_number_abundances_to_mass(present_ss_abundances)
        )
    systems_to_print = [
        "G29-38Corr",
        "GaiaJ0644-0352",
    ...
    for system in systems_to_print:
        ...
        print(
            100
            * all_wd_ss_wt_linear_abundances_dict[system][ci.Element.Fe][gi.Layer.bulk]
        )
        print(
            100
            * all_wd_ss_wt_linear_abundances_dict[system][ci.Element.Mg][gi.Layer.bulk]
        )
        print(
            100
            * all_wd_ss_wt_linear_abundances_dict[system][ci.Element.Si][gi.Layer.bulk]
        )
    make_plot = False
    if make_plot:
        graph_fac = gf.GraphFactory()
        graph_fac.make_composition_plot_multi_system(
            "testmulti", all_wd_abundances_dict, all_errors_dict, upper_limits_dict
        )
...
def example():
    ...
    if (
        (not override_with_synthetic)
        and (not override_with_real)
        and (not override_with_ideal)
    ):
        planetesimal_abundance_dict = geo_model.convert_log_abundances_to_linear(
            wd_abundances
        )
        ...
        if override_with_synthetic:
            if planet == "Earth":
                raw_abundances = geo_model.form_a_planet_iteratively(
                    geo_model.get_earth_differentiation_pressure(),
                    geo_model.get_earth_oxygen_fugacity(),
                )[0]
            if planet == "Mars":
                raw_abundances = geo_model.form_a_planet_iteratively(
                    geo_model.get_mars_differentiation_pressure(),
                    geo_model.get_mars_oxygen_fugacity(),
                )[0]
    ...
    adjusted_planetesimal_dict = find_steady_state_adjusted_abundances(
        planetesimal_abundance_dict, HorHe, logg, Teff, wd_abundances[ci.Element.Ca]
    )
    print()
    ...
    wt_adjusted_planetesimal_dict = geo_model.convert_number_abundances_to_mass(
        adjusted_planetesimal_dict
    )
    print()
