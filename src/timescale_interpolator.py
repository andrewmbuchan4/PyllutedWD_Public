...
class TimescaleInterpolator:
    ...
    def __init__(self):
        ...
        for timescale_name in self.all_timescale_data.keys():
            self.expected_vals[timescale_name] = {
                ci.Element.H: dict(),
                ci.Element.He: dict(),
            }
    ...
    def perform_format(input_file_name, output_file_name):
        ...
        with open(input_file_name) as input_file, open(
            ...
            # ^ Intentionally using a delimiter that isn't present
            to_write = csv.writer(output_file)
            for row in read:
                try:
                    line = row[0].lstrip()
                    # ^ This should contain the whole line as a str, minus leading
                    # whitespace
                except IndexError:
                    # Row was empty
                    continue
                if line.startswith("EL"):
                    ...
                    if not header_written:
                        # Write a header saying what values of log(g) and overshoot this
                        # corresponds to
                    ...
                    # ^ This line is actually showing the [Ca/He] values
                    cahe_values = line.replace("  ", " ").split(" ")
                    cahe_values[0] = "CaHe"
                    to_write.writerow(cahe_values)
                elif line.startswith("qcvz"):
                    # ^ This line is showing the log(q) values
    ...
    def perform_format_abed(input_file_name):
        # NB These files contain all log(g) values in one, whereas we need a separate
        # file for each
        ...
        with open(input_file_name) as input_file:
            ...
            # to_write = csv.writer(output_file)
            for row in read:
                # Strategy is to read each line up to the next blank line, saving
                # strings and numbers in 2 separate lists, then pair them afterwards
                if len(row) == 0:
    ...
    def get_csv_format_type(self, timescale_type, HorHe):
        # Different csv files are formatted differently - this just returns a flag to
        # let us know how the particular request csv is formatted
    ...
    def get_values_for_variable(self, timescale_name, HorHe, variable):
        if variable == "g":
            ...
            vals = np.array(
                list(
                    self.all_timescale_data[timescale_name][HorHe][
                        self.get_arbitrary_val(timescale_name, HorHe, "g")
                    ].keys()
                )
            )
        elif variable == "c":
            vals_list = list(
                self.all_timescale_data[timescale_name][HorHe][
                    self.get_arbitrary_val(timescale_name, HorHe, "g")
                ][self.get_arbitrary_val(timescale_name, HorHe, "t")].keys()
            )
            if "logq" in vals_list:
                # Then this was a file set which didn't have CaHe as a variable! We went
    ...
    def set_up_interpolators(self):
        # This assumes that self.timescale_data has no missing keys anywhere or anything
        # like that
        ...
        for timescale_name, timescale_data in self.all_timescale_data.items():
            ...
            for HorHe in timescale_data.keys():
                ...
                if c_vals is None:
                    ...
                    for element in timescale_data[HorHe][
                        self.get_arbitrary_val(timescale_name, HorHe, "g")
                    ][self.get_arbitrary_val(timescale_name, HorHe, "t")].keys():
                        grid_vals = np.zeros((len(g_vals), len(t_vals)))
                        for i in range(len(g_vals)):
                            for j in range(len(t_vals)):
                                grid_vals[i, j] = timescale_data[HorHe][g_vals[i]][
                                    t_vals[j]
                                ][element]
                        # 'linear', False, None means linear interpolation, no error if
                        # we go out of bounds, extrapolate in that case
                        self.interpolators[timescale_name][HorHe][element] = (
                            si.RegularGridInterpolator(
                                (g_vals, t_vals), grid_vals, "linear", False, None
                            )
                        )
                else:
                    for element in timescale_data[HorHe][
                        self.get_arbitrary_val(timescale_name, HorHe, "g")
                    ][self.get_arbitrary_val(timescale_name, HorHe, "t")][
                        self.get_arbitrary_val(timescale_name, HorHe, "c")
                    ].keys():
                        grid_vals = np.zeros((len(g_vals), len(t_vals), len(c_vals)))
                        for i in range(len(g_vals)):
                            for j in range(len(t_vals)):
                                for k in range(len(c_vals)):
                                    grid_vals[i, j, k] = timescale_data[HorHe][
                                        g_vals[i]
                                    ][t_vals[j]][c_vals[k]][element]
                        # 'linear', False, None means linear interpolation, no error if
                        # we go out of bounds, extrapolate in that case
                        self.interpolators[timescale_name][HorHe][element] = (
                            si.RegularGridInterpolator(
                                (g_vals, t_vals, c_vals),
                                grid_vals,
                                "linear",
                                False,
                                None,
    ...
    def load_data(self):
        toret = dict()
        # Structure to build: data[HorHe][g][Teff][CaHe] = {
        #   'logq': <q>, Element.Li: <t_Li>, ... Element.Zn: <t_Zn>
        # }
                                ...
                                for row in read:
                                    if i == 0:
                                        ...
                                        for j in range(1, len(row)):
                                            toret[timescale_set_name][HorHe][g][
                                                int(T_vals[j - 1])
                                            ] = dict()
                                    elif i == 1:
                                        q_vals = row[1:]
                                ...
                                for row in read:
                                    if row[0] == "Teff":
                                        ...
                                        for j in range(1, len(row)):
                                            toret[timescale_set_name][HorHe][g][
                                                current_Teff
                                            ][float(current_CaHe_vals[j - 1])] = dict()
                                    elif row[0] == "qcvz":
                                        q_vals = row[1:]
                                        for j in range(1, len(row)):
                                            toret[timescale_set_name][HorHe][g][
                                                current_Teff
                                            ][float(current_CaHe_vals[j - 1])][
                                                "logq"
                                            ] = float(
                                                q_vals[j - 1]
                                            )
    ...
    def load_3d_patch(self, timescale_data):
        to_add = dict()
        for g_val in timescale_data[TimescaleType.Bedard3DOvershoot][
            ci.Element.H
        ].keys():
            to_add[g_val] = dict()
            for temp, data_vals in timescale_data[TimescaleType.Bedard3DOvershoot][
                ci.Element.H
            ][g_val].items():
                to_add[g_val][temp] = data_vals
            for temp, data_vals in timescale_data[
                TimescaleType.BedardVariableOvershoot
            ][ci.Element.H][g_val].items():
    ...
    def load_wd_data(self, wd_data_file="WDInputData.csv"):
        ...
        with open(
            pu.get_path_to_data() + pu.get_wd_input_file(), encoding="utf-8"
        ) as wdcsv:
    ...
    def extract_timescales(self, HorHe, logg, Teff, CaHe=None):
        ...
        for timescale_name, timescale_grids in self.all_timescale_data.items():
            try:
                # Firstly, we check to see if we happen to land on an exact grid point
                # (i.e., no need to interpolate)
                if self.get_csv_format_type(timescale_name, HorHe) == 1:
                    toret[timescale_name] = timescale_grids[timescale_name][HorHe][
                        logg
                    ][Teff][CaHe]
                else:
                    toret[timescale_name] = timescale_grids[timescale_name][HorHe][
            ...
            if (
                HorHe not in self.interpolators[timescale_name].keys()
                or self.interpolators[timescale_name][HorHe] == dict()
            ):
                # print(
                #     'Warning! Could not interpolate. Did not find an interpolator for'
                #     + str(HorHe)
                # )
            ...
            if self.get_csv_format_type(timescale_name, HorHe) == 1:
                if CaHe is None:
                    ...
                    try:
                        ...
                        toret[timescale_name] = None
                        continue
                    for element in timescale_grid[
                        self.get_arbitrary_val(timescale_name, HorHe, "g")
                    ][self.get_arbitrary_val(timescale_name, HorHe, "t")][
                        self.get_arbitrary_val(timescale_name, HorHe, "c")
                    ].keys():
                        interpolator = self.interpolators[timescale_name][HorHe][
                            element
                        ]
                        point_to_sample = np.array([[logg, Teff, CaHe]])
                        toret[timescale_name][element] = interpolator(point_to_sample)[
                            0
                        ]
                        # ^ The float value is wrapped in a numpy array so indexing [0]
                        # extracts what we actually want
            else:
                try:
                    timescale_grid = timescale_grids[HorHe]
                except KeyError:
                ...
                for element in timescale_grid[
                    ...
                    toret[timescale_name][element] = interpolator(point_to_sample)[0]
                    # ^ The float value is wrapped in a numpy array so indexing [0]
                    # extracts what we actually want
        return toret

    def get_wd_timescales(self, HorHe, logg, Teff, CaHe=None, all_timescales=False):
        # TODO: could HorHe be a ci.Element rather than a str?
        timescales = self.extract_timescales(HorHe, logg, Teff, CaHe)
    ...
    def return_wd_timescales_as_dict(self, wd_entry, all_timescales=False):
        ...
        for timescale_name in self.all_timescale_data.keys():
            if wd_entry[timescale_name] is None:
                ...
                toret[timescale_name] = {"logq": wd_entry[timescale_name]["logq"]}
                # TODO: replace with mp.WDParameter.logq?
                for el in ci.all_elements:
                    if all_timescales or el in ci.usual_elements:
                        try:
                            toret[timescale_name][el] = (
                                10 ** wd_entry[timescale_name][el]
                            )
                        except KeyError:
                            pass
        return toret
                    ...
                    for timescale_name, wd_entry in timescale_data.items():
                        if wd_entry is None:
                            ...
                            output_row = (
                                [wd_name]
                                + [timescale_name]
                                + self.return_wd_timescales_as_list(wd_entry)
                            )
    ...
    for Hx in Hx_values:
        ...
        for logg in logg_values:
            ...
            for teff in teff_values:
                ...
                timescales_e = timescale_interpolator.get_wd_timescales(
                    Hx, logg, teff, CaHe
                )
                for test_el in elements:
                    timescales[Hx][logg][teff][test_el] = timescales_e[timescale_type][
                        test_el
                    ]
    for Hx in Hx_values:
        print()
        ...
        for element in elements:
            ...
            for logg in logg_values:
                row_to_write = [str(logg)] + [
                    str(timescales[Hx][logg][teff][element]) for teff in teff_values
                ]
...
def main():
    ...
    #     print(timescale_type)
    #     print(
    #         result[timescale_type][ci.Element.Mg]/result[timescale_type][ci.Element.Si]
    #     )
    #     print(
    #         result[timescale_type][ci.Element.Fe]/result[timescale_type][ci.Element.Si]
    #     )
    #     print(
    #         result[timescale_type][ci.Element.Mg]/result[timescale_type][ci.Element.Fe]
    #     )
