...
def main():
    ...
    highFe = {
        ci.Element.Fe: {gi.Layer.bulk: 0.4005},
        ci.Element.O: {gi.Layer.bulk: 0.5995},
    }
    pressure_vals = [0.001, 0.01, 0.1, 0.5] + list(range(1, max_pressure + 1, 1))
    for p in pressure_vals:
        r1, m1 = geo_model.calculate_radius_and_mass(
            p, geo_model.element_info, cmb_pressure
        )
