        ...
        if comp[ci.Element.Mg] != 0:
            ...
            for element, el_abundance in comp.items():
                if element != ci.Element.Mg:
                    ...
                    XMg_normalised = (
                        XMg - geo_model_for_solar_abundances.solar_abundances[element]
                    )
...
def main():
    ...
    test_composition = {
        ...
        "Mantle": {
            ...
            # ^ Division by 2 is an approximate correction for weight -> number
            ci.Element.Ca: 0.013,
            ci.Element.Ni: 0.001,
            ci.Element.Fe: 0.024,
            ci.Element.Cr: 0.001,
    ...
    for el in ci.usual_elements:
        test_composition["Impure Core"][el] = (
            impure_core_frac * test_composition["Core"][el]
        ) + ((1 - impure_core_frac) * test_composition["Mantle"][el])
        test_composition["Impure Mantle"][el] = (
            impure_mantle_frac * test_composition["Mantle"][el]
        ) + ((1 - impure_mantle_frac) * test_composition["Core"][el])
    all_wd_abundances = {
        ci.Element.Al: -6,
        ci.Element.Ti: -6,
        ci.Element.Ca: -6,
