...
def generate_fO2_vals():
    # Reducing range to avoid non-convergence!
    return [-3, -2.8, -2.6, -2.4, -2.2, -2, -1.8, -1.6, -1.4, -1.2, -1]


def generate_fcf_vals():
    return np.arange(0, 0.02, 0.001).tolist() + np.arange(0.02, 1.02, 0.02).tolist()
...
def form_planets(geo_model, start_from_prev_result=False):
    ...
    for fO2 in generate_fO2_vals():
        ...
        for pressure in generate_pressure_vals():
            ...
            print(pressure)
            print(fO2)
            abundances, cnf, Ds, all_Ds = geo_model.form_a_planet_iteratively(
                pressure,
                fO2,
                None,
                None,
                cnf if start_from_prev_result else None,
                Ds if start_from_prev_result else None,
                True,
            )
            # abundances, cnf, Ds, all_Ds = geo_model.form_a_planet_iteratively(
            #     pressure,
            ...
            if abundances is None:
                ...
                for element in abundances.keys():
                    ...
                    if prev_Ds is None or prev_cnf is None or prev_P is None:
                        critical_fragment_core_fraction_dict[(pressure, fO2)][
                            element
                        ] = None
                        ...
                        f_crit = geo_model.get_critical_fragment_core_fraction(
                            Ds[element], cnf, dD_dP, dcnf_dP
                        )
                        critical_fragment_core_fraction_dict[(pressure, fO2)][
                            element
                        ] = f_crit
                        for fcf in generate_fcf_vals():
                            if dlogX_dP_dict[(pressure, fO2)].get(fcf) is None:
                                dlogX_dP_dict[(pressure, fO2)][fcf] = dict()
                            dlogX_dP = geo_model.get_dlogX_dV(
                                Ds[element], cnf, fcf, dD_dP, dcnf_dP
                            )
    ...
    return (
        ...
        planet_cnf_dict,
        planet_Ds_dict,
        critical_fragment_core_fraction_dict,
        dlogX_dP_dict,
    )
...
def form_planets_with_variable_sulfur(geo_model, pressure=None, fO2=None):
    ...
    geo_model.reinit()
    # ^ This ensures that the element_info is returned from an unknown state to the
    # default
...
def plot_wd_comparisons(planet_dict, tag):
    ...
    for wd, wd_props in wd_dict.items():
        log_crfe_array = np.zeros(
            (len(generate_fO2_vals()), len(generate_pressure_vals()))
        )
        log_nife_array = np.zeros(
            (len(generate_fO2_vals()), len(generate_pressure_vals()))
        )
        log_crmg_array = np.zeros(
            (len(generate_fO2_vals()), len(generate_pressure_vals()))
        )
        log_nimg_array = np.zeros(
            (len(generate_fO2_vals()), len(generate_pressure_vals()))
        )
        for f_index, f in enumerate(generate_fO2_vals()):
            for p_index, p in enumerate(generate_pressure_vals()):
                ...
                try:
                    wd_abundances = geo_model.find_system_specific_abundances(
                        planet_dict[(p, f)], wd_props["cnf"]
                    )
                    log_crfe_array[f_index][p_index] = np.log10(
                        wd_abundances[ci.Element.Cr] / wd_abundances[ci.Element.Fe]
                    )
                    log_nife_array[f_index][p_index] = np.log10(
                        wd_abundances[ci.Element.Ni] / wd_abundances[ci.Element.Fe]
                    )
                    log_crmg_array[f_index][p_index] = np.log10(
                        wd_abundances[ci.Element.Cr] / wd_abundances[ci.Element.Mg]
                    )
                    log_nimg_array[f_index][p_index] = np.log10(
                        wd_abundances[ci.Element.Ni] / wd_abundances[ci.Element.Mg]
                    )
                except KeyError:
        ...
        wd_props["log_nimg_array"] = log_nimg_array
        # TODO generate the 4 pairs dynamically...
    for wd, wd_props in wd_dict.items():
        ...
        for p_index, p in enumerate(generate_pressure_vals()):
            variable_p_log_crfe.append(
                wd_props["log_crfe_array"][earth_fO2_index][p_index]
            )
            variable_p_log_nife.append(
                wd_props["log_nife_array"][earth_fO2_index][p_index]
            )
            variable_p_log_crmg.append(
                wd_props["log_crmg_array"][earth_fO2_index][p_index]
            )
            variable_p_log_nimg.append(
                wd_props["log_nimg_array"][earth_fO2_index][p_index]
            )
        ...
        for f_index, f in enumerate(generate_fO2_vals()):
            variable_f_log_crfe.append(
                wd_props["log_crfe_array"][f_index][earth_pressure_index]
            )
            variable_f_log_nife.append(
                wd_props["log_nife_array"][f_index][earth_pressure_index]
            )
            variable_f_log_crmg.append(
                wd_props["log_crmg_array"][f_index][earth_pressure_index]
            )
            variable_f_log_nimg.append(
                wd_props["log_nimg_array"][f_index][earth_pressure_index]
            )
    ...
    for wd, wd_props in wd_dict.items():
        ...
        graph_fac.plot_log_elel_ratio(
            ...
            earth_fO2,
            colour_dict,
            ignore_lack_of_observations,
        )
        graph_fac.plot_log_elel_ratio(
            wd_props["variable_p_log_nife"],
            generate_pressure_vals(),
            "Pressure",
            wd,
            ci.Element.Ni,
            ci.Element.Fe,
            wd_props["cnf"],
            wd_props["observed_log_elfe_ratios"][ci.Element.Ni],
            wd_props["observed_log_elfe_ratio_errors"][ci.Element.Ni],
            tag,
            earth_fO2,
            colour_dict,
            ignore_lack_of_observations,
        ...
        graph_fac.plot_log_elel_ratio(
            ...
            ci.Element.Mg,
            wd_props["cnf"],
            wd_props["observed_log_elmg_ratios"][ci.Element.Cr],
            wd_props["observed_log_elmg_ratio_errors"][ci.Element.Cr],
            tag,
            earth_fO2,
            colour_dict,
            ignore_lack_of_observations,
        ...
        graph_fac.plot_log_elel_ratio(
            ...
            ci.Element.Mg,
            wd_props["cnf"],
            wd_props["observed_log_elmg_ratios"][ci.Element.Ni],
            wd_props["observed_log_elmg_ratio_errors"][ci.Element.Ni],
            tag,
            earth_fO2,
            colour_dict,
            ignore_lack_of_observations,
...
def extract_stacked_abundances(
    ...
    bulk_stacked_variable_p_vals = {gi.Layer.core: dict(), gi.Layer.mantle: dict()}
    # ^ These will be weighted by core/mantle number fraction at a particular
    # pressure/fO2
    # Assume they all have the same number of data points...
    # we should bail if this isn't true
    weighted_stacked_variable_offset = [0] * len(variable_p_vals_core[ci.Element.Fe])
    for layer in [gi.Layer.core, gi.Layer.mantle]:
        ref_vals = (
            variable_p_vals_core if layer == gi.Layer.core else variable_p_vals_mantle
        )
        # Assume they all have the same number of data points...
        # we should bail if this isn't true
        stacked_variable_offset = [0] * len(ref_vals[ci.Element.Fe])
        for sne in stacked_named_elements[layer]:
            # If the abundance is None for one element, it should be None for all of
            # them
            data = [
                sum(a) if a[1] is not None else None
                for a in zip(stacked_variable_offset, ref_vals[sne])
            ]
            stacked_variable_p_vals[layer][sne] = data
            stacked_variable_offset = data
            if layer == gi.Layer.core:
                # Multiply by cnf at that pressure. The catch is that each ref val
                # corresponds to a different pressure
                weighted_ref_vals = [
                    (
                        r * planet_cnf_dict[
                            (generate_pressure_vals()[p_index], earth_fO2)
                        ]
                        if r is not None
                        else None
                    )
                    for p_index, r in enumerate(ref_vals[sne])
                ]
            else:
                # Multiply by 1 - cnf at that pressure
                weighted_ref_vals = [
                    (
                        r * (
                            1 - planet_cnf_dict[
            ...
            weighted_data = [
                sum(a) if a[1] is not None else None
                for a in zip(weighted_stacked_variable_offset, weighted_ref_vals)
            ]
        ...
        for ele, abundance in ref_vals.items():
            if ele not in stacked_named_elements[layer]:
                stacked_variable_offset = [
                    sum(a) if a[1] is not None else None
                    for a in zip(stacked_variable_offset, abundance)
                ]
                if layer == gi.Layer.core:
                    # Multiply by cnf at that pressure. The catch is that each ref val
                    # corresponds to a different pressure
                    weighted_ref_vals = [
                        ...
                        for p_index, r in enumerate(abundance)
                    ]
                else:
                    # Multiply by 1 - cnf at that pressure
                    weighted_ref_vals = [
                ...
                weighted_stacked_variable_offset = [
                    sum(a) if a[1] is not None else None
                    for a in zip(weighted_stacked_variable_offset, weighted_ref_vals)
                ]
        stacked_variable_p_vals[layer][ci.Element.Placeholder] = stacked_variable_offset
        bulk_stacked_variable_p_vals[layer][
            ci.Element.Placeholder
        ] = weighted_stacked_variable_offset

    stacked_variable_fO2_vals = {gi.Layer.core: dict(), gi.Layer.mantle: dict()}
    bulk_stacked_variable_fO2_vals = {gi.Layer.core: dict(), gi.Layer.mantle: dict()}
    # ^ These will be weighted by core/mantle number fraction at a particular pressure/fO2
    # Assume they all have the same number of data points...
    # we should bail if this isn't true
    weighted_stacked_variable_offset = [0] * len(variable_fO2_vals_core[ci.Element.Fe])
    for layer in [gi.Layer.core, gi.Layer.mantle]:
        ref_vals = (
            variable_fO2_vals_core
            if layer == gi.Layer.core
            else variable_fO2_vals_mantle
        )
        # Assume they all have the same number of data points...
        # we should bail if this isn't true
        stacked_variable_offset = [0] * len(ref_vals[ci.Element.Fe])
        for sne in stacked_named_elements[layer]:
            ...
            if layer == gi.Layer.core:
                ...
                weighted_ref_vals = [
                    (
                        r * planet_cnf_dict[
                            (earth_pressure, generate_fO2_vals()[fO2_index])
                        ]
                        if r is not None
                        else None
                    )
                    for fO2_index, r in enumerate(ref_vals[sne])
                ]
            else:
                # Multiply by 1 - cnf at that pressure
                weighted_ref_vals = [
            ...
            if ele not in stacked_named_elements[layer]:
                ...
                if layer == gi.Layer.core:
                    ...
                    weighted_ref_vals = [
                        ...
                        for fO2_index, r in enumerate(abundance)
                    ]
                else:
                    # Multiply by 1 - cnf at that pressure
                    weighted_ref_vals = [
        ...
        stacked_variable_fO2_vals[layer][
            ci.Element.Placeholder
        ] = stacked_variable_offset
        bulk_stacked_variable_fO2_vals[layer][
            ci.Element.Placeholder
        ] = weighted_stacked_variable_offset
...
def main():
    ...
    for element in elements:
        ...
        observed_earth_core_abundances[element] = list()
        # The next four lists don't really need to be lists!
    ...
    for element in elements:
        ...
        for p in dlogX_dP_pressures:
            ...
            for fcf in generate_fcf_vals():
                variable_dlogX_dP_dict[element][p].append(
                    dlogX_dP_dict[(p, earth_fO2)][fcf][element]
                )
    ...
    for element in elements:
        try:
            ...
            if ca is not None:
                observed_earth_core_abundances[element].append(
                    geo_model.get_core_abundance(element)
                )
                observed_earth_core_abundances_rel_iron[element].append(
                    geo_model.get_core_abundance(element)
                    / geo_model.get_core_abundance(ci.Element.Fe)
                )
            if ma is not None:
                observed_earth_mantle_abundances[element].append(
                    geo_model.get_mantle_abundance(element)
                )
                observed_earth_mantle_abundances_rel_iron[element].append(
                    geo_model.get_mantle_abundance(element)
                    / geo_model.get_mantle_abundance(ci.Element.Fe)
                )
        except (KeyError, TypeError) as e:
            # Then we don't have information for this element
            pass

    major_elements_for_conv = [
        ci.Element.Hf,
        ci.Element.Ta,
        ci.Element.Nb,
        ci.Element.Si,
    ...
    for p in generate_pressure_vals():
        for element in elements:
            if planet_dict.get((p, earth_fO2), None) is not None:
                variable_p_vals_core[element].append(
                    planet_dict[(p, earth_fO2)][element][gi.Layer.core]
                )
                variable_p_vals_mantle[element].append(
                    planet_dict[(p, earth_fO2)][element][gi.Layer.mantle]
                )
                variable_p_vals_core_rel_iron[element].append(
                    planet_dict[(p, earth_fO2)][element][gi.Layer.core]
                    / planet_dict[(p, earth_fO2)][ci.Element.Fe][gi.Layer.core]
                )
                variable_p_vals_mantle_rel_iron[element].append(
                    planet_dict[(p, earth_fO2)][element][gi.Layer.mantle]
                    / planet_dict[(p, earth_fO2)][ci.Element.Fe][gi.Layer.mantle]
                )
                try:
                    variable_fcrit_dict[element].append(
                        critical_fragment_core_fraction_dict[(p, earth_fO2)][element]
                    )
                except KeyError:
                    # ignore Oxygen
                    pass
    ...
    for f in generate_fO2_vals():
        for element in elements:
            try:
                variable_fO2_vals_core[element].append(
                    planet_dict[(earth_pressure, f)][element][gi.Layer.core]
                )
                variable_fO2_vals_mantle[element].append(
                    planet_dict[(earth_pressure, f)][element][gi.Layer.mantle]
                )
                variable_fO2_vals_core_rel_iron[element].append(
                    planet_dict[(earth_pressure, f)][element][gi.Layer.core]
                    / planet_dict[(earth_pressure, f)][ci.Element.Fe][gi.Layer.core]
                )
                variable_fO2_vals_mantle_rel_iron[element].append(
                    planet_dict[(earth_pressure, f)][element][gi.Layer.mantle]
                    / planet_dict[(earth_pressure, f)][ci.Element.Fe][gi.Layer.mantle]
                )
    ...
    for i, mn in enumerate(variable_fO2_vals_mantle[ci.Element.Mn]):
        mn_cr_mantle.append(mn / variable_fO2_vals_mantle[ci.Element.Cr][i])
    print(variable_fO2_vals_core)
    print(variable_fO2_vals_mantle)
    print(variable_fO2_vals_core[ci.Element.Mn])
    print(variable_fO2_vals_core[ci.Element.Cr])
    print(variable_fO2_vals_mantle[ci.Element.Mn])
    print(variable_fO2_vals_mantle[ci.Element.Cr])
    print(mn_cr_core)
    print(mn_cr_mantle)
    raise

    stacked_named_elements_combined = {
        # We want to bundle up minor elements for readability for the bulk plot
    ...
    (
        stacked_variable_p_vals_for_bulk_plots,
        bulk_stacked_variable_p_vals_for_bulk_plots,
        stacked_variable_fO2_vals_for_bulk_plots,
        bulk_stacked_variable_fO2_vals_for_bulk_plots,
    ) = extract_stacked_abundances(
        stacked_named_elements_combined,
        variable_p_vals_core,
        variable_p_vals_mantle,
    ...
    # planet_dict_sulf, planet_cnf_dict_sulf, planet_Ds_dict_sulf, sulf_vals = (
    #     form_planets_with_variable_sulfur(geo_model)
    # )

    # variable_s_vals_core = dict()
    # variable_s_vals_mantle = dict()
    # # all_elements = planet_dict_sulf[sulf_vals[0]].keys()
    # important_s_elements = [
    #     ci.Element.Si,
    #     ci.Element.Cr,
    #     ci.Element.Ni,
    #     ci.Element.Ti,
    ...
    #     for s in sulf_vals:
    #         try:
    #             variable_s_vals_core[element].append(planet_dict_sulf[s][element][
    #                 gi.Layer.core
    #             ])
    #         except KeyError:
    #             variable_s_vals_core[element].append(None)
    #         try:
    #             variable_s_vals_mantle[element].append(planet_dict_sulf[s][element][
    #                 gi.Layer.mantle
    #             ])
    #         except KeyError:
    ...
    #     tag = str(ktp[0]) + '_' + str(ktp[1])
        # try:
        #    graph_fac.make_convergence_plot(
        #        variable_Ds_dict[ktp],
        #        run_tag + '_' + tag, colour_dict
        #    )
    #     except KeyError as e:
    #         print(e)
    #         print(tag)
    ...
    s_tag = "no_sulf"
    # graph_fac.make_planet_formation_plot(
    #     variable_p_vals_core,
    #     'Pressure',
    #     generate_pressure_vals(),
    #     'core',
    #     observed_earth_core_abundances,
    #     run_tag + s_tag
    # )
    # graph_fac.make_planet_formation_plot(
    #     variable_p_vals_mantle,
    ...
    #     generate_pressure_vals(),
    #     'mantle',
    #     observed_earth_mantle_abundances,
    #     run_tag + s_tag
    # )
    # graph_fac.make_planet_formation_plot(
    #     variable_fO2_vals_core,
    ...
    #     run_tag + s_tag
    # )
    # graph_fac.make_planet_cnf_plot(
    #     variable_p_cnf_core,
    #     'Pressure',
    #     generate_pressure_vals(),
    #     observed_cnfs
    ...
    # graph_fac.make_planet_formation_plot(
    #     variable_p_vals_core_rel_iron,
    #     'Pressure',
    #     generate_pressure_vals(),
    #     'core',
    #     observed_earth_core_abundances_rel_iron,
    #     run_tag + s_tag,
    #     'Iron'
    # )
    # graph_fac.make_planet_formation_plot(
    #     variable_p_vals_mantle_rel_iron,
    #     'Pressure',
    #     generate_pressure_vals(),
    #     'mantle',
    #     observed_earth_mantle_abundances_rel_iron,
    #     run_tag + s_tag,
    #     'Iron'
    # )

    # graph_fac.make_earth_composition_plot(
    #     observed_earth_core_abundances,
    #     {
    #         'Model (' + str(earth_pressure) + ' GPa, IW' + str(earth_fO2) + ')':
    #             planet_dict[(earth_pressure, earth_fO2)]
    #     },
    #     gi.Layer.core,
    #     'core' + run_tag + s_tag
    # )
    ...
    #     },
    #     gi.Layer.mantle,
    #     'mantle' + run_tag + s_tag
    # )

    # graph_fac.make_planet_formation_stacked_plot(
    #     stacked_variable_p_vals_for_single_plots[gi.Layer.core],
    #     'Pressure',
    #     generate_pressure_vals(),
    #     'core',
    #     observed_earth_core_abundances,
    ...
    # )
    # graph_fac.make_planet_formation_stacked_plot(
    #     stacked_variable_p_vals_for_single_plots[gi.Layer.mantle],
    #     'Pressure',
    #     generate_pressure_vals(),
    #     'mantle',
    #     observed_earth_mantle_abundances,
    ...
    #     run_tag + s_tag
    # )
    # graph_fac.make_planet_formation_combined_stacked_plot(
    #     bulk_stacked_variable_p_vals_for_bulk_plots,
    #     'Pressure',
    #     generate_pressure_vals(),
    #     'bulk',
    ...
    # graph_fac.make_planet_formation_stacked_plot(
    #     stacked_variable_fO2_vals_for_single_plots[gi.Layer.core],
    #     'fO2',
    #     generate_fO2_vals(),
    #     'core',
    #     observed_earth_core_abundances,
    ...
    # )
    # graph_fac.make_planet_formation_stacked_plot(
    #     stacked_variable_fO2_vals_for_single_plots[gi.Layer.mantle],
    #     'fO2',
    #     generate_fO2_vals(),
    #     'mantle',
    #     observed_earth_mantle_abundances,
    ...
    # plot_wd_comparisons(planet_dict, run_tag)
    # ^ Deprecated: Use plot_isolated_pressure_fO2_effect.py instead
    ...
    # geo_model.tabulate_output(
    #     planet_dict[(earth_pressure, earth_fO2)],
    #     planet_cnf_dict[(earth_pressure, earth_fO2)],
    #     planet_Ds_dict[(earth_pressure, earth_fO2)][-1:][0],
    #     'test_out'
    # )
