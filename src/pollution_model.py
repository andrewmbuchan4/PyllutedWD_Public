...
class PollutionModel:
    ...
    def __init__(self, basename, timescale_type, enhancement_model_name, prior_name, consider_thermohaline=False, live_points=2000, seed=-1, verbose=True, resume=True):
    ...
    def get_prefix(self):
        return self.timescale_type.short_str() + '_' + self.get_identifier1() + self.basename + self.get_identifier2()
    ...
    def execute_pymultinest(self, output_dir):
        ...
        if not disable_filename_error and len(self.get_full_prefix(output_dir)) > self.get_max_file_length():
            # TODO: In this case, we can now make use of the new functions in the WhiteDwarf class to abbreviate the system_name and variant!
            raise IOError('\nOutput path ' + self.get_full_prefix(output_dir) + ' is too long!\nLength was ' + str(len(self.get_full_prefix(output_dir))) + ', but max length is ' + str(max_file_length) + ' so that the full names fit in 100 characters\nThe 100 character limit is hardcoded in MultiNest v3.10.\nIf you are using MultiNest 3.11 or later, you should be able to just disable this error (go to execute_pymultinest in ' + str(Path(__file__).resolve()) + ')\n(This is untested though!)')
    ...
    def execute_pocomc(self, output_dir):
        ...
        sampler = pmc.Sampler(
            ...
            #vectorize=False, Switching this to True could be good for speed-up purposes, but requires some rewiring...
            random_state=0, # This should be self.seed - pocomc doesn't accept the default value of -1 as a valid input unlike pymultinest though
            dynamic=True, # Seems good? Dynamically adjusts particle count as necessary
            output_dir=output_dir,
            output_label=self.get_prefix()
        )
        # To do checkpointing, need to call resume_state_path = "states/pmc_final.state" or pmc_i.state where final should be preferred, otherwise higher values of i
