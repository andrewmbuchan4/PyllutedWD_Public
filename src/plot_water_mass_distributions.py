...
def main():
    ...
    wc_to_plot_dict_log = {"Wet_Log": wet_log, "Dry_Log": dry_log}
    ...
    for pop_name, vals in wc_to_plot_dict_linear.items():
        heights, bins2 = np.histogram(vals, linear_bins, density=True)
    ...
    for pop_name, vals in wc_to_plot_dict_log.items():
        heights, bins2 = np.histogram(vals, log_bins, density=True)
