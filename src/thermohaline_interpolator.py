...
class ThermohalineInterpolator():
    # Partially based on Keith Williams' solution: https://stackoverflow.com/questions/20516762/extrapolate-with-linearndinterpolator
    # But with a different solution for extrapolation since the nearest neighbour solution had weird behaviour
    ...
    def setup(self):
        # Setting up a 3D interpolator that finds logMCa_therm as a function of logMdot, Teff and logg
        # And a 3D interpolator that finds logMCa_diff as a function of logMdot, Teff and logg
        # And a 3D interpolator that finds therm_factor as a function of logMdot, Teff and logg
    ...
    def compile_MESA_outputs(self, base_path):
        ...
        all_teffs = np.array([])
        all_logg = np.array([])
        all_therm_factors = np.array([]) # We will use this array to save the thermohaline correction factor, on a log scale. Generally negative - thermohaline mixing decreases surface mass
        for subdirname in subdirnames:
            ...
            for Teff_file_name in Teff_file_names:
                ...
                logXCa_therm = np.log10(d[:,1]) # This is the Ca surface abundance with thermohaline mixing
                logXCa_diff = np.log10(d[:,2]) # This is the Ca surface abundance without thermohaline mixing, just diffusion
                teff = d[:,3]
                logg = d[:,4]
                ...
                if not np.isnan(therm_factors).any():
                    ...
                    print('This logic is hopefully irrelevant now - there used to be some invalid values in the raw tables')
        ...
        with open(output_file_name, 'w', newline='', encoding='utf-8') as output_file:
            ...
            to_write.writerow(['log(Mdot)', 'Teff', 'log(g)', 'log(XCa)_therm', 'log(XCa)_diff', 'Therm_factor'])
            i = 0
            while i < len(all_logmdots):
                if np.isnan(all_therm_factors[i]):
                    ...
                    raise
                to_write.writerow([all_logmdots[i], all_teffs[i], all_logg[i], all_logXCa_therms[i], all_logXCa_diffs[i], all_therm_factors[i]])
                i += 1
    ...
    def find_accretion_rate_limits(self, Teff, logg):
        if (Teff, logg) in self.accretion_rate_limits_dict:
            ...
            # At a given Teff/logg, there will be a maximum and a mininum value of logMdot for which interpolation (rather than extrapolation) is possible - we want to find these limits (and cache them)
            # Then if we come across a logMdot outside these limits, snap to the closest limit
            # Returns a tuple: (minimum accretion rate in log Mdot, maximum accretion rate in log Mdot)
            ...
            iterations_to_try = 100 # The parameter space will halve each iteration, so this effectively sets the tolerance level. 100 seems like ample
            ...
            while iteration_count < iterations_to_try:
                trial_upper_logmdot = (current_upper_maximum_logmdot_limit + current_lower_maximum_logmdot_limit)/2
                trial_lower_logmdot = (current_upper_minimum_logmdot_limit + current_lower_minimum_logmdot_limit)/2
                interpolated_upper_result = self.interpolate((trial_upper_logmdot, Teff, logg))
                interpolated_lower_result = self.interpolate((trial_lower_logmdot, Teff, logg))
                if np.isnan(interpolated_upper_result):
                # Then we were out of bounds, and the trial value becomes the new upper limit:
                ...
                if np.isnan(interpolated_lower_result):
                    # Then we were out of bounds, and the trial value becomes the new lower limit:
            ...
            if valid_min_limit and valid_max_limit:
                self.accretion_rate_limits_dict[(Teff, logg)] = current_upper_minimum_logmdot_limit, current_lower_maximum_logmdot_limit # Conservatively these as the min/max values (to make sure we can only ever interpolate within bounds). There's some inefficiency here (i.e. some info from final iteration can potentially be unused) but I'm not too concerned
            else:
    ...
    def find_max_temperature(self, logg):
        if logg in self.temperature_limits_dict:
            ...
            lower_bound = 15000
            upper_bound = 25000 # I think it should be safe to assume that the max temperature will always be between 15000 and 25000K
            ...
            while iteration_count < iterations_to_try:
                ...
                interpolated_upper_result_1 = self.interpolate((low_logmdot_edge, trial_upper_bound, logg))
                interpolated_upper_result_2 = self.interpolate((high_logmdot_edge, trial_upper_bound, logg)) # Need BOTH of these to be valid
                if np.isnan(interpolated_upper_result_1) or np.isnan(interpolated_upper_result_2):
                    # TOO HOT! we were out of bounds, and the trial value becomes the new upper limit:
                ...
                iteration_count += 1
            if valid_max_limit:
                self.temperature_limits_dict[logg] = current_lower_bound # Conservatively use the lower limit (to make sure we can only ever interpolate within bounds)
            else:
    ...
    def __call__(self, *args):
        ...
        # We firstly need to find the max usable teff, for this log(g) - define this as the highest teff which gives
        # valid therm factors at both logmdot = 4 and logmdot = 11 (basically the full range of the grid). Then Step 1 is to snap to this teff if we're above it.
        # After that, proceed as before
        max_teff = self.find_max_temperature(logg)
        if Teff > max_teff:
            Teff = max_teff

        min_accretion_rate, max_accretion_rate = self.find_accretion_rate_limits(Teff, logg)
        if logMdot > max_accretion_rate:
            ...
            # We'll do something v simple! Take two points and interpolate linearly between them (BUT only for low Mdot!!!)
            point1_mdot = min_accretion_rate
            point1_therm_factor = self.interpolate((min_accretion_rate, Teff, logg))

            point2_mdot = 0 #
            point2_therm_factor = 0.0 # By assumption (i.e., we'll force thermohaline mixing to be completely inactive for logMdot = 0)

            if logMdot < point2_mdot:
                return 0.0

            fraction_to_interpolate = (min_accretion_rate - logMdot)/(min_accretion_rate - point2_mdot)

            interpolated_therm_factor = point1_therm_factor + fraction_to_interpolate*(point2_therm_factor - point1_therm_factor)
