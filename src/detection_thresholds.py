...
min_mdot_bank = {
    # Calculate this using the find_mdot_cutoff_as_function_of_teff function in
    # find_synthetic_wd_mass_cutoff.py
    "Hollands": {
        "DB": {
            "gradient": 0.00102,
    ...
    "Default": {
        "DA": {
            # These values are not wrong, but are a bit inefficient because it means we
            # will likely forward model a lot of systems with no detectable pollution
            "gradient": 0,
            "yintercept": -np.inf,
            "leeway": 0,
        }
    ...
    "Realistic": {
        # These values are not wrong, but are a bit inefficient because it means
        # we will likely forward model a lot of systems with no detectable pollution
...
threshold_bank = {
    # Used for defining detection thresholds if
    # observation_type == ObservationType.TeffIndividualElementCutoff
    "Default": {
        "DA": {
            # 1st element is the gradient, 2nd element is the y-intercept
            ci.Element.Al: (0.000384039372, -(14.1531876 - 1.337685817)),
    ...
    "560mA": {
        "DA": {
            # 1st element is the gradient, 2nd element is the y-intercept
            ci.Element.Al: (0.00038403937, -(12.5511276 - 1.337685817)),
    ...
    "Hollands": {
        # For present purposes I only care about the Ca, Fe and Mg thresholds.
        # Everything else is set to arbitrary levels (makes no difference).
        # Could expand later!
        "DB": {
            ci.Element.Al: (0, 0),
            ci.Element.Ti: (0, 0),
    ...
    "HollandsCr": {
        # The same as above but now with Cr detectable, to see if it matters
        "DB": {
            ci.Element.Al: (0, 0),
            ci.Element.Ti: (0, 0),
            ci.Element.Ca: (0.00101, -16.6),
    ...
    "ELB_DT": {
        # These are the 'real', not the conservative, limits.
        # Interpolated between 10,000 and 20,000 K
        "DA": {
    ...
    #'v1': {
    #    'DA': {
    #        # 1st element is the gradient, 2nd element is the y-intercept, to 4sf
    #        ci.Element.Al: (0.0001714, -10.86),
