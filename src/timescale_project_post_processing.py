...
def combine_stats(stats_1, stats_2):
    ...
    if stats_1 is None or stats_2 is None:
        ...
        for stat_index in [0, 1]:
            if (stats_1[stat_index] == 'N/A') ^ (stats_2[stat_index] == 'N/A'): # Exclusive OR
...
def process_entry(entry, grid1, grid2, thermohaline):
    ...
    results_dir_1, file_1 = get_results_dir_and_file_name(entry, entry_dir, grid1, thermohaline)
    if not thermohaline:
        results_dir_2, file_2 = get_results_dir_and_file_name(entry, entry_dir, grid2, thermohaline)
    else:
        results_dir_2, file_2 = get_results_dir_and_file_name(entry, entry_dir, grid1, False)
...
def output_rows(list_of_rows):
    with open('timescale_project_postprocess.csv', 'w', newline='', encoding='utf-8') as f:
        to_write = csv.writer(f)
        to_write.writerow(['Entry','Sigma Overturned','Sigma Unoverturned','Delta Time 1 (Myr)','tau Mg 1 (yr)','D 1','Delta Time 2 (Myr)','tau Mg 2 (yr)','D 2'])
        for row in list_of_rows:
            to_write.writerow(row)

def plot_metric_prediction():
    metric_overturned = [1.8, 1.8, 1.8, 1.3, 1, 1.8, 1.3, 2.6, 7.3, 9.1, 9.1, 9.1, 9.1, 3.3, 3.4, 3.6, 3.1, 3.5, 7.9, 7.9, 3.9, 1.2, 1, 2, 1.5, 1.1]
    sigma_overturned = [1.35966916, 1.35966916, 1.49955924, 2.562796205, 1.407278135, 1.401395077, 2.140207376, 1.26684019, 2.192559182, 4.408530133, 4.408530133, 4.274483102, 4.274483102, 4.685945573, 3.636892944, 2.399284553, 1.245786305, 2.717599212, 1.407278135, 2.680665767, 1.401395077, 1.661239502, 2.439914937, 1.921626685, 2.009237626, 1.270904128]
    metric_unoverturned = [1.3, 1.3, 1.9, 1.9, 1.9, 1.9, 1.1, 1.1, 1.3, 1.3, 1.3, 1.3, 1.5, 1.5, 1.5, 1.5, 2, 2, 1, 1, 2.8, 2.8, 1.3, 1.3, 1, 1, 2.3, 2.3, 2.3, 2.3, 6.8, 6.8, 6.8, 6.8, 1.1, 1.1, 1.8, 1.8, 1, 1, 1.5, 1.5, 2.8, 2.8, 2.7, 2.7, 3, 3, 3.3, 3.3, 3.4, 3.4, 4.2, 4.2, 3.6, 3.6, 3.2, 3.2, 2.6, 2.6, 2.6, 2.6, 2.1, 2.1, 11.3, 11.3, 11.3, 11.3, 1, 1, 3.6, 3.6, 3.6, 3.6, 1.3, 1.3, 1.3, 1.3, 1.4, 1.4, 1.6, 1.6, 1.6, 1.6]
    sigma_unoverturned = [5.93830883, 6.070770433, 8.852872431, 9.254754982, 8.065222436, 7.5112284, 16.03050551, 16.73824838, 2.904947043, 2.614783563, 6.026608289, 5.286614302, 3.794235581, 4.408530133, 3.794235581, 4.408530133, 3.490238854, 2.723185747, 3.361971651, 3.159852496, 10.97029122, 11.06997159, 6.332186616, 6.493544509, 2.770069303, 2.680665767, 3.121450785, 2.017806324, 3.437825175, 2.017806324, 5.944501108, 6.373208649, 5.944501108, 6.373208649, 1.439628473, 1.851318457, 2.057168595, 2.487228715, 5.382709731, 6.070770433, 2.366261061, 1.752728128, 16.6477865, 16.73824838, 16.35001836, 16.98119396, 1.337698271, 2.007598521, 3.346083396, 4.685945573, 8.781783749, 9.580226671, 5.226961048, 4.696653494, 4.587500593, 2.615399381, 16.57337666, 16.99112206, 3.361032456, 4.951153208, 3.498240605, 4.951153208, 1.649503024, 1.128966, 31.28482972, 31.24975509, 11.07853148, 10.78170478, 3.861457164, 2.773061662, 7.825539286, 8.977025944, 7.825539286, 8.977025944, 2.128206891, 2.995845198, 2.1675833, 2.995845198, 6.0862972948038, 5.50673590356409, 1.67688789460071, 5.91218518640607, 2.22030121464052, 5.88465037756019]
    series_dict = {
        'Unambiguous': {
