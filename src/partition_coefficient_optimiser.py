...
target_logds_dict = {
    "Earth": {
        ...
        ci.Element.Fe: 1.136,
        ci.Element.Cr: 0.195,
        ci.Element.O: -0.6505,  # For use with Badro O Calculation:
        # Badro 2015 gives core O as 2.7-5 wt%.,
        # i.e. ~ 9.42 - 17.45 mol %,
        # i.e. 0.1344 +- 0.0402.
...
target_logd_errors_dict = {
    "Earth": {
        ...
        ci.Element.Co: 0.013,
        ci.Element.Pb: 0.118,
        ci.Element.Fe: 0.013,  # This is arbitrary
        # ^ No error was given because it's well constrained!
        # ^ Just made it equal to Ni's error (should maybe be Co?)
        ci.Element.Cu: 0.099,
        ci.Element.V: 0.042,
        ci.Element.Cr: 0.175,
        ci.Element.Mn: 0.274,
...
def chi_squared(ds, target_name):
    ...
    for element, target_value in target_logds.items():
        ...
        penalty = (((target_value - calculated_value) ** 2) / (error**2)) * weight
        # ^ Should probably divide by degrees of freedom => reduced chi squared
