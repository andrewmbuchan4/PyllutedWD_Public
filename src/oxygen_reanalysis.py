...
def get_sample_dict():
    ...
    # From Trierweiler, we exclude WD1226+110 (alias of SDSSJ1228+1040) WD1248+1004
    # (alias of 1248+1005) Ton345 (alias of 0845+2257).
    # Should note these aliases in the write-up! They are actually not excluded at all!

    # The IDs of the systems which have O, Mg, Fe, Si detections or upper bounds
    # i.e. the main rock-forming elements)
    ...
    # GALEX1931+0117 = 353 REMOVE: O/Si higher than for 352
    # ^ NB: Marc used this set of data instead of 352!!! He cited the wrong source
    # ^ (i.e., he cited Vennes but used the Melis data) OBSOLETE
    # GALEXJ2339 = 260 OLD
    # GD133 = 522 NEW
    # GD378 = 261 OLD
    # GD40 = 358 OLD
    ...
    # SDSSJ0956+5912 = 462
    # ^ (note: not GTC, which would have lower O) (should probably use GTC then!)
    # ^ REMOVE OBSOLETE
    ...
    # WD1145+017 = 472 (Budaj, would be the more conservative choice!)
    # ^ NEW DATASET, OLD SYSTEM
    # WD1232+563 = 373 OLD
    # WD1244+498 = 525 NEW
    # WD1350-162 = 377 OLD
    # WD1415+234 = 527 NEW
    ...
    # Now adding ones from the newest version of Laura's data.
    # (Which also removes GaiaJ0611-op-S = 536, and WD0611-6931 by proxy)
...
def compile_oxygen_stats(sample_dict):
    ...
    for name in sample_dict:
                ...
                if len(row) > 0:
                    ...
                    if row[0] == "Results from model:":
                        if row[1] == best_heated_model_name:
                            ...
                            reading_from_correct_section = False
                    if (
                        row[0] == "Median excess (default)"
                        and reading_from_correct_section
                    ):
                        median_fractional_excess_oxygen = float(row[1])
                    if (
                        row[0] == "Upper error excess (default)"
                        and reading_from_correct_section
                    ):
                        median_fractional_excess_oxygen_upper_error = float(row[1])
                    if (
                        row[0] == "Lower error excess (default)"
                        and reading_from_correct_section
                    ):
                    ...
                    if (
                        row[0] == "P(deficit) (default)"
                        and reading_from_correct_section
                    ):
                        p_deficit = float(row[1])
                    if (
                        row[0] == "Sigma excess (default)"
                        and reading_from_correct_section
                    ):
                        oxygen_sigma_significance = float(row[1])
                    if (
                        row[0] == "Build Up % (sampled):"
                        and reading_from_correct_section
                    ):
                        buildup_pc = float(row[1])
                    if (
                        row[0] == "Steady State % (sampled):"
                        and reading_from_correct_section
                    ):
                        ss_pc = float(row[1])
                    if (
                        row[0] == "Declining % (sampled):"
                        and reading_from_correct_section
                    ):
                        dec_pc = float(row[1])
        ...
        ] = median_fractional_excess_oxygen
        sample_dict[name][
            "Median Fractional Excess Oxygen Upper Error"
        ] = median_fractional_excess_oxygen_upper_error
        sample_dict[name][
            "Median Fractional Excess Oxygen Lower Error"
        ] = median_fractional_excess_oxygen_lower_error
