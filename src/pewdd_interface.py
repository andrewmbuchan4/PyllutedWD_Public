...
def filter_wds_by_num_modellable_detected_elements(
    white_dwarfs, num_elements, detectable_elements=ci.usual_elements
):
    toret = list()
...
def filter_wds_by_teff_logg(white_dwarfs, min_teff, max_teff, min_logg, max_logg):
    ...
    for wd in white_dwarfs:
        ...
        if (
            ...
            and teff.value <= max_teff
            and teff.value >= min_teff
            and logg.value <= max_logg
            and logg.value >= min_logg
        ):
            toret.append(wd)
    return toret
