...
def ks_test(data1, data2):
    ...
    max_bin_edge = 0.64
    half_bin_size = 0.01
    ks_statistic, p_value = pipeline.run_ks_test_on_samples(
        data1, data2, min_bin_edge, max_bin_edge, half_bin_size
    )
...
def read_data_from_txt(filename):
    # This function assumes that the input file has no header, and two columns (of equal
    # length) separated by a space
    data1 = list()
    data2 = list()
