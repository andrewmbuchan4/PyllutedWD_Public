...
def get_hollands_abundances_for_dt_plot(element, manager=None):
    ...
    for wd in manager.white_dwarfs:
        if i in hollands_ids:
            abundance_info = wd.get_abundance(element)
            if abundance_info is not None and abundance_info.included:
                hollands_abundance_tuples.append(
                    (wd.get_teff().value, abundance_info.value)
                )
        i += 1
...
def extract_posteriors(wd_name):
    ...
    # Step 2: Read the best_fits file to find out what the parameters for the best model
    #         was
    # Step 3: Look up what that model would have been called in PWDCode.py
    #         (nb: not necessarily the same as the model name listed in the best fits
    #         file) by finding the matching params file
    ...
    with open(
        ...
        for row in csv.reader(bfcsv):
            if row[0] == wd_name:
                ...
                model_parameters = [
                    model_parameter.replace("'", "").lstrip().rstrip()
                    for model_parameter in model_parameters_raw
                ]
                break
    ...
    for matching_json_file in matching_json_files:
        model_no = int(
            matching_json_file.split("model")[1]
            .split("params")[0]
            .replace("bu", "")
            .replace("ss", "")
            .replace("h", "")
    ...
    a = pn.Analyzer(
        n_params=n_dims,
        outputfiles_basename=pu.get_path_to_historical_output_dir() + model_basename,
    )
    stats = a.get_stats()
    weightpost = a.get_equal_weighted_posterior()[:, 0:n_dims]
...
def get_median_t_t_event_myr(wd_name):
    (
        ...
        weightpost,
        log_likelihood,
        bes_fit_parameter_values,
        model_parameters,
        model_number,
    ) = extract_posteriors(wd_name)
    ...
    accretion_timescale = (
        10 ** np.percentile(weightpost[:, accretion_timescale_index], 50)
    ) / 1000000
...
def get_hollands_common_el_values(list_of_elements, manager=None):
    # Returns lists of element abundances for all wds that have ALL the specified
    # elements, such that the length of each list is the same
    if manager is None:
        manager = load_manager()
    toret = list()
...
def main():
    ...
    print(len(ni_values))
    print(
        get_hollands_common_el_values(
            [ci.Element.Ca, ci.Element.Fe, ci.Element.Mg, ci.Element.Cr]
        )
    )
