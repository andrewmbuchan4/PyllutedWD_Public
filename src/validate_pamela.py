...
experiments = {
    "Corgne": {
        "PL-169": {"P": 3.6, "T": 2123, "fO2": -0.8, "nbot": 2.1},
        "PL-185": {"P": 3.6, "T": 2273, "fO2": -0.9, "nbot": 2.1},
        "PR-369": {"P": 3.6, "T": 2473, "fO2": -1.1, "nbot": 2.1},
        "PR-373": {"P": 7.7, "T": 2273, "fO2": -0.6, "nbot": 2.2},
        "PR-376": {"P": 3.6, "T": 2273, "fO2": -3.2, "nbot": 1.8},
        "PR-383": {"P": 3.6, "T": 2273, "fO2": -3.1, "nbot": 1.7},
        "PR-375": {"P": 3.6, "T": 2273, "fO2": -3.0, "nbot": 1.7},
        "PR-368": {"P": 3.6, "T": 2473, "fO2": -3.1, "nbot": 1.8},
        "PR-365": {"P": 7.7, "T": 2273, "fO2": -3.2, "nbot": 1.7},
    },
    "Fischer": {
        "DAC1": {"P": 31, "T": 4250, "fO2": -1.0},
        "DAC2": {"P": 39, "T": 3600, "fO2": -1.2},
        "DAC3": {"P": 56, "T": 4360, "fO2": -1.1},
        "DAC4": {"P": 57, "T": 4440, "fO2": -0.9},
        "DAC5": {"P": 100, "T": 5700, "fO2": -1.1},
        "MAP1": {"P": 25, "T": 2740, "fO2": -2.2},
        "MAP2": {"P": 25, "T": 2740, "fO2": -2.7},
        "MAP3": {"P": 25, "T": 2850, "fO2": -2.4},
        "MAP4": {"P": 25, "T": 2850, "fO2": -2.8},
        "MAP5": {"P": 25, "T": 2860, "fO2": -2.3},
        "MAP6": {"P": 25, "T": 2950, "fO2": -2.1},
        "MAP7": {"P": 25, "T": 2950, "fO2": -2.5},
        "MAP8": {"P": 25, "T": 2950, "fO2": -1.9},
        "MAP9": {"P": 25, "T": 2950, "fO2": -3.1},
        "MAP10": {"P": 25, "T": 2950, "fO2": -2.6},
        # "MAP11": {"P": 15, "T": 2807, "fO2": -3.7},  # This one fails to converge
        "MAP12": {"P": 15, "T": 2807, "fO2": -2.5},
        "MAP13": {"P": 15, "T": 2802, "fO2": -2.0},
        "MAP14": {"P": 15, "T": 2720, "fO2": -2.8},
        "MAP15": {"P": 20.5, "T": 2727, "fO2": -2.8},
    },
    "Test cases": {
        "Earth (assumed)": {
        ...
        "Earth": {"P": 49.5, "T": None, "fO2": -1.4},
        "Mars": {"P": 2.5, "T": None, "fO2": -1.19},
    },
}
...
results = {
    ...
    "Test cases": {
        "Earth (assumed)": {
            ...
            # McDonough 2003 implies this^ should be somewhat higher
            ci.Element.O: 10**-0.6505,
            # ^ For use with Badro O Calculation:
            # Badro 2015 gives core O as 2.7-5 wt%., i.e. ~ 9.42 - 17.45 mol %,
            # i.e. 0.1344 +- 0.0402.
            # Take mantle O as 0.6011.
            # D = core concentration/ mantle conc. = 0.2236 +- 0.0669,
            # so logD = -0.6505 += 0.1544 ish
            # (taking lower bound to give largest error, to be conservative)
        ...
        "Mars (assumed)": {
            # Values taken from table 10 in Yoshizaki 2020. D = Core/BSM
...
errors_on_logD = {
    # NB these are errors on log(D), not on D!
    "Test cases": {
        "Earth (assumed)": {
            ci.Element.W: 0.077,
            ci.Element.Ni: 0.017,
...
observed_mantle_abundances = {
    ...
    "Mars (assumed)": {
        # This is by number. Yoshizaki 2020
    ...
    for ex_name, ex_runs in experiments.items():
        ...
        for run_name, run_params in ex_runs.items():
            ...
            if run_name == "Mars":
                ...
                a, w, Ds, all_Ds = geo_model_earth.form_a_planet_iteratively(
                    P, fO2, T, nbot
                )
            ...
            for element, element_result in result.items():
                ...
                try:
                    log_error = (
                        log_prediction - log_element_result
                    ) / log_element_result
                except (TypeError, ZeroDivisionError):
                    log_error = None
                print(
                ...
                try:
                    observed_mantle_abundance = observed_mantle_abundances[run_name][
                        element
                    ]
                except KeyError:
                ...
                try:
                    error_ma = (
                        predicted_mantle_abundance - observed_mantle_abundance
                    ) / observed_mantle_abundance
                except (TypeError, ZeroDivisionError):
                    error_ma = None
                print(
            ...
            if ex_name == "Test cases":
                ...
                print(Ds)
                print(w)
                graph_fac.make_Ds_comparison_plot(
                    results[ex_name][run_name],
                    Ds_dict[ex_name][run_name],
                    "sisi",
                    run_name,
                    errors_on_logD[ex_name][run_name],
                )
